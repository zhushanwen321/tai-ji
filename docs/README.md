...
taiji 的文档统一存放于 `docs/`。按内容性质分目录，全项目共享。

> 文档分层理念：**局部信息进代码**——注释只写「为什么做/有什么坑/有什么好处」，不写「做了什么」；机器可校验的规则进 constraints.json / lint / hook，文档只留入口指针；docs/ 只保留全局视角内容（跨模块串联、决策史、行为规范）。行级为什么进代码注释，决策级为什么进 ADR（代码内挂 `// ADR-xxxx` 锚点回链）。
...
├── PRODUCT.md             ← 产品愿景/用户画像（tech-design 设计期边界核对）
├── ARCHITECTURE.md        ← 系统架构总览（进程拓扑 / 数据流 / 核心约束）
├── CONTEXT.md             ← 领域术语表（统一语言）
├── DESIGN.md              ← 视觉设计权威（v6 范式 + token 登记对照 + traffic light 布局）
├── STANDARDS.md           ← 前端编码规范（原则层；lint 已强制的规则不重复转述）
├── TEST-STRATEGY.md       ← 测试策略 SSOT（分层/mock/回归基线）
├── FEATURE-PRIORITIES.md  ← 功能分级 P0-P3（风险打分锚定 + 回归排序）
├── TROUBLESHOOTING.md     ← 问题排查（症状 + 命令 + 指针）
├── design-evolution.md    ← UI 设计演变史（Warm&Soft → v3 → v6 → 太极纯灰）
├── release-notes.md       ← 发布 notes 写作规范 + 待发布草稿暂存
│
├── adr/                   ← 架构决策记录（决策级「为什么」唯一家园；decisions.md 单文件 SSOT，沿用 ADR 编号）
...
> 功能开发地图已并入 [architecture/feature-map.md](architecture/feature-map.md)（滚动快照，只留最新一份）。
...
- [constraints.json](./constraints.json) — **架构约束登记表 SSOT**（机器权威，结构校验经 `node scripts/validate-constraints.mjs`；CR 动态加载经 `node scripts/select-constraints.mjs --base main`）
- [ARCHITECTURE.md](./ARCHITECTURE.md) — 系统架构总览
- [PRODUCT.md](./PRODUCT.md) — 产品愿景 / [CONTEXT.md](./CONTEXT.md) — 领域术语表
- [DESIGN.md](./DESIGN.md) — 视觉设计权威（含 traffic light 布局 §11）
- [STANDARDS.md](./STANDARDS.md) — 前端编码规范
- [TEST-STRATEGY.md](./TEST-STRATEGY.md) — 测试策略 / [FEATURE-PRIORITIES.md](./FEATURE-PRIORITIES.md) — 功能分级
- [TROUBLESHOOTING.md](./TROUBLESHOOTING.md) — 问题排查指南
- [design-evolution.md](./design-evolution.md) — UI 设计演变史
- [adr/decisions.md](./adr/decisions.md) — 架构决策 SSOT（现行有效决策，沿用 ADR 编号）
...
- taiji-harness 工作流产出物（spec / plan / test / retrospect / review / impl-plan）→ 本地 `.taiji-harness/<date>-<slug>/`（gitignored，不入库）
- 单模块设计文档 → 蒸馏进代码注释后删除（见上文归属判定）
- 一次性审查日志、已完成的修复记录 → 完成后删除（git 可追溯）
- UI demo / HTML 设计稿 → `.tmp/`（不入库；禁止散落仓库或 `demos/`、`impeccable/` 目录）
- 竞品/技术调研资料 → 本地归档目录（不进 git）
