...
pi 的消息 content 是数组；taiji 侧 `Message.content` 为 `string | Segment[]`（ADR-0043：user 消息富内容化为 Segment[]，`normalizeContent()` 归一化；类型 SSOT 在 `packages/shared/src/message.ts` 注释）。转换职责在 `packages/runtime/src/infra/pi/` 适配层（EventAdapter / session-entry-mapper），规则细节以适配层代码注释为准，本节不维护逐字段转换表。
...
前端 mock 有且只有一个合法入口：**`packages/core/src/transport/mock/` 层**（@taiji/core）。该层模拟 runtime WS 协议返回的数据（`session`、`chat`、`config`、`model`、`extension`、`plugin`、`settings`）。`api/index.ts` 通过 `VITE_MOCK` 环境变量切换 real/mock 实现（true 时直接 import `@taiji/core/transport/mock`，不走 transport）。
...
- **禁止组件直接 `import` `@taiji/core/transport/mock` 下的任何文件** — 组件应通过 `@/api` 或 `events` 获取数据
...
3. 运行 `pnpm --filter @taiji/frontend run typecheck` 和 `pnpm --filter @taiji/runtime run typecheck` 验证
...
错误处理的判定单位是「错误路径的业务角色」，不是通用套路。对每个错误路径先判定角色，再核对实现策略，不匹配即缺陷（表述为「应 X，实际 Y」）。机械缺陷（吞错 / Promise 未 await / 错误后状态未重置）按常规处理，本契约约束的是**策略选择**。

### 11.1 角色 → 策略对照

| 业务角色 | 正确策略 | 要点 |
|---------|---------|------|
| 核心功能（主流程、数据完整性、状态机、持久化） | 快速失败（fail-fast） | 异常上抛或显式落败并停止推进；禁止吞错、禁止静默降级产出错误数据；错误日志可操作（错误 → 原因 → 恢复动作） |
| 辅助功能（通知、统计、装饰渲染、widget、上报） | 主流程接入点降级 | 辅助故障不得拖垮核心主流程；降级 ≠ 吞错——必须 warn 留痕，下游能消费降级产物 |
| 用户可见的辅助能力 | 降级 + 可见失败反馈 | 返回合理结果 + 失败原因（toast/错误气泡/可区分文案），不给静默空值 |

### 11.2 两条分通道纪律

1. **读失败 vs 合法缺省**：文件不存在/域未启用属合法缺省（debug 或静默，用 ENOENT 等 errno 判别）；权限、损坏等其他 IO 故障必须 warn 留痕——把 IO 故障伪装成 not-found 会误导排障。
2. **静默丢弃必须登记**：对坏输入选择「静默丢弃」时，要么注释声明为设计裁决 + 测试锁定行为，要么补日志；「静默且无任何登记」态不允许（无法区分有意取舍与漏写）。

### 11.3 两种伪防御（按反缺陷对待）

- 把设计内合法产出当错误拦截（空列表、可选字段缺失、设计内空态）
- 吞掉会损坏状态/产生错误数据的错误继续跑（伪鲁棒）
