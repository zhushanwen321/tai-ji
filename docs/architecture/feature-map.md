...
- 设计已实施全单元落毕（U1-U9 + u-foundation + U6b/U8a/U8b，dev-flow），偏差 40+ 条登记于 impl-plan §5（`.taiji-harness/`，不入库）；约束已回写 C-data-20 / C-data-22 / C-proc-13。
...
| D4/D5 历史预算 | 读侧：`services/session-history.ts` + `services/session/history-reverse-read.ts` + `infra/pi/session-file-streaming.ts`（32MB 预检五档 + 逆序分块尾读）；传输侧：20 turns / 640KB 双预算 + 游标分页（`session.history` RPC cursor 参数，core `useChat.ts` 翻页 + `truncated-window.ts` 窗口态 SSOT）；「加载更早」prepend + **触顶自动续载**（renderer `useLoadMoreHistory.onScrollOffset`，2026-09-19） |
...
| 集成测试 | `packages/runtime/test/`（real-pi 端到端回收集成用例，CI 侧 TAIJI_SKIP_REAL_PI=1 仅本机真跑） |
...
| E5b 看门狗 | `packages/runtime/src/infra/watchdog.ts`（60s 采样环 + 70%/85% 两级阈值 + relief 持续性条件 + armed 门）；renderer `useMemoryPressure`（refCount 单订阅 + core `setLruMaxSessions` 可变窗压窗）；约束 C-proc-19（`TAIJI_RUNTIME_WATCHDOG_ARMED` 默认 off = Gate W 纯观测） |
...
| 批次补课 | tee size 轮转：`infra/logger.ts`（gzip `.1.gz` 单代 + `TAIJI_LOG_MAX_BYTES` 单旋钮，C-build-09，2026-09-13 改号）；入站 parse 守卫：core `ws-client.ts` 80MB + 单 session 终止阀（C-comm-15）；O3-C superseded-by 注记 + `scripts/check-stale-ctx-audit-coverage.mjs` 审计覆盖守卫；reap 判据 v2 marker 化（C-proc-20） |
...
- v3 视觉重建 / 双 Panel：见 2026-06-20.md（已删除，git 可追溯）
