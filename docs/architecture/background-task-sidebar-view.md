...
**设计目标**（从使用者体验倒推）：
1. **G1 可见**：用户在该 session 的任务观察入口（设计期 = plugin 区「后台命令」L2 tab，三桶筛选：运行中/已结束/全部；现 = composer 任务托盘 bash 面板，两桶：运行中/已结束）默认只看运行中的任务（+ 计数预告），状态实时翻转（running→killing→exited）无需手动刷新；入口计数（原 L2 tab 角标，现托盘 icon 计数）与「运行中」桶计数同源（亮 = 有命令在跑 = 默认桶非空）。
...
用户点击运行中的行 → 右侧 drawer 打开「后台命令」tab：
  命令全文（pnpm test --filter @taiji/frontend，可复制）
