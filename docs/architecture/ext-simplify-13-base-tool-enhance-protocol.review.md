...
| F11 | runtime tsup noExternal 已含 protocol（tsup.config.ts:57）；bte dependencies 已含 protocol（§3.2 证据） | runtime `tsup.config.ts:57` 精确命中；bte package.json dependencies 含 `"@taiji/extension-protocol": "workspace:*"`；protocol 有 publishConfig（发 npm dist/index.mjs）——bte「独立 pi 用户可单独安装」定位不受新增影响（无新增依赖边） | 属实 |
...
| N3 | bte 独立安装定位是否被 protocol 依赖破坏（方向 A vs B 核心取舍） | 不破坏：bte package.json dependencies **已含** `@taiji/extension-protocol: workspace:*`（现状，非本设计新增）；protocol 有 publishConfig（dist/index.mjs，version 0.9.0）随发布管线发 npm，独立用户安装时为普通传递依赖。「两侧零新增依赖边」声称属实 |
