...
- **S（情境）**：taiji 是 Electron + Vue 3 + Node runtime 的 AI Agent 桌面工作台。renderer 持有 session 状态副本，runtime/pi 持有真值，两者经 WebSocket 广播 + RPC 同步。landing 页（新建任务空态）的 composer 有一排 chip：模型、思考档位、预设、目录。
...
- pi 源码（MANDATORY 不改 pi；所有适配在 taiji 侧）
...
- **采用**：全库 ~45 处对账机制按下表三分处置。**判据**：该机制对抗的竞态根源是「外部异步真值」（pi 事件时序/钳制/静默换模/进程死亡——taiji 无法消灭的）还是「自致发散」（双链解析/乐观写无回执/多真源——本设计根修的）？
...
**改动规模：大（架构行为变更）→ 多场景真实验证。验证环境：`pnpm dev` 起真实 Electron app（dev renderer :1420），真实 pi 子进程，真实数据目录 `~/.taiji-dev`；浏览器侧用 browser-automation 连 :9222 截图断言；runtime 侧用日志探针。**
