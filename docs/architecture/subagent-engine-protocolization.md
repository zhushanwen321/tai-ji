...
> - v5（2026-09-08，R4 修复）：①**env 契约改四层**（L0 基础设施键 core 后置注入 > L1 deny/剥除（含凭证 `TAIJI_AGENT_API_KEY`）> L2 manifest 放行（保留前缀拒绝表）> L3 extras）+ **relay 按实装名**（SOCKET/NODE/SCRIPT 透传；SESSION_ID/RECORD_ID 剥除并覆写）+ 基座常量**构建期从 shared SSOT 生成**（不引入 core→shared 运行时依赖）；
...
>   ⑥启动解析执行器单一 env `TAIJI_AGENT_ENGINE_NODE` 经 L0 注入（不依赖继承）+ `ELECTRON_RUN_AS_NODE` 归 L0；
...
>   ②**自灭主判据改 stdio EOF**（stdin EOF/EPIPE），超时只对 **in-flight 反向请求**计时（env 改 `TAIJI_ENGINE_HOST_REQUEST_TIMEOUT_MS`），A8④ 补「静默长任务不被自灭」负向验收（ADR-0047 反向通道版）；
...
**系统是什么**（给不熟悉内部的读者）：`subagent-core` 是跨宿主（taiji 的 pi 壳 / zsw 的 zcode 壳）
...
| P6 | 动态发现只到「清单」 | `engine-discovery.ts` 从**代码注册表**生成 `engines.json`；扩展 `package.json.taiji.subagentEngines` 只是**冷启动回退清单**（`runtime/session-records.ts:334-392`） | 有「清单」但没有「装载」 |
...
**引擎包 manifest**（写进引擎包自己的 `package.json`；下例为**示意**，现行权威取值以各引擎包 `package.json` 的 `taiji.subagentEngine` 为准——如 `packages/zcode-subagent-cli` 现值 `sandbox:none / permissionMode:native / conversation:cold`）：
...
| `buildEngineChildEnv()`（env 契约） | F9：`buildOutboundChildEnv` 在 `@taiji/shared`（zsw 不可依赖）——SDK 是跨宿主 SSOT |
...
**数据根注入矩阵（单一 env 名，消灭双名风险）**：引擎数据根**只认实装的 `TAIJI_AGENT_DATA_DIR`**
（`data-dir.ts:26-46` 读它，缺省回退 `HostServices.dataRoot()`）——**D8 原新造的 `TAIJI_AGENT_ENGINE_DATA_DIR` 删除**
（全仓无实装，双名会导致 pi 宿主引擎与 runtime 引擎指向**不同隔离库**→ ①级读静默降②级）。

| 宿主 | 注入方式 |
|------|---------|
| pi 扩展进程（taiji） | **core 侧解析 `getEngineDataDir()` 并注入**（不依赖继承）：`process-manager.ts:119-140` 已注入 `TAIJI_AGENT_DATA_DIR: getConfigDir()`，引擎 spawn 时仍显式写入同一值 |
| **standalone pi（env 缺省）** | 宿主 env **无** `TAIJI_AGENT_DATA_DIR`（`getEngineDataDir()` 回退 `HostServices.dataRoot()` = `getAgentDir()`，`data-dir.ts:56-70` / `pi-host.ts:120-128`）→ **必须由 core 侧解析后注入**；`resolveEngineDataDir(env)` 的缺省语义写死 = **缺 env 且无注入 → 显式报错 `engine_not_found`（附期望路径）**，不静默回退 |
| runtime 进程 | `getDataDir()`（= `TAIJI_AGENT_DATA_DIR`）自身即权威 |
| zsw CLI | D8 deps 映射：`engineDataDir()` → 子进程 env `TAIJI_AGENT_DATA_DIR`（若与宿主值不同则 warn + 以显式值为准） |
...
| 旧 deps 字段 | 新落点 | 说明 |
|-------------|--------|------|
| `engineDataDir(): string` | 子进程 env **`TAIJI_AGENT_DATA_DIR`**（与实装同名，见 §3.6 注入矩阵） | **引擎数据根**（隔离库/池/journal 都相对它推导）；与 `initialize.hostInfo.dataRoot`（**宿主**数据根）区分；两者不同值时以显式值为准 + warn |
...
② zsw 显式注入 `TAIJI_AGENT_ENGINE_ROOTS`（一行 env）。
...
| `<engineDataDir>/logs/zcode-appserver-stderr-<pid>.log` | 引擎包 | append 累积 | **文件名带实例维度**（同路径双实例并发写入会 rename 竞争）；清理通道 = 引擎包自实现，判据 = 同前缀 + **pid 已死（OS 存在性探测，不是进程内表）** + mtime 过期（三者同时成立才删）；参数读宿主 `TAIJI_LOG_*`（缺省 50MB / 7 天）；宿主 logger 看不见它 → **W11 落地 + A13**；实现级判据由代码承载（原 impl-plan §2.11 已删除，git 可追溯；入口 = 引擎包 stderr 轮转实现） | 可接受（通道自建） |
...
> 每个场景都在真实宿主上跑（taiji dev / zsw CLI）；fake 引擎只用于协议单元回归。
>
...
| A7 | 宿主零改动 | taiji 扩展与 zsw **业务逻辑**不改；允许的机械改动 = 依赖升级 + vendor 刷新 + 发现根注入 | A1-A6 通过；扩展 `index.ts:37/103` 与 zsw `runner-core.js:75/161/428` 的**业务调用点未改**而行为不变；zsw deps 映射后 `engineDataDir` 无漂移 | G3 |
