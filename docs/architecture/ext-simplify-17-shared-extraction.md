...
- 修法：`subagent-guard.ts` 判据改锚 `TAIJI_AGENT_SUBAGENT === "1"`；谓词与常量收敛进 ext-guards（建议新增 `isSubagentProcess()` + `SUBAGENT_MARKER` 导出；**实施终名 = `isSubagentProcess(env?)`（可选 env 形参便于测试注入）+ `SUBAGENT_MARKER_ENV`**，ENV 后缀更准确——值是 env 键名非标记值），bte 改 import。smart-context `pure.ts:258-260` 的单键版同谓词顺带收敛（其消费语义与 bte 同源）。
