...
> taiji 引擎数据目录下的**独立会话库**（spawn env `ZCODE_SESSION_DB_PATH`，**已真机探针验证**），
...
>   ⑥§2.4.1 跟踪落点落成 W4 子项 companion-surface-governance 文档 + 余量基准改写；⑦**派生行（4 条）纳入删除集**；
...
- **Q（问题）**：怎么让 taiji 派发的 zcode subagent 会话**不再污染用户侧边栏**，
...
| G5 | 残留可回收 | 隔离库归 taiji 独占，具备可判定的清理通道（我们拥有全部行 + 存量宿主行可用自己的 record 白名单定位）；**口径 = 「行可回收」**（磁盘回收另需 VACUUM） |
...
```
taiji 派发 subagent(engine=zcode)
   │
...
| F8 | 当前 taiji 已产生 50 条 zcode 会话（今日 10:34–13:17），全在宿主库；这 50 条**当前 0 条**在索引里——因为所在三个 worktree 不在 GUI workspace 列表 | 双库交叉 + `recentProjects` 对比 | 【实测】 |
...
- **启动条件落成可验证项**：owner = **合入 PR 作者**；期限 = **G1 止血合入后 30 天内**；跟踪位置 = **W4 的子项 companion-surface-governance**——原计划以合入 PR 新增独立文档承载，该文档原档案族已清理，owner/期限/上界/复测命令四要件即以本节登记为准（逾期未排期即**升为阻塞项**，检查主体 = 下一次触碰该面的设计/PR 评审）；
...
| **分支 B：用户判「不可接受」** | 候选① 必须成为 §5 的 W 单元（含 owner/期限）**且同批补「宿主库新表写入面」分析**（对方 schema 迁移容忍度 / 消费方 / 清理通道——§2.4.1 未穷举该面）；或改选候选②（taiji 侧自建视图，不写宿主库） |
...
- **继承时序（写实）**：taiji 合入 → core 版本 bump → zsw re-vendor（**版本门槛 = 本设计合入的 core 版本**）
...
- **dataDir 权威源（含传播前提，勿当同源事实）**：两站点各自用**它打开数据库时所用的同一个 dataDir** 构造集合——
  生产链用 runtime 传入的 `dataDir`（`getDataDir()`，缺省 `~/.taiji`），API/写侧用 `deps.engineDataDir()`
  ...
  **前提**：该 env 必达；任一 spawn 点剥掉它，写侧落 `<piAgentDir>/engines/zcode/session-db/db.sqlite`
  ...
  兜底：单元断言「taiji spawn 配置下两站点 dataDir 相等」+ D3 单列该形态。
...
- **归属**：隔离库只被 taiji 的 zcode 引擎写（GUI 不认这个路径）→ **全部行都是我们的**。
...
**独立参照 SQL（不依赖工具自算）**：**本节以下代码块即权威文本**（原 `docs/design/probes/zcode-session-db/counts.sql` 已删除并入正文；跨双库：宿主库直接集/派生集 + 索引库四冲突源只读预检；「W5 参照查询」节为 D7 I1 的单一文本来源，勿在别处复制），工具自算值必须等于该参照；不相等则逐条归因。
...
-- ── 2) F8：taiji 自身产生的会话与其索引命中数 ─────────── 两库 ──────
-- 白名单来源 = taiji record 存储里的 engineHandle.sessionRef.sessionId：
--   grep -h -o '"sessionId":"[^"]*"' ~/.taiji/pi/sessions/*.jsonl | sort -u
...
-- 白名单：先用第 2 节的 `grep -h -o '"sessionId":"[^"]*"' ~/.taiji/pi/sessions/*.jsonl | sort -u`
...
--     const dir=path.join(os.homedir(),".taiji/pi/sessions");
--     const out=[];
