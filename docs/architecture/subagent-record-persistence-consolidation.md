...
| one-shot **纳管态**（resumable 且无 result，监督器死亡接管后重启）× parent-shutdown | **两形态分裂**（审查轮 2 影响面 MF）：优雅退出 = dispose 合成 result=""（空串非 undefined，merge 后不进纳管分支）→ 直断 gc；SIGKILL = 无合成 result → 保留 running 交 round-supervisor boot **自动重认领**（isBootReadoptable 只收 running，`domain.ts:79-88`；give-up 发 supervisorNotify 终止通知） | closed(parent-shutdown) → 重认领谓词不命中（status 已 closed）→ **自动重认领失效**，降级为手动 message resurrect（可重连） | **降级，已接受**：双巧合窄形态（监督器死亡纳管 + 宿主被 SIGKILL）；手动通道在 + fork-from/start fresh 兜底；supervisorNotify give-up 通知面消失登记（用户侧表现：无自动终止通知，需主动查看）；S1/S6 补该子类断言。**entry-born 作用域注记（Gate B F3 裁决，2026-09-12）**：本行「可重连」**仅有锚（pi）形态可达**——entry-born（zcode）record 无子 session 文件：message 冷查不可达（终态写点未跑、无 manifest）+ fork-from 守卫 6 结构性硬拒（无子文件可继承）+ 续跑链锚点守卫 throw（无 transcript 可续），承接通道仅 start fresh；其纳管态重启经 `finalizeEntryOnlyOrphan` 直断 closed/gc（Gate B S1-adopt 真机实证），错误文案已随裁决改为如实描述。根治向（ref 锚：zcode 引擎上报 sessionRef 使其获得自有 transcript 引用）立项另议 |
  ...
  **【2026-09-13 演进注记】[superseded by subagent-permanent-session-model §3.2.4/§3.2.5]**：终态概念已删除（状态机两态化 `running | idle`，closed 与 ClosedReason 的资格角色消亡），本节按落地事实演进为三条——①**终态原语全同步 → settle 收条写**：`markFinalized`/`markCancelled` 退役（@deprecated，仅 workflow-origin D7 例外族——finalizeFailed/finalizeAborted/settleOneShotOutcome 按 `origin==='workflow'` 分流终态化——经 finalize-record 保留调用）；正常收口归 `markSettled`：`.state` 新格式收条 `{status:"idle", stopReason?, endedAt?}`（writeSettledState），写序 `.state` 先 manifest 后**不变**，round/usage 轮间真相源移 binding（不再由 `.state` 冗余承载）。②**disposeAllRecords 行为变化矩阵被「编排性关闭 = 自动收起」取代**（U5）：intent=archived + stopReason=interrupted-by-parent + 立即打断在飞轮（回 idle 不终态化），旧 session 树内仍可 message 寻回——本矩阵「chat×parent-fork/new 硬拒收紧」行不再成立（收起非告别），「one-shot×shutdown 放宽」行被万物可续（锚判据，U4）整体覆盖。③**重建单规则**（U3）：孤儿恢复直断分支（SP-5 完成态直断 closed/gc、finalizeEntryOnlyOrphan 直断）删除，buildRecord 重建**一律得 idle**（stopReason 取自 `.state`，缺失视为 interrupted-by-restart），旧 finalized/cancelled 值读侧上行映射。manifest 双写投影的现行规则见 D5 演进注记；偏差全表见实施计划 §5。
