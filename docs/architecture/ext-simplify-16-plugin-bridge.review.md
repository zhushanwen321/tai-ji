...
- **A3 改挂真实观测物**：v1.1 :241 通过标准 = 插件工具注册成功 + `TAIJI_AGENT_DEBUG=1` 下桥侧 extension-logger 输出 `synced N plugin tool(s)`。复核 src/index.ts:290 `logger.debug(\`[plugin-bridge] synced ${result.tools} plugin tool(s)\`)` 逐字命中，观测物真实存在。
