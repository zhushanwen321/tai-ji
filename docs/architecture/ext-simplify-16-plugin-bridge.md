...
- **职责**（mandatory-extensions.json，tier=infrastructure，不可禁用）：register plugin tools into pi and relay events/intercepts via select marker channel。契约两端（pi extension ↔ runtime plugin-service）都在 taiji 体系内，故归 taiji 组。
...
**使用者 = 插件作者 + taiji 维护者**（bridge 是 infrastructure 通道，终端用户无感知面）。
...
**验收环境**：dev 实例（`TAIJI_DEV_BACKGROUND=1 pnpm dev`）+ fixture 插件（写入实例 plugins 目录的最小插件模块：1 个工具 `probe_echo` + 1 个 `before_agent_start` hook 返回 `["TOKEN_INJECT:<n>]"`）——bridge-rewrite Gate B V 系列同款机制。
...
| A3 | sync 快照消费 | 插件工具注册成功 + 桥侧 sync debug 日志可见（TAIJI_AGENT_DEBUG=1 下 extension-logger 输出 `synced N plugin tool(s)`）——D2 消费方透明性；「负载无 commands 键」由 N1 rg + u2 单测断言（r1 审查 S1：sync 回包经 pi stdin 静默 resolve、不在 stdout tee 内，无真机取证物，诚实以单测兜底） |
