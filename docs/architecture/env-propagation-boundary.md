...
在打包版 TaiJi.app 的 agent 会话里，对本仓库 worktree 执行 `git commit`：
...
3. 这个 runtime **继承了会话进程链上的 `TAIJI_AGENT_PACKAGED=1`**；
...
临时绕法是 hook 触发前 `env -u TAIJI_AGENT_PACKAGED`。绕法的存在说明缺陷已知但没有收敛机制，下一个新变量必然重演。已有反例先例：`terminal-service.ts` 曾因 `ELECTRON_RUN_AS_NODE` 泄漏污染用户终端，PR #105 做了点位删除，但同类思路没有推广到其他边界。
...
| G2 | 最小暴露面：进程自身状态标志与凭证不出产品边界 | 会话 bash `printenv` 与契约快照比对，无 `TAIJI_AGENT_PACKAGED` / `TAIJI_RUNTIME_TOKEN` |
| G3 | 未来新增任何产品 env 变量不再重蹈覆辙（防复发） | 新 spawn 点写漏构建器时 pre-commit 报错；透传必须有消费证据 |
| G4 | 反向边界（用户 shell → dev 链）既有防线不回归 | `TAIJI_AGENT_PACKAGED=1 pnpm run dev` 行为与现状一致 |
...
- **入站白名单（inbound allowlist）**：子进程**能从父进程继承什么**。现行实现 `ENV_WHITELIST_PREFIXES`（`packages/shared/src/constants.ts:72-81`）。例：runtime 能继承 `NODE_*`、`PATH`、以及整个 `TAIJI_*` 前缀。
- **出站契约（outbound contract)**：runtime **孵化子进程时主动给什么 / 不给什么**。现状各 spawn 点自行组装，无统一契约——本案要建的就是它。
- **进程自身状态标志（lifecycle-state flag）**：描述「我这个进程是怎么被拉起来的」的变量，如 `TAIJI_AGENT_PACKAGED`。只对持有它的进程有意义，穿越边界即为噪声。
- **功能契约变量（functional contract var）**：为下游功能服务的变量，如 `TAIJI_AGENT_DATA_DIR`（runtime 与 pi 内 extensions 必须同源数据目录）。
...
[TaiJi.app 打包版]                        [pnpm run dev]
 ...
 │   isPackaged() ≡ process.env.TAIJI_AGENT_PACKAGED==='1'   utils/runtime-env.ts:12
 ...
 │     入站：process.env × 白名单 —— 白名单含裸前缀 'TAIJI_' ⇒ 所有 TAIJI_* 放行 ★真实泄道★
...
**根因定谳（修正任务原始假设）**：泄漏不是「pi 子进程默认全量继承」。rpc-client 存在 deny-by-default 的白名单过滤，真正的泄道是**白名单里的裸前缀 `'TAIJI_'`（`constants.ts:74`）把 runtime env 中全部 `TAIJI_*` 当作合法透传物整段放行**——包括只在 runtime 自身有意义的生命周期标志。治理对象因此不是「补全过滤」，而是「给出站面立契约」。
...
**审计方法（可复核）**：`rg -o 'TAIJI_[A-Z0-9_]+' -g '!node_modules' -g '!dist'`（branch `fix-add-session-to-project`，2026-08-27）得 **41 个候选名**；逐一回查生产代码赋值/读取锚点，剔除文本误报后得 **28 个真实 env 变量**（A-E 组）+ **13 个非 env 归类**（F 组）。`TAIJI_` 前缀：**0 命中**（仅 CSS token 命名）。以下行号均为本次会话 grep/read 复核值。

#### A 组：main ↔ runtime 启动契约（核心争议面）

| 变量 | 设置点 | 穿越 | 消费点 | 泄露危害判定 |
|------|--------|------|--------|---------------|
| `TAIJI_AGENT_PACKAGED` | main 注入（`process-control.ts:260`；dev 下 undefined 清残留 `safe-env.ts:36-40`） | B2 合规；B3/B4/B5/B6/B7/B8 无合规依据 | runtime 六处（§2.2 图）；**生产代码消费者全部位于 runtime 进程自身**，pi 子树零消费（grep 证实） | **有害**（本案元凶，探针 P1/P2 实锤）。出站应为零 |
| `TAIJI_AGENT_DATA_DIR` | 用户 shell；dev 缺省 `~/.taiji-dev`（`main.ts:123`） | B2 extras(:262)；B3 **显式注入**（`process-manager.ts:124`，与白名单双保险） | runtime `shared getDataDir()`（`shared/src/paths.ts:40-42`）；pi 内 extensions（`subagent-workflow/src/execution/engine/common/data-dir.ts:45`） | **合法跨界功能契约**，应显式 forward，不应靠前缀侥幸 |
| `TAIJI_AGENT_PORT_OFFSET` | 用户 shell；dev 缺省（`main.ts:125`） | B2 extras(:263)；随白名单漏入 B3 后 | main/runtime 端口计算；pi 子树零消费 | 无害但违反最小暴露；观察名单（D3） |
| `TAIJI_RUNTIME_TOKEN` | main 注入（`:266`，WS 鉴权令牌） | B2 extras；白名单漏入 B3/B4…（P2 活体取证） | renderer 经 IPC 读 supervisor；pi 子树零消费 | **有害（安全级）**：任意 hook/npm 脚本可读 WS 凭证。出站应为零 |
...
|------|---------------|-------------------|
| `PI_CODING_AGENT_DIR`（非 TAIJI_ 前缀，产品自有契约） | B3 显式追加（`rpc-client.ts:168`） | 数据隔离根目录约定（`shared/src/paths.ts:50-52` 同源推导）＋extensions fallback 读 |
| `TAIJI_AGENT_DEBUG` | B3 白名单放行 | extension-logger、subagent-workflow 引擎（extensions 内 4 处生产读取） |
| `TAIJI_GLOBAL_AGENTS_DIR` | B3 白名单放行 | `extensions/taiji/system-prompt/src/index.ts:91`（全局 agents 目录 override） |
| `TAIJI_SUBAGENT_RELAY_SOCKET/_NODE/_SCRIPT` | B3 经 `getRelaySpawnEnv()` **显式注入**（`process-manager.ts:123`；常量定义 `subagent-workflow/src/execution/relay-env.ts:13-15`，构建 `runtime/infra/relay/relay-env.ts:107-126`，「全有或全无」降级 :110-112） | 代理链路三基础设施；嵌套 spawn 时五键被剥防旧值误导（`relay-registry.ts:177-178`） |
| `TAIJI_ZCODE_CLI` | B3 白名单放行 | `packages/zcode-subagent-cli/src/registration.ts:40`（zcode CLI 路径 override，引擎孵化外部 CLI；[W11 终态] core 内建 engines/zcode 已删，core 侧等价通道 = d8-compat `createZcodeEngine` 的 `deps.cliPath`） |

#### C 组：plugin 子进程域

| 变量 | 设置点 | 消费点 |
|------|--------|--------|
| `TAIJI_PLUGIN_SANDBOX_DIR` | `plugin-host-process.ts:394`（fork 时注入） | sandbox bootstrap 插件定位 |

#### D 组：dev / mock / e2e / 测试专用

| 变量 | 锚点 |
|------|------|
| `TAIJI_MOCK` | `apps/electron/package.json:15`（dev:mock 设置 → vite WS mock） |
| `TAIJI_E2E` | `window-factory.ts:68` |
| `TAIJI_DEVTOOLS` | `window-factory.ts:78` |
| `TAIJI_DEV_ELECTRON_VERBOSE` | `apps/electron/scripts/dev-electron.mjs:160` |
| `TAIJI_DEV_BUNDLE_ICON` | `main.ts:239` |
| `TAIJI_DEV_MOCK_UPDATE` | `apps/electron/main/dev/mock-release-checker.ts` |
| `TAIJI_SKIP_REAL_PI` / `TAIJI_TEST_SIGTERM_MARKER` / `TAIJI_TEST_ENV_DUMP` | 测试基建（仅测试/脚本引用） |
| `TAIJI_SETTINGS_CLI` | `runtime/src/cli/resolver.ts:19`（settings 路径 override，测试/CI） |

#### E 组：runtime 自身行为开关（单点消费为主）

| 变量 | 锚点 |
|------|------|
| `TAIJI_LOG_LEVEL` / `TAIJI_LOG_MAX_BYTES` / `TAIJI_LOG_KEEP_DAYS` | `infra/logger.ts:119` 起 |
| `TAIJI_DEBUG_PI_EVENTS` | `infra/pi/event-adapter.ts:1102` |
| `TAIJI_EXTENSION_PATHS` | `extension-service.ts:169` + pi 内 `resource-discovery.ts:502`（**双端读取**：跨 B3 两侧都有消费者） |
| `TAIJI_AGENT_API_KEY` | `cli/commands.ts:143,:193`（CLI 域凭证透传） |

> E 组多数不必进 pi 子树，但部分存在双端消费且无实证危害——首轮不做全量甄别，统一进观察名单（D3）。

#### F 组：候选名剔除明细（13 项，佐证审计严谨性）

`TAIJI_GUI_WIDGET` / `TAIJI_ASK_USER` / `TAIJI_SESSION_MANAGER` / `TAIJI_AGENT_SUBAGENT`：流内协议 marker（`extension-protocol/src/core/markers.ts` 等，`\x00` 前缀 stdout 协议），非 env；`TAIJI_AGENT_VERSION`：tsup 构建期 define（`packages/runtime/tsup.config.ts:53`）；`TAIJI_DATA_DIR_ENV`：常量名持有者（值即 `'TAIJI_AGENT_DATA_DIR'`）；`TAIJI_SUBAGENT_RELAY_`：动态拼接前缀痕迹；`TAIJI_STREAMING_TIMEOUT_MS`：仅注释提及（`packages/core/src/domain/chat/store.ts:212`）；`TAIJI_BUNDLED_PI_BIN` / `TAIJI_DEPS` / `TAIJI_AGENT` 碎片：无生产消费命中（测试/文档残留）。

### 2.5 审计方法与命中数汇总

```bash
# ① 候选名枚举（去重）
rg -o 'TAIJI_[A-Z0-9_]+' -g '!node_modules' -g '!dist' --no-filename . | sort | uniq -c | sort -rn
...
数字：候选名 41 → 真实 env 28（A-E 组明细可数）+ 非 env 13（F 组可数）；进程边界 8 类；runtime 自身 `isPackaged()` 消费点 6 处；**实证受害变量 2 个**：`TAIJI_AGENT_PACKAGED`、`TAIJI_RUNTIME_TOKEN`。

### 2.6 反向边界现状复述与漏网检查（G4）

`safe-env.ts` 已具备「undefined = 显式清除」语义（:36-40，注释点名 `TAIJI_AGENT_PACKAGED` 残留场景）——开发者 shell 导出的陈旧标志不会穿透 B2 进 runtime。**漏网评估**：白名单同样含裸 `'TAIJI_'`，shell 里任意 `TAIJI_FOO=bar` 会经 B1（无过滤）→ B2（放行）→ B3（放行）抵达 pi。危害低于正向（GUI 打包态接触不到用户 shell），且属「来路变量无法甄别意图」的固有问题；本轮不动（守住 G4），「入站白名单管准入、不管来路合法性」写进出站契约文件头注释（§5-U1）。
...
**成功路径**：用户在打包版 TaiJi.app 的 agent 会话里说「把这个 bug 修了提交」→ agent 执行 `git commit` → pre-commit 全套检查照常执行 → validate-runtime-bundle 健康检查启动的隔离 runtime 拿到干净 env → `findPackagedPi` 不被错误触发 → 全绿，commit 完成。全程无需知道 `env -u` 的存在。

**契约验证路径**（随时自查）：会话 bash 里跑 `printenv | grep '^TAIJI_'`：
...
- **R1 只操作传给子进程的 env 副本，绝不修改 `process.env` 本体。**
  ...
  delete process.env.TAIJI_AGENT_PACKAGED
  ```
...
- **步骤**：① 构建并安装修复后的打包版 TaiJi.app；② 在 app 会话中对 taiji 任一 worktree 创建临时分支、做一次微改动并 `git commit`（触发全套 pre-commit 含 validate-runtime-bundle）；③ 观察 hook 输出与退出码。
...
- **步骤**：会话 bash 工具内执行 `printenv | sort | grep -E '^(TAIJI_|PI_CODING|ELECTRON_)'`；打包版与 dev（`pnpm run dev`）各采样一次，与 §3.3 期望表逐行比对。
- **通过标准**：两种模式下 `TAIJI_AGENT_PACKAGED` 与 `TAIJI_RUNTIME_TOKEN` 均不出现（diff 计数 0）；`TAIJI_AGENT_DATA_DIR`、`PI_CODING_AGENT_DIR` 始终出现；打包模式额外出现的条目不多于 §3.3 表所列。
...
- **步骤**：打包版打开内置 terminal → `printenv | grep -E '^(TAIJI_|ELECTRON_)' | sort` 与 `echo $TERM`。
- **通过标准**：输出不含 `TAIJI_AGENT_PACKAGED`、`TAIJI_RUNTIME_TOKEN`；`ELECTRON_RUN_AS_NODE` 缺席（现状保持）；TERM 为 xterm-256color 或继承值（渲染不损）。
...
| # | 操作 | 结果 | 结论 |
|---|------|------|------|
| P1 | `TAIJI_AGENT_PACKAGED=1 bash scripts/validate-runtime-bundle.sh` | exit 1；[6/6] `[runtime] fatal: relay server init failed: Error: Bundled pi binary not found at <root>/pi/pi-darwin-arm64`，栈顶 findPackagedPi ← RelayRegistry 构造器 | 受害链实证：污染标志使 runtime boot 即按打包态解析捆绑二进制 |
| P2 | 会话 bash 环境 `printenv \| grep -E '^TAIJI_\|^PI_CODING\|^ELECTRON_'` | 命中 4 个：`TAIJI_AGENT_PACKAGED=1`、`PI_CODING_AGENT=true`（pi 自设）、`PI_CODING_AGENT_DIR=~/.taiji/pi/agent`、`TAIJI_RUNTIME_TOKEN=<hex，此处脱敏>` | 泄漏活体取证：令牌级变量直达任意后代进程；亦证明 validate-runtime-bundle 从 agent 会话内天然被污染 |
| P3 | `env -u TAIJI_AGENT_PACKAGED -u TAIJI_RUNTIME_TOKEN bash scripts/validate-runtime-bundle.sh`（真基线对照） | exit 0；七步全 OK | 控制变量成立：唯一差异变量即缺陷元凶 |
...
- 入站白名单 SSOT（含 `'TAIJI_'`）：`packages/shared/src/constants.ts:72-81`
