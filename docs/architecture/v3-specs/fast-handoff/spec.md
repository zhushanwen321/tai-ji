...
**生成**：当前 session 的 pi 进程跑 `/skill:handoff`（提示词 = `~/.agents/skills/handoff/SKILL.md`），agent 产出自由格式 markdown 文档字符串。**不需要 taiji-handoff-extension.js、不需要 setActiveTools 三件套、不需要 structured-output schema 校验。**
...
## 4. Extension 设计（taiji-handoff-extension.js）
...
- `taiji-handoff-extension.js`（项目根）：registerCommand('handoff') + 三件套（§4.2）
...
- `electron-builder.yml`：extraResources 加第三条 `taiji-handoff-extension.js`（`extension-service.ts:98-99,122-123,261-265` 三处对称加 `handoffExtensionFilePath` 字段 + 初始化 + push）
