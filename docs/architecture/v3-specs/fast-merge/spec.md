...
| **C: before_agent_start** | `pi.on("before_agent_start", ...)`（`runner.ts:1054-1057`） | 往 system prompt 追加强约束："本轮唯一任务是调 structured-output，schema=XXX，禁止文本输出"。taiji 已有 `taiji-system-prompt-extension.js` 走这个 hook |
...
## 4. Extension 设计（taiji-merge-extension.js）
...
```javascript
// taiji-merge-extension.js（跑在主线 pi 进程内）
...
4. **`electron-builder.yml:65-70`**：extraResources 加第三条 `- from: ../../taiji-merge-extension.js / to: taiji-merge-extension.js`
...
| 分支选择器 | Dialog/Popover，复用 @taiji/ui，列出子分支（标题 + 时间 + 状态点）+ checkbox |
...
- `taiji-merge-extension.js`（项目根）：registerCommand('merge-branches') + setActiveTools + **context event** + turn_end 三件套（§3 技术裁决：context event 非 before_agent_start）
...
- taiji-system-prompt-extension.js 的 before_agent_start hook 先例
...
- `electron-builder.yml`：extraResources 加 `taiji-merge-extension.js`
