...
读者假设：会用 taiji（派过 subagent、看过侧边栏 subagent 列表），但不懂内部实现。
...
- **对话记录（transcript）**：subagent 与模型的完整对话存在一个独立的 JSONL 文件里（pi 引擎形态），由子代理进程自己写。「续聊」的本质 = 让一个（新的或还活着的）子代理进程**接着写同一个文件**，历史由 pi 自己从文件加载，taiji 侧不解析内容、只传文件路径。
...
**不变量**（沿用 H4 已确立的，全部保留）：
- pi / zcode 是 transcript 的唯一写者，taiji 侧只传指针（两侧现状一致，runtime session-store.ts:9-10 与 spawn-args.ts:53-55）；
