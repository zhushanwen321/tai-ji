...
| 14 | B3：6 工具 description 未声明 taiji runtime 依赖，独立 pi 用户误调用白烧 30-60s（F4/§6.5 B3） | index.ts:182,:191,:200,:209,:218,:227 六个 description 均无依赖声明 ✓；README.md:22-24「运行要求」节自认「独立 pi CLI 环境无 handler 时工具将等待至超时（create/history 60s、其余 30s）」✓；SELECT_TIMEOUT_MS 分档 :44-51 ✓ | **属实（未实施）** |
