...
- **重审记录（2026-09-16）**：重审已执行（触发 = 首发 0.10.1 时守卫 warning，files 求和 9.07MB），结论 = 不拆双包，subagent-core `files` 移除 `src/`。依据：D4 双形态语义本身即「TS 源供 workspace、dist 供 npm」，`src/` 进 files 是白名单落地时的偏离（先例 extension-protocol / session-delivery files 只发 dist）；npm 消费面经 publishConfig 整体替换后全指 dist，`src/`（5.7MB，其中测试 3.2MB）在 tarball 零消费。收敛后 tarball 解包 4.3MB，warning 消除。已知残留：顶层 `main`（`src/index.ts`，workspace 形态载体）被 npm files 规则强制隐含打包（约 34KB，无开关）；publishConfig 替换后 exports 严格限死入口，该文件对 npm 消费者不可达，纯体积噪声。
...
| 2026-09-16 | v8 | D7 重审执行（触发：subagent-core 0.10.1 首发，守卫体积 warning files 求和 9.07MB 超 5MB）：`files` 移除 `src/`——D4 双形态语义对齐（npm 消费面经 publishConfig 全指 dist，TS 源仅服务 workspace；src 5.7MB 中 3.2MB 为测试，tarball 零消费），tarball 收敛至解包 4.3MB、warning 消除；登记 npm main 隐含打包残留（src/index.ts 约 34KB，exports 面不可达，纯噪声）；守卫复跑全绿 |
| 2026-09-16 | v9 | D7 重审裁决同型适用：subagent-engine-sdk `files` 同步移除 `src/`（含 `!src/__tests__` 排除项一并失效）——该包与 subagent-core 同型双形态（$comment 明写「双形态同 subagent-core 先例 D4」），src 进 files 是同一处偏离的两处落点；量级差异仅 252KB（低于 5MB 阈值，守卫从不 warning 故无人审视——偏离靠一致性巡检暴露而非体积报警）。tarball 收敛至解包 1.2MB；dist 无 sourcemap/URL 引用，移除零断链；npm main 隐含打包残留同 subagent-core（src/index.ts 3.2KB）。至此 7 个 dist 发布包 files 口径统一（extension-protocol / session-delivery / subagent-core / subagent-engine-sdk / pi-rpc / pi-subagent-cli / zcode-subagent-cli = 只发 dist，bin/workflows/agents 资产条目除外）|
