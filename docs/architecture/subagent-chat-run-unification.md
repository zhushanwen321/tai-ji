...
> **实施状态（2026-09-13）**：v7 已实施交付（chat 域统一进 run 域全单元落地；实施后语义演进见下条注记）。
> **演进注记（2026-09-13）**：本文 H1 语义面已被 [subagent-permanent-session-model.md](subagent-permanent-session-model.md)（永久会话模型，同日实施完毕）**partially superseded**——① §3.1 失败路径表与 §3.3 D4 的「closed（user-close/cancelled）→ endedMessageGuard 硬拒」已消亡：现行 reviveOrThrow 万物可续（conversation-continuation.ts），endedMessageGuard 已 [U4] 缩型为异进程占用单形态（subagent-actions-core.ts 段注释自证）；② D4 close 行「abort 在途+清队+立即终态化」已改 U5 优雅收口（closeAfterRound 挂起→通知送达后收口落账）；③ D7 通知门 `notifyGateAllowsDelivery(record.closedReason)` + `NOTIFY_BLOCKED_CLOSED_REASONS` 集合已被 U5 通知 gate 取代（同名函数判据源换放弃轮标记；原 intent 判据分支已随 2026-09-16 裁决删除，notifier.ts）；④ 通知 dedup key `record:round` 已扩为 `id:epoch:round`（epoch=0 保持旧格式）；⑤ U6 保留清单 consumeCloseAfterRound 已更名 consumePendingArchive（run-orchestration.ts）；⑥ S6/S7 验收场景的 user-close 硬拒指引不再可构造（任何 closed 形态 message 均可续聊）。偏差全表见 `.tmp/dev-flow/subagent-permanent-session-model.impl-plan.md` §5。
>
...
- **S（情境）**：taiji 的 subagent 子系统有两种执行域。**run 域**：一次性任务——派发 → spawn 引擎子进程 → 事件流 → 终态 → 通知 → 回收，全链路一套机制。**chat 域**：GUI 续聊型 subagent（`conversation: true` 派发，record 带 `chatMode` 标记）——引擎侧会话跨轮长驻，续聊经 `interact` 控制面投递，为维护「长驻」存在一整套独立状态机（§2.2）。
...
- **conversation 能力位的消费方不止 interact**（影响面审 F1）：①fork 门控 **OR 借位**——`capability-gate.ts:91-99` 在 `steer === "unsupported" && conversation === "unsupported"` 才拒 fork，pi 声明 `steer:"unsupported"` + `conversation:"native"`，**pi 的 fork 放行完全依赖 conversation 位**；②message 资格门——unsupported 引擎 message 续聊拒（modeless 波5 起 start 即拒已放宽为放行，capability-gate.ts [modeless 波5 收尾] 注）；③manifest/握手 mismatch 判定；④引擎发现与 zcode stub 快照逐位一致要求。
...
> **[演进注记 2026-09-13]** 上表「record 已 closed（user-close/cancelled）→ endedMessageGuard 硬拒」行与「用户 close → 立即终态化」行已随永久会话模型取代（万物可续 + close 优雅收口），见头部演进注记①②。
...
- **D7 轮末处置分流（承载 MF-6 / round 计数 / settle 段交棒）**：chatMode record 的 run 轮末**不走 run 域终态化短路**（否则容器被首轮吞掉），按 outcome 分流（成功与失败**都**到达轮终处理）：
  ...
  > **[演进注记 2026-09-13，2026-09-16 裁决修订]** 本条 notifyGateAllowsDelivery(closedReason) 门与 NOTIFY_BLOCKED_CLOSED_REASONS 集合已随 U5 消亡（gate 判据 = 放弃轮标记；原三元组中的 intent 分支已随 2026-09-16 裁决删除，notifier.ts——见头部演进注记③）；dedup key 已扩为 `id:epoch:round`（epoch=0 保持 `id:round` 旧格式，注记④）；closeAfterRound 消费已更名 consumePendingArchive（run-orchestration.ts，注记⑤）。
...
| `extensions/universal/subagent-workflow/src/__tests__/first-round-closure-service.test.ts` | roundLifecycle idle 相位注释 ×1 | 改写（U6 批，依赖 U2 轮末分流） | 首轮收口测试改 run 应答回调形态 |
...
**文件改动地图**：SDK `protocol/{methods,contract-types,schema,reverse-channels,port-contract,engine-protocol}.ts`；core `execution/subagent-service.ts` + `execution/assembly/subagent-actions-core.ts` + 新增 `execution/assembly/conversation-continuation.ts` + 删 `execution/cold-resurrect.ts` + `execution/persistence/finalize-record.ts` + `execution/notify/notifier.ts`（轮末分流/round 写点）+ `execution/persistence/execution-record.ts`（base 两函数与 `roundBaseTurnIndex` 字段清理，U6）+ `execution/assembly/types.ts`（字段声明清理）+ `execution/engine/{port.ts,client/{remote-engine,reverse-router,engine-client}.ts,host/host-bridge.ts,common/capability-gate.ts}`（capability-gate 注释更新 + SP-5 升级 gate 错误构造单源 engineConversationUpgradeUnsupportedError——conversation 位保留）+ `execution/lifecycle/settled-watchdog.ts`（刷新源）；pi `pi-engine.ts`/`server.ts`/`spawn-runner.ts` + 删 `chat-session.ts`；zcode `server.ts`/`zcode-engine.ts`；extensions `subagent-workflow/src/__tests__/{first-round-closure-service,one-shot-upgrade}.test.ts`（改写）+ interface 面。
