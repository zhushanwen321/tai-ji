...
**In-scope**：`extensions/universal/pending-notifications/`（src + tests）、`packages/subagent-core/src/execution/engine/__tests__/conformance/registry-fork-filter.test.ts`（直穿 import 本包 src 的连带改写）、`docs/constraints.json` C-proc-13 ③ 措辞（+ 改后跑 `node scripts/validate-constraints.mjs` 结构校验；原 `render-constraints.mjs` 已于 2026-09-13 删除，constraints.md 视图不复存在，无需重新生成）、bte 包内一处注释行修正（`base-tool-enhance/src/background/pending-reconcile.ts:115`，E8）、docs/design 悬空引用/行为断言失实回写 4 文件（E9）、`scripts/check-doc-symbol-drift.mjs` DOC_MODULE_MAP 映射登记（E10）、`docs/design/ext-simplify-index.md` 13 号行跨设计协调注记（§6.4⑤ 批级锚点，设计交付时落地）。
**Out-of-scope**：
...
| E7 | `docs/constraints.json` C-proc-13 ③ | 读侧消费枚举「countActiveFromEntries/subagent-workflow 后代判定/registry rebuild+工具投影/goal 守卫口径」→「…/pending_notifications 工具投影（entries 现算）/goal 守卫口径」；「禁回翻 session 档」机理表述保留；改后跑 `node scripts/validate-constraints.mjs` 结构校验（原指令「跑 render-constraints.mjs 重新生成 md」的脚本已删除，md 视图不再生成）；顺带核实 `extension-dependencies.json` 对本包的 reason 描述（「pending-notifications 消费该事件并向用户发送完成通知」与现状不符——完成通知已由 subagent-workflow bg-notify-render 承担），失实则同批修正 | 审计修正 #3，C-proc-10 直接执行 |
...
| M2 连带面（E6-E10） | conformance 测试改写 + constraints.json 回写校验（E6+E7，单 commit）；bte 注释行修正 + docs/design 悬空引用/行为断言失实回写 + 守卫映射登记（E8-E10，单 commit——E9 回写口径以 M1 删除后代码为基准，须排在 M1 之后） | 跨包测试绿 + SSOT/文档面/守卫能力一致；V2/V5 验证 |
