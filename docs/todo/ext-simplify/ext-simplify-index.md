...
| 04 | ext-simplify-04-session-reader.md | session-reader | C5 find 三次全量扫盘；low 群：doctor 缓存机（+设计 §6.3 回写）、测试 re-export 块×4 域模块、RESULT_ACTION_DEPS、err 双轨、fullEntry、formatLine/formatOutlineText 双份、family enrichRefs（contested→裁决）、readTailIdentity 双写、SessionRoot.id、byteBudget、OutlineOptions.budget、content 提取 5 变体 | session-view-01a08ff3-3bba / 3ba8 / 3b9a / 3b81 | 设计就绪（v2.1 20260914：MF1 G5 扩为 C-proc-10 回写清单落到执行项验证列 + S1-S5 全修；聚焦复审 PASS + 5 条一句级清偿，2aa048e7f/c5e56a7de） |
| 05 | ext-simplify-05-permission.md | permission | C6 注入仪式（setDefaultListAvailableModels）；C7 rpcDeps 注入面；M6 审批卡双份；M7 CommandDeps×2；M8 cancel() 孤儿；M9 ResolvedModelEntry 三字段；M10 pattern 双写；M11 ui 适配闭包×3；low：rules barrel、pipeline export 面、winner 循环×3、toolName 守卫、SelectItem、rerender() | session-view-01a0904a-4e93 / 4e7e / 4e69 / 4e4b / 01a08ff3-3bae | 设计就绪（v3.1 20260914：MF1 审批卡内核重设计为分组结构化字段集 {base, ai?} + R2 供给方定性（preClassification 生产零供给方，reasoning 为渲染函数级漂移、用户可见零变化）+ R3 文本清扫；复审 3 轮闭环，bbeb63a0b/dd7628a80/3af9f5436） |
| 06 | ext-simplify-06-plan.md | plan | C8 isolation×execMode 矩阵（正确性）；M1 三源模板机制；M2 extractPlanSteps 脱节；M3 动态 import×3；+peer 依赖 optional 化（审计发现 5） | session-view-01a09070-77dd-*.md | 设计就绪（v2.1 20260914：MF1 D5 收缩——logger 剥离子项基于 crash-resilience 前过期快照撤销、包级依赖保留 + 全基线刷新（1096 行/7 测试文件/0.4.4）+ 复审清单 17 处行号锚点二次刷新；4edc42e12） |
...
| 设计就绪宣告 + commit | 01/02/03/09/12/14 + **07/15/13**（20260914，B 组全链路交付：设计闭环 → dev-flow 七阶段 → 终态同步）+ **16**（20260914，dev-flow 全七阶段交付）**十份实施完成**；04/05/06/08 设计就绪（20260914），实施待用户指示 |
