...
| `e2e/skill-reload-askuser.spec.ts` | S1b | 双 session 并发 reload 存活：全局 skill 目录变动 → dir=global 归因行列双 sid / preserved 行每 session 一条 / reload 后触发的 ask_user 反向请求送达 form-overlay 且可应答（D3 槽现读，无静默取消）；run 完成（主 session JSONL 终态 done 落盘）后 ≤30s `session.workflowUpdate` done 帧到达 spec WS（G1 必达窗口，A1 断言升级 2026-09-19）；dialog 阶段断言顺序解耦——select 后先 answerOverlay 再断言 composer（FormOverlay 与 composer 互斥，u0 复跑运行 2 的 30s 扑空假失败形态修复） | ⚠️ 已知失败态：reload 存活面全过（含 form-overlay 渲染/应答/关闭 + pi 恢复 turn）；终态收口断言受残留风险 #8（run 收口面 stale 引用，未修）影响暂红——2026-09-19 两次稳定复现并归因：run 的 done 处理在 stale ctx emit 失败 → done 广播未送达 spec WS → awaitDone 150s 超时（reload preserved 收口链既有问题，非表单协议迁移回归）；修复后复跑 |
| `e2e/skill-reload-spawn-race.spec.ts` | S2 | 派发 run 后 <1s 编辑 skill 撞 spawn 窗口竞态：run 确定收口（workflowUpdate 广播）+ 主 session JSONL 末条 workflow-record 可读回（无状态分裂）+ 引擎执行树 ps 归零（无孤儿）；run 完成后 ≤30s `session.workflowUpdate` done 帧到达 spec WS（G1 必达窗口，A1 断言升级 2026-09-19） | ⚠️ B5b 已真机执行：竞态命中 + preserved 过；终态收口断言受残留风险 #8 影响暂红，修复后复跑 |
| `e2e/workflow-disconnect-recovery.spec.ts` | A2 | WS 断开期间 run 完成 → 重连后收敛（传输兜底面回归，水位结构性不触发场景）：恢复来源双断言 = `session.subscribe` reply 的 stateSnapshot 携带该 run 的 done workflowUpdate last-value 回放帧 + `session.getWorkflows` 冷拉 RPC 返回终态记录；GUI 收敛（托盘 workflow 条目 data-state 回 idle ≤30s）；重连稳态无重复 done live 帧（断连空投下 publish 已完成、重连后 diff 恒空） | ⏳ 待执行（2026-09-19 reload-closeout-reliability u3a 编写，执行归 u3b——依赖 u1 送达水位对账落地） |
