...
**关键设计**：replace（启动期 CLI 注入）与 append（运行期 hook 注入）走两条独立链路；配置存独立文件 `<dataDir>/system-prompt.json`（configDir 与 dataDir 同根，dev=`~/.taiji-dev/`、prod=`~/.taiji/`），插件每轮直读，故追加保存后立即生效。

**[HISTORICAL]** 旧根文件版 `taiji-system-prompt-extension.js`（repo root 文件型 builtin）已于 2026-08 builtin→npm 迁移删除；其「当前生效提示词快照」卡（写 `<dataDir>/system-prompt-snapshot.md` 供 UI 回读）一并移除，现行无实时快照入口，参考区只展示默认提示词常量。
...
| 测试文件 | 覆盖用例 |
|---------|---------|
| [`system-prompt-extension.test.ts`](../../packages/runtime/test/system-prompt-extension.test.ts) | append 开启且非空 → BASE+全局段+\n\n+EXTRA / append 关闭 → undefined / 配置缺失 → undefined / JSON 损坏 → undefined / append.prompt 纯空白 → undefined / 全局指令注入（存在→带头部、空白→跳过、候选顺延） / --no-context-files 在 argv → 全局不注入（append 仍生效） / TAIJI_AGENT_DATA_DIR 与 PI_CODING_AGENT_DIR 回退定位 |
...
| 步骤 | 操作 | 期望 |
|------|------|------|
| 1 | Settings → 系统提示词 → 开替换卡开关 + 填文本 → 保存 | toast 提示成功；`~/.taiji-dev/system-prompt.json` 写入 |
...
| **L2** | pi 自己写的**产物文件**（session JSONL 的 `model_change`/`thinking_level_change` entry）| 「pi 收到 `--model p/m:level` 后真实拆字段落盘」「零 taiji 代码介入」| LLM 是否跑完产出 | `e2e/workflow-thinkinglevel-real.spec.ts` TC2 |
...
调研 pi 源码（`~/GitApp/pi-ecosystem/pi-mono`）发现三套 real-mode 模式，taiji 目前**未直接采用**，但未来若要做「真 pi 子进程 + 可控 LLM」的测试，这是最干净的参照：
...
**对应 taiji 场景**：若未来要在 extension 层验证「resolveModel 选对 model」而不仅「agent-call 带对 model」，可引 faux provider 模式（但 taiji 经子进程调 pi，faux 需注入 pi 侧，跨进程复杂度高）。
...
`packages/coding-agent/src/modes/rpc/rpc-client.ts`。`spawn("node", [cliPath, "--mode","rpc", ...])` 起真实 pi CLI 子进程，typed API：`setModel/setThinkingLevel/promptAndWait/collectEvents/waitForIdle`。taiji 自己就是 spawn pi + RPC 通信，`RpcClient` 模式可直接复用。

**real-LLM gated 模式**：`describe.skipIf(!process.env.ANTHROPIC_API_KEY)` 门控，无 key 自动 skip。taiji 的 `packages/coding-agent/src/modes/rpc/rpc-mode.ts` 可复用此模式写 real-LLM 测试，断言 `model_change`/`thinking_level_change` JSONL entry（L2）。
...
失败行为：任一步 exit 非 0，打印 `[FAIL]` 行 + `[定位]` runtime stdout / 日志目录路径 + stdout 尾部 30 行；**失败现场保留**（`/tmp/taiji-plugin-e2e.*/`，进程照常清理），成功路径全量清理。

## 3. V6 场景关联

本基线 D 步是 [dev-acceptance.md V6 场景](../../.taiji-harness/2026-08-15-perf/dev-acceptance.md)的自动化版本（fake-session 触发）。V6 补测（2026-08-16）在此之上做了真实 session 全链路手工实测：transform 后内容送达 pi 并持久化（session JSONL user 消息为 `hello [V6-HOOK-APPLIED] end-to-end`，原始 `v6magic` 0 次出现）——该全链路（需模型配置）未自动化。
...
1. 启动 dev app（带 mock）：
   ```bash
   TAIJI_DEV_MOCK_UPDATE=1 pnpm dev
   ```
