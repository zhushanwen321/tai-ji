...
>
> 上游资产：[subagent-dual-track-convergence.md](subagent-dual-track-convergence.md)（双轨收敛，已落地）、[subagent-core-package-extraction.md](subagent-core-package-extraction.md)（core 抽包 + D5 barrel 定稿，已落地）、subagent-engine-abstraction.md（引擎中立抽象，已被引擎协议化取代；已删除，git 可追溯）。
