...
- **采用**：`useTrayCounts` 增加第 4 个 kind（`session`），数据 = renderer session store 中 `parentAgentSessionId === 当前 sessionId` 的条目（该字段 live 从内存透传、reload 从 `.agent.json` 读，两条读取链齐备），**零新协议**。三态沿用既有契约：有进行中 → accent 计数 + 呼吸点；仅历史 → dim；全无 → 不渲染。面板 400px、行 = 7px 状态点（`useSessionDerivations.derivedStatus` 同源）+ label + cwd 末段 + 时长；pin 态行内「打开」（`useSidebar.selectSession`）与「停止」（既有 `session.abort` RPC，两段确认）；行点击 = 打开该子会话。固定序位于 built-in 四件末位（bash → subagent → workflow → session）。
