...
**系统是什么**（给不熟悉 subagent 体系的开发者）：subagent 是 taiji 的子任务执行单元，5 类包协作——shell（`extensions/universal/subagent-workflow`）→ host core（`packages/subagent-core`，record 状态机 SSOT）→ 引擎（`packages/pi-subagent-cli` / `zcode-subagent-cli`）→ 契约（`packages/shared`，跨进程 SubagentRecord）。执行状态的读写链路：
...
**D2：判据 SSOT 落点 = `lib/subagent-bucket.ts`（纯函数模块）**。选择：`isRunningProjection` 改为占用判据 SSOT，`hasRunning`/`isStreamingSubagent`/`isStreaming` 三处改为 import 消费（hasRunning 保留 origin 过滤参数在调用点）。被否：落 store——subagent-bucket 是纯函数、测试矩阵已在此（subagent-bucket.test.ts 断言表），设计期消费方（侧栏任务卡片 / 筛选条 / 侧栏计数段）已在用；此设计期三处消费方随侧栏任务视图退役删除，而 `hasRunning` / `isStreamingSubagent` 两个 wrapper 仍现行——`hasRunning` 由 `packages/renderer/src/composables/features/chat/useBackgroundWork.ts` 消费，`isStreamingSubagent` 由 `packages/renderer/src/components/panel/SubagentTab.vue` / `packages/renderer/src/composables/panel/useSubagentThinking.ts` / `packages/renderer/src/components/panel/tray/TrayNativePanel.vue` 消费——仅 `isStreaming`（随侧栏任务卡片列表组件消亡）不再存在；现行消费方 = 托盘（`useTrayCounts` 计数 + 托盘面板行集）。宽松口径 `isRunning`（SubagentTab 订阅流）**保留原样**——分工注释更新为「翻边后与占用判据天然合流，保留订阅语义」。
...
> 三要素：场景 / 步骤 / 通过标准。每条回溯 §1 目标。真机环境：`TAIJI_DEV_BACKGROUND=1 pnpm dev`（browser-automation 连 CDP 截图验证）。
