...
>
> 上游资产：subagent-engine-abstraction.md（引擎中立抽象，D1-D12，P1-P5 已落地；已被引擎协议化取代；已删除，git 可追溯）、[subagent-core-package-extraction.md](subagent-core-package-extraction.md)（core 抽包 + HostServices，已落地）、subagent-engine-gui-visibility.md（record 通道引擎中立；已删除，git 可追溯，现行权威 = [subagent-engine-protocolization.md](subagent-engine-protocolization.md)）。
