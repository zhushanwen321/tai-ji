...
| 04 | ext-simplify-04-session-reader.md | session-reader | C5 find 三次全量扫盘；low 群：doctor 缓存机（+设计 §6.3 回写）、测试 re-export 块×4 域模块、RESULT_ACTION_DEPS、err 双轨、fullEntry、formatLine/formatOutlineText 双份、family enrichRefs（contested→裁决）、readTailIdentity 双写、SessionRoot.id、byteBudget、OutlineOptions.budget、content 提取 5 变体 | session-view-01a08ff3-3bba / 3ba8 / 3b9a / 3b81 | **设计就绪 20260914（v2-r2）**：over-eng 审查 1 MF + 5 sug 已修（G5 扩 5 笔回写清单）；双审查 R1 主审 1 MF（G5① impl-plan 侧落点）修复后 R2 双 PASS 至 0 |
| 05 | ext-simplify-05-permission.md | permission | C6 注入仪式（setDefaultListAvailableModels）；C7 rpcDeps 注入面；M6 审批卡双份；M7 CommandDeps×2；M8 cancel() 孤儿；M9 ResolvedModelEntry 三字段；M10 pattern 双写；M11 ui 适配闭包×3；low：rules barrel、pipeline export 面、winner 循环×3、toolName 守卫、SelectItem、rerender() | session-view-01a0904a-4e93 / 4e7e / 4e69 / 4e4b / 01a08ff3-3bae | **设计就绪 20260914（v2-r1）**：over-eng 审查 1 MF + 3 sug 已修（M6 内核改 ApprovalFieldSet 结构化字段集，v1 string[] 公式记入被否谱系）；双审查 R1 双 PASS（D5 新公式四条断言验证可兼得） |
| 06 | ext-simplify-06-plan.md | plan | C8 isolation×execMode 矩阵（正确性）；M1 三源模板机制；M2 extractPlanSteps 脱节；M3 动态 import×3；+peer 依赖 optional 化（审计发现 5） | session-view-01a09070-77dd-*.md | **设计就绪 20260914（v2-r1）**：over-eng 审查 1 MF + 4 sug 已修（D5 收缩——extension-logger 依赖保留、compact.ts 行号刷新）；双审查 R1 双 PASS；实施前置 u0 探针门（P1/P2 未实跑） |
...
| 08 | ext-simplify-08-scheduler.md | scheduler | M18 croner optional peer 误报；M19 queuedInDeliveryAt 补偿机制（contested→设计裁决：登记跨包债务 vs 根修 session-delivery settled 回调）；low：errorCode、delivery handle 中转、void ctx | session-view-01a09053-242e-*.md | **设计就绪 20260914（v2-r2）**：over-eng 审查 1 MF + 4 sug 已修（TTL 兜底两类触发源）；双审查 R1 主审 2 MF（慢投递行为表征失实：副本数 ⌈超窗/10min⌉ + nextRunAt 绝对锚定不跳周期；排查通道 warn 不可操作：改时间线主判 + 排除法）修复后 R2 双 PASS 至 0 |
...
| 10 | ext-simplify-10-structured-output.md | structured-output | M24 assertSafeTimerDelay+锚点漂移；low：RetryState.reset、Symbol.for slot（contested→裁决：C-ext-06 惯例 vs 回退）、env 重读、出口面测试专用导出 | session-view-01a09070-7809-*.md | **设计就绪 20260914（v2-r1）**：over-eng 审查 PASS（0 MF / 5 sug 已修）；双审查 R1 双 PASS（194 用例基线实跑复核吻合） |
| 11 | ext-simplify-11-ask-user.md | ask-user | M23 Other label 校验缺失（正确性）；low：AnswerValueSchema/ResultSchema、TUI 样板双份、channel registry 外沿（resolve/list 死成员 + gui_widget 空置登记 doc-right） | session-view-01a09053-2422-*.md | **设计就绪 20260914（v2-r1）**：over-eng 审查 PASS（0 MF / 2 sug 已修，trim 口径）；双审查主审 R1 PASS + 影响面 R2 PASS（补 runRpcForward 穷举 + 版本滚动缺口后确认） |
...
| 设计就绪宣告 + commit | 01/02/03/09/12/14 六份已实施完成（u 单元全部 committed）；**A 组 6 份设计就绪 20260914、待实施（06 前置 u0 探针门）**；07/13/15 待修复后进双审查；16 待起草 |
