...
> **⚠️ 状态：搁置（2026-09-14 用户裁决，登记 `docs/todo/tdflow-two-phase-workflow.md` §五）**——本设计的唯一消费方是 plan 的 complete goal 档，plan 包存废待 tdflow 两阶段工作流重构落地时一并裁决：plan 保留则按本设计实施（u1-u4）；plan 退役则本设计作废，仅保留 D6 两项登记（pi 官方机制观察项 + `__workflowRun` 死通道加注，后者可随任一批次独立实施）。设计本身已就绪（三审 0 must-fix），重启无需返工。
...
- v2.4（2026-09-14）：**搁置登记（用户裁决）**——plan 消费方存废待 tdflow 两阶段工作流重构（`docs/todo/tdflow-two-phase-workflow.md`）落地时一并裁决，本设计停止实施排期。设计结论不变（v2.3 三审 0 must-fix），重启条件与独立价值资产（D6 两项登记）见头部注记。plan 包冻结（不再投入），`pi.__workflowRun` 死通道加注（pi-ext-020）可随任一批次独立实施。
