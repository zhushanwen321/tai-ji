...
> 本文档面向需要为 taiji 创建**内置插件**（built-in plugin）的开发者。内容覆盖从零搭建到最佳实践的全流程。
>
...
内置插件是随 taiji 应用一起分发的插件，位于 `resources/plugins/` 目录下。由 `PluginService` 统一管理生命周期：trusted 插件共享 **Worker Thread**（≤10 插件/Worker），外部 sandbox 插件独占 **fork 子进程**（`plugin-host-process.ts`）。

### 1.2 内置插件 vs 外部插件

| 维度 | 内置插件 | 外部插件 |
|------|---------|---------|
| 位置 | `resources/plugins/<name>/` | `~/.taiji/plugins/<name>/` |
| 信任等级 | `trusted`（默认） | `sandbox`（默认） |
| Worker 策略 | 共享 trusted Worker | 独占 sandbox fork 子进程 |
...
插件由 runtime 进程内的 PluginService 管理执行：**trusted（内置）插件共享 Worker Thread**，**外部 sandbox 插件独占 fork 子进程**（`plugin-host-process.ts`：「sandbox 插件独占子进程；trusted 插件共享 ≤10 插件/进程」）：

```
┌─ Runtime 进程 ─────────────────────────────┐
│  PluginService                             │
│    ├── Trusted Worker Thread（共享）        │
│    │     ├── statusline (内置)              │
│    │     ├── todo (内置)                    │
│    │     └── ... (其他内置插件)              │
│    └── Sandbox fork 子进程（外部插件独占）   │
└────────────────────────────────────────────┘
```

- **隔离**：每个 Worker/fork 有独立的 `globalThis`，同名变量不冲突
- **通信**：trusted 经 JSON-RPC over `MessagePort` 与 runtime 主线程通信；sandbox fork 经 stdio RPC
...
| `description` | ✅ | 一句话功能描述 |
| `taijiPlugin.manifestVersion` | ✅ | 当前必须为 `1` |
| `taijiPlugin.main` | ✅ | 入口文件路径（编译后的 `.js`，相对于 package.json） |
| `taijiPlugin.activationEvents` | ✅ | 激活事件数组，见下表 |
| `taijiPlugin.trustLevel` | 推荐 | 内置插件填 `"trusted"` |
| `taijiPlugin.source` | 推荐 | 内置插件填 `"built-in"` |
| `taijiPlugin.permissions` | 视情况 | 需要的权限列表。内置 trusted 插件可省略（默认全授权） |
...
Statusline 插件监听 pi extension 通过 `plugin:statusSetUpdate` 事件发送的状态更新，将状态数据映射到 taiji 的状态栏系统。例如，pi 的 goal/todo/workflow 状态会实时显示在 taiji 底部状态栏中。
...
```json
{
  ...
  "description": "Bridges pi extension setStatus events to taiji status bar system",
  "taijiPlugin": {
    "manifestVersion": 1,
    "main": "index.js",
