...
>
> **与 [docs/CONTEXT.md](../CONTEXT.md) 的分工**：xyz-agent 全局领域词（Session / Panel / Provider / Skill / Agent / Compaction 等）的权威定义在 context.md，本表只收录 pi 平台层术语与 pi 边界域词条——「pi 边界可靠性（语义吸收层）四支柱」的权威词条在本表（context.md 仅留一行概要指路）。
