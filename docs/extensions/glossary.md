...
> 本术语表整合自 taiji-pi-extensions 项目的 CONTEXT.md。收录 pi 平台通用术语与 taiji ↔ pi 边界概念。
> 各 extension 的专属概念已迁至对应包 README（goal / todo / permission / plan / subagent-workflow 等）或随包退役删除（2026-09-13 清理）。
>
> **与 [docs/CONTEXT.md](../CONTEXT.md) 的分工**：taiji 全局领域词（Session / Panel / Provider / Skill / Agent / Compaction 等）的权威定义在 context.md，本表只收录 pi 平台层术语与 pi 边界域词条——「pi 边界可靠性（语义吸收层）四支柱」的权威词条在本表（context.md 仅留一行概要指路）。
...
**Context Files**
`AGENTS.md`（候选 AGENTS.md / AGENTS.MD），作为系统提示词的一部分加载。从 `~/.pi/agent/`、父目录、当前目录自动发现并拼接。
...
### pi 边界可靠性（taiji ↔ pi，pi-boundary-reliability 2026-08-28）

**语义吸收层**
taiji 与 pi 之间对 pi 私有语义的统一适配层（ADR-0064）：推断与跨边界承诺只在边界一次吸收（能力注册表 / 生效回执 / 确认式送达 / 漂移守卫四支柱），域内只剩确定性。EventAdapter 只适配传输格式，语义适配归本层。
