...
### 8. 区分 TUI 主进程 vs GUI 主进程（taiji / RPC mode）
...
| 维度 | TUI 主进程 | GUI 主进程（taiji） | subagent 子进程 |
|---|---|---|---|
...
| 环境变量（如 `TAIJI_AGENT=1`）| ✗ 不在 SDK | 子进程继承父环境，但 taiji 是否设置未约定，不要依赖 |
...
**GUI 主进程**（taiji）→ streamSink 启用 → `ctx.ui.setWidget` → sidecar EventBus → GUI chatStore。
