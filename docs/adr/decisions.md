...
widget 推送从「每 30s 无条件全量」改为**任务集指纹跳推**（稳定字段 id/name/schedule/enabled/nextRunAt/locale 序列化对比，不变不推；时间信息只随状态帧一次性下发——`TreeItem.when` 裸时间戳，GUI 倒计时由 renderer 每分钟本地计算，TUI 臂行内为一次性格式化的具体时间点——时间流逝不是状态变化，不得触发推送）。widget 帧曾意外承载的两个隐藏职责显式接管：①**空闲保活心跳**（入站全帧 touch `lastActivityAt`，30s 帧掩护下 scheduler 会话永不 idle）→ 显式化为「有任务且距上次推送 >10min」的保活底线帧（方案不变量：保活间隔 ≪ idle 回收阈值 ≥3 倍余量；空任务不发——pi 清屏帧同样 touch 心跳，空任务保活 = 空会话永不回收）；②**reload 恢复时机**（renderer widget 纯帧驱动无恢复链路）→ runtime per-session 帧缓存 + `sendInitialState` 补发段（清屏 = 清缓存）。同批裁决：30s tick 扫描机制抽象为**任务调度扫描通用组件**（间隔/到期判定/dispatch/tick 后回调参数化，任务抽象通用化，包内独立模块零领域 import，暂不抽独立包——第二使用方出现再抽）。设计文档 `.tmp/tech-design/scheduler-widget-push.md`（过程产物），实施落点 = u0 调度扫描抽象（行为零变化门）→ u1 跳推/when/保活（extension + extension-protocol TreeItem + renderer TrayAggregatePanel 链）→ u2 runtime 帧缓存/补发段 → u3 回归面。
