...
- ~~record-store.ts 行数 1207（U1 后）——eslint 提额 1400 过渡~~——**已收账（2026-09-13 三轴拆分执行）**：曾涨至 2698 物理 / 1429 eslint 折算（design-code-sync 追认 1450 过渡）；按 H4 登记三轴拆为 record-store.ts（容器+意图原语立面，折算 677）+ record-store-terminal.ts（终态原语轴 275）+ record-store-rounds.ts（轮次簿记轴 101）+ record-store-rebuild.ts（重建与投影轴 539）——纯搬迁、public 签名/测试零变化，D7 七名写函数调用仍物理收口在 record-store.ts（轴文件经 ctx 注入，守卫白名单零改动）；eslint 提额回落 1450→800 + rebuild 轴新设 700，登记见设计文档头部演进注记
