...
> 逐字摘录设计 §1（不改写；2026-09-16 随设计 abort-only 修正——见 §4 A2b 修正标注与偏差 D51——同批重摘目标 1）。
...
| A2b | workflow 面板与操作（abort-only、行点击 drawer workflow tab；pause/resume 已随 D-2 一次性生命周期退役，见偏差 D51 与设计 §4 A2b 修正标注） | L4 agent | 8 | 8 | 核心 | A1 | 同上；与 A2 合并同一真机会话内执行（同环境并行） |
...
| D53 | 交付后用户终裁（2026-09-16 问题 3 第二轮，推翻 D52① 保留结论） | 「已收起」第三状态**全链路删除**：shared `SubagentRecord.intent` 字段、runtime extractor 投影与 publish 去重比对、subagent-core `Intent` 类型/`ExecutionRecord.intent`/`markArchived`（语义重表达为 `markSettledOut` 资源收尾原语，不写意愿字段）/`markReactivated`/gate ①（archived 静默守卫）/注销 reason 词 archived→completed、持久化 intent 写读、全部文档机制叙述一并清除 | 用户复查后裁定「不要就是不要」——任何层不得残留该状态词汇；考古报告（.tmp/dev-flow/u-arch-archaeology.md）拆解 markArchived 六职责：仅意愿字段写随概念删除，资源收尾五职责由 markSettledOut 承接（幂等恒 true），寻回翻字段删除（revive 链 reviveOrThrow/reopen 天然承接），gate ① 删除（放弃轮标记 + status 守卫构造性覆盖，证明测试改造落地） | 三单元并行执行（u-arch-b12/b3/b4）；提交顺序 B→A 保证每个 commit 自身可编译；全量验证绿（subagent-core 3109 / runtime 5841 / 四包 typecheck / validate-constraints / doc-symbol-drift）；ADR-0067 登记 |
...
| u-fix-final-docs | committed | 3 | commit c98bba195「test(e2e): fail fast on stale mock bundle and sync protocol docs」（8 文件）；launch-app-real pre-flight（mock 产物在场即 fail-fast 带重建命令，tasks-drawer-real 经共享 fixture 获得该门）+ 协议文档 §9.1/§15.3/§15.4 后「§11 实施分阶段」表 P2 行终态刷新（原记「§15.5」系该文档章节序错乱——§11 块位于 §15 之后的存量问题——下的误标；章节序错乱独立工单，不在本轮修）+ guide Helper 行真实导出名 + todo/goal 注释改 setWidgetDual + e2e-map 注记事实修正；守卫三连（doc-symbol-drift / validate-e2e-map / select-affected-e2e --check）+ playwright --list 编译冒烟全绿，主 agent 逐 diff 核验 |
| u-fix-stage5 | committed | 1 | commit 0bd5e91a3（fix pause/resume 死链退役 abort-only，= 偏差 D51 修复）；复验证据 = §6.1 验收记录（A2b 行）；设计判据修正与 P7/检查点回填 = ba00f8887；FEATURE-PRIORITIES abort-only 行 = f99fa7e68 |
| u-two-bucket | committed | 1 | commit 8d80acabf「feat(tray): two-bucket subagent panel and fixed-height tray cards」（11 文件）：TrayNativePanel 两 tab、托盘卡片固定高 h-[340px]/max-h-[60vh] + 溢出滚动、subagent-bucket 分桶面退役（谓词 isRunningProjection/isDoneProjection 留守 SSOT）、i18n archived/pause/resume 键清理；偏差 D52 |
| u-arch-b12 | committed | 1 | commit 6f4f3bdca「refactor(subagent-core): replace markArchived with markSettledOut, purge intent mechanism」（46 文件，+589/−731）：Intent 类型/ExecutionRecord.intent 删除、markArchived→markSettledOut（3 调用点）、markReactivated 删除、gate ① 删除（迟到回注改放弃轮标记 + status 守卫证明测试）、注销 reason 词 archived→completed、持久化停写停读（旧 entry 残留键读侧忽略）、constraints C-proc-13/C-data-20 同步；全量 3109 用例绿（1 例 P99 时延 flake 系并行套件 CPU 争用，单独复跑过） |
| u-arch-b3 | committed | 1 | commit 73e295e63「refactor(subagent): drop archived intent field from shared/runtime/renderer chain」（10 文件）：shared intent 字段 + extractor :299 投影 + session-records :632 去重比对行删除，renderer 注释改两态终裁表述 + fixture 删 intent prop；异义词（killing intent / DeliveryIntent）保留；runtime 全量 5841 绿 + renderer 定向 61 绿 + 三包 typecheck 绿 |
| u-arch-b4 | committed | 1 | commit 70c309e0d「docs: purge archived third-state narrative after 2026-09-16 ruling (ADR-0067)」（6 文件）：subagent-permanent-session-model 核心重写（章节号保留，C-proc-13 引用自洽）、extensions architecture 原语清单、testing 02 两桶化、persistence-consolidation/chat-run-unification 演进注记、ADR-0067 新条目（历史条目零改写）；doc-symbol-drift 绿 |
...
| A2b workflow 面板（abort-only 口径） | PASS（12 项中 11 直过 + 1 项「行点击 no rows」由 a2b-v2b 补证 PASS；pause/resume 死链修复后复验：无 pause/resume 钮 + abort 两段式 1→0 + 行→drawer workflow tab + composer 并排） | a2b-v2.result.json（8 checks）+ a2b-v2b.result.json（4 checks） |
...
| A3b 刷新不重置用户所在 tab | 部分通过（如实拆分）：视图保持 PASS（a3b-v2.result.json 检查 3「用户所在 tab 不被重置」）；badge 2→1 传播真机时序错过——LLM 在轮询窗口外完成 todo，窗口内 badge 恒 1（1→1）无变化可观测，传播行为由托盘组件测试覆盖；badge=1 静态值已补登记 | a3b-v2.result.json + a3b-static.png + a3b-static.result.json |
| A4 常态归零（bash kill 两段式 + subagent cancel + todo 清空 + goal complete） | PASS（旧 session 4/4：呼吸点/计数全消、三件 dim 常驻；widget 侧 todo 清空消失 + goal complete 终态后消失 widgets=0——复核补采，result note 说明 goal complete 发生于验收时段末尾、本断言于 design-code-sync 阶段补跑） | a4.result.json + a4-goal-terminal.result.json（widgets=0）；中间态证据 a4-widgets.result.json（采集时 goal 尚处 blocked 非终态，widgets=1 保留显示 = 正确行为） |
...
0. **文档存量滞后（非本次引入/未闭环者，另行建档）**：① ~~extension-gui-protocol.md §4.3 / §5.2 / guide Helper 表~~（pilot 修复单元 u-fix-proto 已闭环：更新为 setWidgetDual 现行形态 + 导出清单补齐 + §9.3 废弃标注 + guide §4 整列刷新）；② ~~goal/todo 源码注释与 todo/ARCHITECTURE.md 仍写 `guiSetWidget`（实装走 setWidgetDual）~~——主句 3 处已由 c98bba195 修正（`goal/src/adapters/goal-control-adapter.ts` / `goal/src/projection/gui.ts` / `todo/src/tool.ts` 均改「经 setWidgetDual 推送」）；剩余面精确化：`extensions/universal/todo/ARCHITECTURE.md:128` 主句仍写 refreshDisplay 走 `guiSetWidget` 推送（实装走 setWidgetDual，1 处真滞后）；另注 `guiSetWidget` 是 extension-protocol 真实导出（`packages/extension-protocol/src/index.ts:42`、`core/helpers.ts:65`），上述文件与 goal 测试头注中「GUI 臂 = guiSetWidget/marker 通道」式括号引用指向真实 GUI 臂导出、语义准确，不属滞后、不处置——ARCHITECTURE.md 一处交后续 doc-sync；③ guide §3.4/§5 与 arch doc §6.3/§6.5 的 `allowComment`/`getAskUserComment` 条目指向包内已不存在的 helper（comment 随 D2 删除）——独立 doc-sync 单元范围；④ ~~docs/testing/00-overview.md 覆盖表仍列 6 个已合并的源文件名（04/05/06/07/09/14-*.md 全部 MISSING）~~——已核实为伪问题撤销：该表「合并自（旧路径，git 历史可查）」列表头明示列语义即合并来源记载（旧文件名 + git 历史可查），并非断言现行存在的滞后。

1. ~~**探针 P7（浮层溢出/翻转）+ §5 检查点 3（composer.toolbar 布局挤压）**~~——已闭环：阶段 5 真机执行完毕并回填设计 §3.6 P7 ✅ 与 §5 检查点 3 ✅（回填 commit ba00f8887；执行记录见 §6.1 P7 行）；P5/P6 已实施期关闭（设计 §3.6 已回填 ✅）。降级路径（手写 anchored 浮层）未启用——reka Popover collision 处理生效。
...
7. ~~**独立工单候选（dcs 复审 R4-1 登记，非本流水线引入）**~~——**已全部闭环（2026-09-16 用户裁决「不要下次了」，双单元并行 + 回归补修）**：① ~~commands.ts:164 TUI noop 文案~~——修复 = commit ea303c5b0（4 文件：commands.ts + subagents.ts 同款退役入口文案 + 断言联动 + SKILL.md 入口名）；② ~~domains.test `"pause"` 断言~~ + ③ ~~core 基线类型错（含 489 × TS5097：core tsconfig 缺 allowImportingTsExtensions，跨包源码直读为 tsconfig 注释声明过的有意设计）~~——修复 = commit 76a4bbf96（core tsc 496→0，2106 用例全绿）。**闭环过程带出清除管线漏网回归**：subagent-workflow `parent-child-matrix.test.ts` 6 failed（断言 record.intent / reason=archived——批3 侦察 grep 误滤测试文件所致），补修 = commit 98e426a02（对齐 markSettledOut 收口语义，包全量 944/944 绿）；教训：清除类管线的残留扫描必须含测试文件面。
...
| 2026-09-16 | u-two-bucket commit 8d80acabf 落盘（补记）：subagent 面板两桶化 + 托盘卡片固定高/滚动 + subagent-bucket 分桶面退役 + i18n 键清理（= 偏差 D52）；全局 AGENTS.md 增补 background bash 用完即关条目；dev 实例按新规则关闭 |
| 2026-09-16 | **用户终裁「已收起」全链路清除（偏差 D53）**：考古报告落盘 → 三单元并行（批1+2 subagent-core 机制 / 批3 投影链 / 批4 文档+ADR-0067）→ 提交 73e295e63 → 6f4f3bdca → 70c309e0d（B 先 A 后，逐 commit 可编译）；全量验证：subagent-core 3109 用例 / runtime 5841 用例全绿、四包 typecheck 绿、validate-constraints 131 条 / doc-symbol-drift 绿、全仓残留 grep 零（异义词 killing intent / DeliveryIntent 保留）；core 基线既有类型错登记 §7 残留风险 7③ |
| 2026-09-16 | 清除后真机冒烟（dev 实例按需开/用完即关）：8/8 PASS——12 个旧 session（entry 含 intent 残留键）加载零控制台错误（extractor 读侧容忍实证）、旧 session subagent 记录照常入托盘、面板桶恰两枚 running/ended 无 archived；证据 .tmp/dev-flow/composer-task-tray.acceptance/purge-smoke.mjs + purge-smoke.png |
| 2026-09-16 | 工单 §7-①②③ 全量清零（用户裁决「不要下次了」，双单元并行）：ea303c5b0（TUI 退役入口文案 ×2 + 断言 + SKILL.md）/ 76a4bbf96（core tsc 496→0：allowImportingTsExtensions + 补 import + 删 chatMode 死行 + 'pause'→'abort'，core 2106 用例全绿）；闭环扫描带出清除管线漏网回归 parent-child-matrix 6 failed → 98e426a02 补修（944/944 绿） |
