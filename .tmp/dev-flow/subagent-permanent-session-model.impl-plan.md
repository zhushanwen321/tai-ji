...
基线: c2c111170 | 来源设计: docs/design/subagent-permanent-session-model.md（创建时点路径；该文件后随 design/ 目录消解批次迁移至 docs/architecture/subagent-permanent-session-model.md，§2 U9 行的 docs/design/ 引用同理）| 日期: 2026-09-13
...
| S3-R5（定向复审 P3 边角登记）：markRoundIdle `.state` 收条「轮中崩溃过时展示」——round N 成功（收条 completed）→ round N+1 在飞宿主崩溃 → 重建读出 stopReason=completed 而非 interrupted-by-restart 兜底。收条语义 = 最后一次收口轮（非最后一轮活动），展示位偏差不参与资格判定，接受；若不可接受再评估 markRoundStarted 清收条（A-full 方向需重裁决） | 定向复审 | 已知边角显式接受 |
| ~~U5-D8~~（验收 P3 修正失效）：reconstruct 重建依据已脱离注册表（repoPath 入参 + pi-sub-&lt;id&gt; 命名约定 + rev-parse --verify），归档 cleanup keepBranch:true 保留分支——归档寻回恒走 rebuilt 正常形态（commit 8ea19681e，A5 剧本 18/18 PASS，P3-8 断言已翻转为「原地续聊不降级」） | 验收P3 | 设计 §3.2.5「续聊时按需重建」能力兑现 |
| AC-P1（验收发现/已修）：cancel 收敛窗 3s→30s（实测 pi 停轮 15s）+ 迟到轮应答双闸（abortAndClearQueue 废弃轮身份 + 回调闭包 runId 全等校验）——S1 主路径 8/8 复现闭合（d01f0f225，A1 22/22 两跑）；杀目标收窄（killAll 组杀→按 run）登记后续裁决 | 验收P1 | 量级按对象粒度校准原则 |
| AC-P2（验收发现/已修）：resurrectColdRecord 水合 engine/engineHandle——重启后 zcode 续聊错投 pi 闭合（5dcb99453，A3 16/16） | 验收P2 | 单点，变体实验先行验证 |
| DCS-S3（design-code-sync contested→doc-side 默认）：归档保留分支（keepBranch）无独立回收通道——裁决显式接受「分支随 record 语义存续」（轻量 ref，量级远小于 worktree/transcript），重审条件 = 主仓 pi-sub-* 分支数超 500 或分支名冲突事故，届时复审 transcript GC 联动回收通道；裁决已回写设计 G3 注记 | 阶段6 | 两合法出口取声明侧，通道缺位显式化 |
...
| U8b | committed | 1 | commit c47871d05（29 文件）：idle 归进行中桶（经 U8a helper 投影）+ 默认可见性翻转 + archived 专属过滤器（旧 ended/all 视图退役）+ badge 判据切 isRunningProjection + cancel 乐观更新两态化 + 快修①③④（⑤U7 根修 + GUI 验收断言）+ renderer tsc 19 存量清零（ShellInputInstance 结构契约）；renderer 4314 / core 2064 / runtime session 子集 544 全绿 |
...
- 2026-09-13 阶段 3 完结：三分区对抗审查（core 状态机/引擎 RPC/投影 GUI）+ Gate A 全量（本流水线领地全绿，归因记录 gate-a.log：2 失败=并发 flake 单跑绿 + 并行会话在制）+ 修复批次全 commit（守卫补齐 3143447b5 / relay 收敛 a4f55afc3 / 组1 轮终写面 741141321 / 组2 closeAfterRound / 组3a aaf8041f1 / 组3b 2890700d0 / 回流微修 726271d33）+ 设计事件表 A-lite 注记 8ec9a4d10 + 定向复审 7 条 unreasonable 全闭合（verdict 由 fail 转清：唯一 P2 = 并行 commit 9f914b749 影响面遗漏已代修）。事故：ca0d412f8 裹挟并行会话 staged 内容 → soft reset + pathspec 重提交（726271d33），教训=commit 一律 pathspec。外部残留：inflight-reporter.test.ts 3 断言（并行会话 inflight 域领地，待其自修）。阶段 5 验收启动。
- 2026-09-13 阶段 5 验收闭环：三 L3 剧本（真 pi 0.84.4 / 真 zcode 0.16.5+真 LLM / 真 git worktree）各捉真实缺陷并全部修复——P1 cancel 收敛窗+串轮（d01f0f225，A1 22/22 两跑）/ P2 engine 水合（5dcb99453，A3 16/16）/ P3 worktree 重建四层含 U5-D8 修正（8ea19681e，A5 18/18）；A2 体验流畅性挂起（L4 需用户在线）；A7/A8/A9 由 Gate A + U5/U8a/U8b 测试覆盖。剧本沉淀 .tmp/dev-flow/<plan>.acceptance/（一次性，gitignore）。功能分级登记：纯缺陷修复不触发分级变更，跳过。事故：验收探针首版漏设 env 致 pi 写真实 ~/.pi/agent/subagents（37 文件全为本会话产物，已删净验证）；commit 裹挟事故×3（并行会话 staged 内容）均 soft reset + pathspec 重提交。外部残留：inflight-reporter.test.ts 3 断言（并行会话 inflight 域）。阶段 6 design-code-sync 衔接。
- 2026-09-13 阶段 6 终态同步收敛，**流水线整体交付完成（阶段 0-6 闭环）**：design-code-sync 终态校准 8 findings 一轮清零 doc-side（da396cba8：2 must-fix + 4 suggestion + 2 info——U5-D8 worktree 重建演进注记 / relay-registry 残留三处同源指针收敛 / §0 验收 commit 回填 / 轮身份守卫注记 / G3 keepBranch 裁决回写 / U8b 状态行补登 / design-path 迁移注记；定向复审修复 36a36c64a：hash 错字 ×2 / G3 交叉引用重指 / SIGCONT 断言域收窄 kill('SIGCONT') / A1 归因精化），reviewer evidence 代码侧 findings 零 + 反引号标识符 grep 全命中 + check-doc-symbol-drift 绿。**交付遗留跟踪**（非门禁项）：A2 体验流畅性 L4（待用户在线真机体验）；inflight-reporter.test.ts 3 断言（并行会话 inflight 域领地）；zcode -32031 上游缺陷（设计 K6 升级项）。本终笔为 quota-wait 挂起恢复后的收尾校准（恢复会话核实 da396cba8/36a36c64a 后补记，挂起状态文件 .zcode/quota-wait-state.md 同步清除）。
