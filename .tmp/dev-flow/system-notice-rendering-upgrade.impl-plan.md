...
- 2026-09-16 阶段 6 design-code-sync（终态同步，基线 d38f3b499）：双区审查（A 代码↔设计文档 / B impl-plan 现实性+机械信号）共 14 finding（2 must-fix / 4 suggestion 级 / 8 info）。收敛轨迹：R1 修复 14 条（contested 1 条裁决记录：A-F1 D6 双测量路径按 **code-right** 收口——实施期证明浏览器环境 markdown 行高可精确实测（探针 P1 实测 21.0px），「约」估算变体失去动机，from–to demo 裁决形态保留；本 contested 为 must-fix 级，按全托管模式自主裁决，**供用户事后复核否决**）→ R2 复审 13 closed + 3 新差距（NEW-1 修复未提交 must-fix / NEW-2 检查点 2 标注缺失 / NEW-3 简称残留 info）→ R2' 修复 4 处（7aa7d7a0e）→ R3 复审 **must-fix==0** 终止（残留 1 处 info 简称当轮修完，1e10a1a81）。Step 5 退役判定：本设计伴生产物合规（设计文档/impl-plan 在 .tmp/ 约定路径、探针 spec 已删）；docs/ 下 ext-simplify-*.review.md 系列属其他设计历史遗留，不在本轮范围。
