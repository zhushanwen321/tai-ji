...
- 全量（阶段 3 尾 Gate A，范围 = 改动面圈定 [2026-09-16 用户裁决修订：Gate A 范围须在计划期约定，非默认全仓]）：改动面 5 包测试全跑——`pnpm -r --filter '@zhushanwen/pi-subagent-workflow' --filter '@zhushanwen/pi-structured-output' --filter '@zhushanwen/pi-session-reader' test` + `packages/subagent-core` 全量 vitest + `packages/shared` 全量 vitest（constants 契约域）——+ `node scripts/check-doc-symbol-drift.mjs` + 三门禁（validate-e2e-map / select-affected-e2e --check / pi-semantics 守卫）+ typecheck/lint 全量（extensions 三连前两段）+ pre-commit 全套（历次 commit 附带跑过，引用其输出）。范围裁剪依据：runtime 零改动、renderer 三包只读核实零 diff、其余 16 个 extension 包与改动面无 import 关系（零回归信息）
...
2. ~~⛔ 4 项实施期门（设计 §5）~~ **已全部闭合（2026-09-16/17，逐项证据）**：S5 = **静默忽略分支**（u4 typebox 证据链见偏差表 u4 行 + A5 真机确认 collect:'sync' 透传无报错，缓解动作①②均已落地：subagent-tool.ts:165 迁移提示句 + CONTEXT.md 迁移说明）；runWorkflow 同步段 = **毫秒级、guard 保留**（u2 偏差行「期门关闭」+ A1 基本链）；通知体积截断 = **truncated 保序落地**（u1 状态表契约抽验 fan-out.js L185-216 + A1 通知实测）；存量读侧容忍 = **u6 mock 轨 24/24 + A5 真机**（D6 无需回改）。终局结论同步见设计文档 §5 待验证检查点节
3. u5 删除型单元机械联动文件数超 5（全局经验值约束）——按 D6 清单 + grep 终扫控制范围，行数净减，风险可控。**已消化（u5 committed 后未发生联动事故，grep 终扫见 u5 状态表）**
4. run-spec.ts「≤20 字符」注释与 SLUG_MAX_LENGTH=35 既有不一致——实施期登记不扩大处理（设计 P1.2 裁决）。**维持登记（范围外存量，git 可追溯）**
...
- 2026-09-16 v6：定向复审返回——**unreasonable 为零**（12 条闭合点逐项核实通过，S3 测试锁定真实），5 条 doc_errors 经主 agent 逐条核实后处置：DR-1（architecture.md:5 五聚合漏改，f0026b5f3 只改 :67）/ DR-2（BATCH-03 note 将来时）/ DR-3（record-lifecycle warn 死参数指引 conversation:true + architecture.md:118 同款）/ DR-4（chat-run-unification 两处未随 modeless 波5 演进）4 条属实已修；**DR-5 裁决为误报**——notifier.ts 三处 + pi-semantics 测试的「D5 单通道」锚的是 pi-boundary-reliability.md 附录 D D5（courier 单通道化，被否方案原文含 nextTurn 滞留证伪），复审 agent 只查了批量设计的 D5；该误报反向暴露 b18759086 微修把 session-lifecycle.ts:342 与 notifier-flush.test.ts 5 处**正确锚**错挂 u9 名下（u9 锚只适用 notifyDone 降级路径），同批回改修复。修复 commit 5da0ea830（6 文件，核验：diff 逐行 + shell 970/970 + recovery-bounds 6/6 + validate-e2e-map + doc-symbol-drift，pre-commit 全绿含聚合边界守卫机器印证六聚合）。**审查清零判定：unreasonable 连续两轮为零**（第 1 轮三区审查 7 条已修 + 第 2 轮定向复审 0 条），转入 Gate A 全量测试
- 2026-09-16 v7：**Gate A 全量测试通过**（范围 = §4.1 修订后改动面全量，用户裁决；输出落盘 `.tmp/dev-flow/subagents-batch-tool-fanout.gate-a.log`）：① extensions typecheck+lint 全量 exit=0；② 改动面 3 extension 包全绿——subagent-workflow 970/970、structured-output 193/193、session-reader 405 passed + 2 skipIf；③ shared 416/416；④ subagent-core 3042/3046（+1 文件 skipped 计入 207 passed | 1 skipped）；⑤ 三门禁 PASS/OK/OK（select-affected-e2e --check 7/18 命中改动面无漏登记 / pi-semantics 37 条 / validate-e2e-map 18 rules）。**skip 披露（零容忍纪律核对，均非绕过）**：session-reader 2 = 真实数据守卫 `describe.skipIf(!HAS_REAL_WF_*)`（workflow.test.ts:378/:393 既有形态，本分支该文件零改动）；core 4 = engine-conformance.live.test.ts 手动门（真实引擎进程 conformance，设计内按改动面开发期跑，e2e 三态纪律 R2）。**误跑段无效判定**：一段命令因 cwd 落在仓库根触发 vitest workspace 全量（1774 文件/391 failures）——抽查 packages/ui useTailScroll 于本包目录 10/10 绿，实锤为环境错配假失败（项目验证形态 = pnpm 逐包跑非根触发），该段输出不作 Gate A 证据（log 内留有 CORRECTION 标记）。**pre-commit 全套**：b18759086 / 3398be2d0 / 5da0ea830 / f1cda5ffd 各次 commit 附带全绿（lint/hooks 引用证据）。**覆盖矩阵**：u1（fan-out-script 19 + builtin-structure）⊂②④；u2/u4 shell 族 ⊂②；u5/u9 core 族 ⊂④；u6 存量容忍 ⊂②session-reader+④record-store；u8 三门禁 = ⑤——全部单元领地有测试认领，runtime/renderer 零改动（零 diff 佐证）无 uncovered 区。**阶段 3 收口，转入阶段 5 端到端验收**
- 2026-09-16 v8：**阶段 5 验收中期结论**（剧本/断言产物落 `.tmp/dev-flow/subagents-batch-tool-fanout.acceptance/`）。**A1/S1 真机基本链 PASS**：模型一次 subagents 调用 → fan-out run → 单条 workflow-result 通知（status=ok / results=2 / taskIndex 0,1 派发序 / 计数 3+2 正确 / Agent Trace 双 completed）+ ledger 落账键 wf-done:wf-… 实证。**A2/S2 三审负载 PASS**（并行线）：3 tasks 单条通知收齐（runId wf-1789572379302-amt23t，三成员 150/188/242s 依次收口），三份报告落盘三视角正交（12K/17K/13K 互异），父 idle 215s 后通知唤醒零轮询，stderr 零 429。**A4/S4 abort PASS**：done (aborted) 通知照达（非静默）+ 双成员 failed 留痕 + abort 后零孤儿进程。**A5/S5 collect 退役 PASS**：collect:'sync' 真实透传静默放行（S5 ⛔期门真机闭合——与 u4 typebox 证据链一致）、零批副作用（batchFinalized/workflow-record 全零）、单发 async 收口（notifyId sa-*:1 非 wf-done 前缀）；「旧 session 可读」真机裁剪（u6 mock 轨 24/24 已覆盖，pi CLI 无渲染面）。**A3/S3 4/5 PASS、④ FAIL=真缺陷进修复循环**：①partial 收口（1 failed 带 error+2 ok）②③taskIndex 完整 ⑤agents 错配 fail-fast 带 Correct 示例且零成员派发均过；④批成员收口归档后 message 走冷查复活链绕过 D7 守卫（成员真收到消息并回话）——归因（主 agent 行级核实）：cold-lookup.ts resurrectColdRecord 的 createRecord 透传清单缺 origin/parentRunId（热路径 workflow-dispatch:226 / 落盘 run-orchestration:453 binding sidecar / 热重建 record-store:1473 均带，唯冷复活链漏），守卫 actions-core:537 判 undefined 放行。修复 commit 1543b39c1（identity 域经 createRecord，engine 先例同款；详见 v9）。**真机环境三要素（踩坑固化）**：①TAIJI_AGENT_ENGINE_ROOTS=<worktree>/packages（裸跑三级发现全落空——L1 pnpm symlink 被 Dirent.isDirectory() 过滤/L2 argv 上溯不到/L3 config 无）②pi 进程 cwd=worktree 根（findWorkspaceRoot 前提；首版剧本 cd 误写管道左端子 shell）③.pi/workflows/ 布置六模板+_shared/ 副本（含 agent-refs.cjs；验收临时产物，收尾清理）。**执行侧事故披露**：A2 agent 首轮清理误用宽泛 pkill 全机杀 pi（23:10-23:12 窗口）；A3-5 agent 对账正式轮全在窗口外（23:18/23:32/23:44 起），窗口内仅其剧本 bug 秒退废轮，无断言污染
- 2026-09-17 v9：**阶段 5 验收全场景收口**。**A3④ 缺陷闭环**：修复 commit 1543b39c1（createRecord identity 域扩 origin/parentRunId + cold-lookup 透传 + 冷复活正反双场景用例；核验 73/73 + pre-commit 全绿），真机重验派后台执行中（结论见 v10）。**A6/S7 断连重放 PASS（ground truth = 会话文件 + runtime log，GUI 截图为辅——图片模型小字读数不可靠，8n4bkc/partial 读数系误读，真身 sdr56e）**：①注入 = kill 成员 relay 代理（PID 精确）→ runtime log `connection lost, killing child … exited code=143`（kill-on-disconnect 连坐杀链实录）；②run 收口 done(failed)（traceLength 2 全员死亡形态——比 partial 更强的通知压力位；partial+通知照达已由 A3 CLI 真机覆盖）；③**通知照达且恰好一条**（entry 唯一 workflow-result custom_message；「计数 2」系把 ledger 条目 deliveryCustomType 字段误计）；④**ledger 恰 1 条 wf-done:sdr56e + ack 销账恰 1 条**（at-least-once 真实断连下落账/销账正确，对照 collect 时代通知黑洞）。**A8/P1.3 判定 = L0 pass + L3 大部分**：u3 代码核对在案（Block.vue isWorkflow→collapsed+openWorkflowDrawer）；GUI 证据 = 批量启动卡/通知卡三形态（completed×2+failed）/Trace 行可见/批量链路双 run 稳定；**缺口登记**：「点击折叠块开 drawer 成员可见」实操截图未取得（dev GUI 视图考古超时预算——conversation 中 subagents 调用渲染为 args-chip+启动卡形态、trace 页为 raw entry 行，Block.vue workflow 折叠块的挂载位置未定位到，留 design-code-sync 阶段对照 D8 渲染归宿表述）。**终态同步定谳（v12）**：缺口实为结构性差距——批量块无 name 入参 → drawer 选中不可达（成员永不入列），详见 v12 纠错记录。**dev GUI 链路顺带实证**：批量 tool 在 taiji dev 真实链路（runtime 注入 staged 引擎根 + agentDir 隔离发现模板）双 run 稳定收口；模型对通知的误读（自称 failed 实为 completed）与自发重派均为模型质量观察项非系统缺陷。**验收环境清理完毕**：dev 实例与 pi 进程零残留（用户自装打包版太极未触碰）、`.pi/workflows` 与 `~/.taiji-dev/agent/workflows` 模板副本已删、fixture 临时目录留 OS 清理。**dev-flow skill 增补**（源 repo commit 23f03bf）：验收预飞自检两步 [MANDATORY]（环境 smoke + 剧本 dry-run）——本次 5 轮浪费的流程化复盘
- 2026-09-17 v10：**A3④ 双缺陷闭环 + 阶段 5 全场景收口**。真机重验暴露第二个存量缺陷（归因 agent 行级钉死，非 1543b39c1 回归）：message 冷查兜底全扫（coldLookupDeps 装配 record-access.ts:480-481 三参调用）吃到 store 出口的 origin 展示过滤（record-store.ts:918-920 includeWorkflow 缺省 false）→ 归档 workflow 成员被滤光 → 候选 undefined → 上转为误导性「different session tree」文案（同树 rootSessionId 逐字相同）且 D7 守卫冷路径不可达；原 A3 的 delivered:true 竟系「模型碰巧先调两次 list 预热 idToFile 索引走了无过滤直查」的无关副作用，单测盲区同源（cold-lookup.test mock collectRecords 无过滤）。修复 commit 5ccdccd59（冷查口径 includeWorkflow:true——治理语义非展示面，+红绿翻转回归用例；核验 80/80 + tsc 0 + pre-commit 全绿）。**真机终验全绿**：断言 0/1/2/3 全 True，D7 守卫文案逐字命中（workflow-origin record + To get these results, re-dispatch via the subagents tool），无 delivered 透传。**阶段 5 终判**：A1/A2/A3(5/5)/A4/A5/A6 全 PASS，A8 partial（drawer 实操截图缺口登记）；顺带挖出并修复两个存量缺陷（冷复活 identity 透传缺失 + 冷查口径滤错域）。重验模板副本已清理。**登记待裁决项（归因 agent deviations，不阻塞）**：①endedMessageGuard 跨树分流假设（not owned + 全态查得到 = 跨树）被证伪未加固——修复后该误报形态消亡，防御性比对 snap.rootSessionId 属超范围 ②closeHandler 对 workflow record 无 D7 同款域拒绝（修复后冷查可定位，行为 = 幂等收口与预热时一致，是否补域边界属设计裁决。转入阶段 6 design-code-sync 终态同步
- 2026-09-17 v11：**阶段 6 design-code-sync 终态同步**。终审 10 findings（must-fix 4 / suggestion 5 / info 1；关系 d 零发现、D1-D7 六项终态逐条通过、追加双修复三方一致）。处置：F1（残留风险 #2 期门状态过时）/F2（pi-boundary 附录 B notifyDone 销账）/F3（e2e-map BATCH-03/06 终态）/F5（D4 aggregate 失败语义补记）/F6（D9 执行体缺席行）/F7（TEST-STRATEGY 现行示例）/F8（设计期门节终局结论）/F9（close 批成员裁决 → architecture.md 批量编排行）全部主 agent 亲修（fix-hint 逐字级登记回写，doc_errors 亲修先例）；F10 info 级按 fix-hint 无需处理。**F4（D8 GUI 归宿，contested must-fix）取证闭合**：补 L3 实操——workflow 折叠块 [data-testid=workflow-block] 在 conversation 实测存在（形态「Workflow·<slug>」，此前搜不到系我搜大写 WORKFLOW 而 i18n 渲染 Workflow），点击开 drawer workflow tab。~~二次点击选中 run → 成员 agent call（fan-out-0/1）入列可见（证据 f4-drawer.png / f4-drawer2.png）——D8 表述与实现一致，A8 由 partial 升 **PASS**，阶段 5 验收全场景无缺口~~**【v12 纠错】以上删除线内容为错误结论**：截图实为 drawer 空态（我误读图片 + 忽略了当时 DOM 探针返回的空态文案——双重失误），聚焦复审经代码链证伪（openWorkflow(name) 且批量无 name → 选中结构性不可达）。块存在 + 点击开 tab 两项证据仍有效；成员入列不成立。
- 2026-09-17 v12：**design-code-sync round-1 复审 fail → round-2 修复**。复审裁决：F1-F3/F5-F8 修复成立（8/10），F4 取证链断裂（截图空态 + 代码链结构性反证）→ **纠错**：撤销「A8 升 PASS」（回落 partial），真实差距 = 批量块 drawer 选中链路不可达（schema 无 name → openWorkflow('') 仅切 tab，Block.vue:461-472 + 注释明载）；v11 的错误证据声明已在原位打纠错标注（保留可追溯）。F9 修复引入表格语法破坏（追加文本悬于闭合管道后）已修（并入第 3 单元格）。本轮修复：设计文档 D8 终态修订（批量成员入列断言按现实改写 + 成员查看现实路径 + renderer 补链登记待用户裁决——超「零 renderer 改动」边界）+ P1.3 条款同步划线；次要顺手项：设计正文 4 处 ⛔ 期门回指终局、TEST-STRATEGY 示例句 R3 措辞修正。**教训登记**：验收/取证断言必须以 DOM 探针/会话文件等机器可读 ground truth 为准，图片读数（无论图片模型还是主 agent 亲读截图）仅作辅助——本轮同一陷阱踩两次（runId 误读 + drawer 空态误读），均已纠错。**待用户裁决项汇总（交付总结呈现）**：①批量块 → drawer 选中 → 成员入列通路的 renderer 补链（产品取舍）②endedMessageGuard 跨树分流防御性加固 ③closeHandler 是否补 D7 同款域拒绝（现 = 幂等收口放行）
