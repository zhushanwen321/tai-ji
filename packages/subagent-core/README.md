...
活例：本仓 `extensions/universal/subagent-workflow/src/host/pi-host.ts`（`createPiHostServices`）。要点：`dataRoot` / `discoveryRoots` 每次调用现取 `getAgentDir()`（禁模块级缓存——pi 实例按 session dir 隔离），log 桥接 `@zhushanwen/pi-extension-logger`，notify 桥接 `@taiji/session-delivery` + `@zhushanwen/pi-pending-notifications`。扩展初始化最早处：
...
`dist.bundle/index.cjs` 是**单文件自包含 CJS 产物**：tsup `noExternal` 把全部运行时依赖（`@taiji/*`、`ajv`、`yaml`、`proper-lockfile`）内联进产物，require 链上只剩 node 内建模块。
