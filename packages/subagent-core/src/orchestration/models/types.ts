 ...
 */
  ...
  returnMeta?: boolean;
 /**
  * 空闲超时毫秒数（原 AgentTaskSpec.idleTimeoutMs 并入，全 record 生效的 idle GC
  * 节奏——原「仅 conversation 模式」限定随 chatMode 消亡移除）。
  * 优先级：参数 > env TAIJI_SUBAGENT_IDLE_TIMEOUT_MS > 默认 300000ms；显式 0/负 = 禁用
