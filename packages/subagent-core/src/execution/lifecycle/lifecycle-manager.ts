// src/execution/lifecycle/lifecycle-manager.ts
//
 ...
 * 错误文案基准（run-orchestration.ts）+ env TAIJI_SUBAGENT_IDLE_TIMEOUT_MS 非法值
...
/**
 * 从环境变量 TAIJI_SUBAGENT_IDLE_TIMEOUT_MS 读取全局默认超时。
 ...
 * 白名单（packages/shared/src/constants.ts 只有 TAIJI_ 等，无 PI_），taiji 桌面
 * spawn 链（safe-env 过滤）会丢弃该 env，配置口在桌面场景静默失效——已改名 TAIJI_
...
function getEnvIdleTimeoutMs(): number | undefined {
  const raw = process.env.TAIJI_SUBAGENT_IDLE_TIMEOUT_MS;
  if (!raw) return undefined;
  const parsed = Number(raw);
  if (!Number.isFinite(parsed) || parsed <= 0) {
    logger.warn(
      `[lifecycle-manager] TAIJI_SUBAGENT_IDLE_TIMEOUT_MS="${raw}" is invalid (expected a positive millisecond number) — falling back to DEFAULT_IDLE_TIMEOUT_MS (${DEFAULT_IDLE_TIMEOUT_MS}ms); set a plain ms value (e.g. 1800000) to override`,
    );
    return undefined;
  }
  return parsed;
 ...
 * 默认行为不变（资源回收性质，默认值保留）：不传 → env TAIJI_SUBAGENT_IDLE_TIMEOUT_MS
 ...
 * @param timeoutMs 可选。SP-6 优先级：参数 > env TAIJI_SUBAGENT_IDLE_TIMEOUT_MS > DEFAULT_IDLE_TIMEOUT_MS；
 *   显式 <=0 表示禁用（不挂 timer）。
...
export function armIdleTimer(
  ...
  // SP-6 优先级：参数 > env TAIJI_SUBAGENT_IDLE_TIMEOUT_MS > 默认 300000ms (5min)。
