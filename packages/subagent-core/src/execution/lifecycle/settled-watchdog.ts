...
//
// 用户通道（规则 19）：env TAIJI_SUBAGENT_SETTLED_WATCHDOG_MS——>0 覆盖收尾段定值，
 ...
 * 可被 env TAIJI_SUBAGENT_SETTLED_WATCHDOG_MS（>0）覆盖，仅此一处定义。
 */
 ...
 * 默认 + warn 留痕（对齐 TAIJI_SUBAGENT_IDLE_TIMEOUT_MS 的 LC-7 教训：非法回落必须
 * 可见）。前缀用 TAIJI_SUBAGENT_*（ENV_WHITELIST_PREFIXES 白名单，PI_ 前缀在桌面
 * spawn 链被静默丢弃——同 TAIJI_SUBAGENT_IDLE_TIMEOUT_MS 改名教训）。
 */
export const SETTLED_WATCHDOG_ENV = "TAIJI_SUBAGENT_SETTLED_WATCHDOG_MS";
