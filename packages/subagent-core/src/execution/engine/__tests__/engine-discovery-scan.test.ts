 ...
 * 防宿主环境（打包态 TAIJI_AGENT_ENGINE_ROOTS / workspace 链接的引擎包）污染断言。 */
...
function scanOpts(overrides: {
  ...
  const env: Record<string, string | undefined> = {
    TAIJI_AGENT_DATA_DIR: "/test-data-dir",
    ...overrides.env,
  };
...
describe("manifest schema 字段级解析", () => {
  ...
  it("envPrefixes 缺省 []；空串/含 */非法形态/保留前缀 → 丢弃该前缀 + warn，包仍可用", () => {
    makeEnginePkg(tmpRoot, "bad-prefixes", {
      envPrefixes: ["", "HAS*STAR", "BAD-DASH", "TAIJI_HOST", "taiji_agent_", "GOOD_", "good_"],
      capabilities: FULL_CAPABILITIES,
    });
    const result = scanEngines(scanOpts({ roots: [tmpRoot] }));
    // 包仍可用（A12②）：装载成功
    ...
    // 保留前缀大小写不敏感（TAIJI_HOST / taiji_agent_ 均拒）
    ...
    fs.writeFileSync(
      ...
      JSON.stringify({
        engines: {
          ...
          "l3-ghost": { command: "definitely-not-on-path-foo" },
        },
      }),
    );
    const result = scanEngines(
...
describe("TAIJI_AGENT_ENGINE_ROOTS env 解析", () => {
...
describe("冲突与失败", () => {
  ...
  it("坏包不阻断其他引擎（A12②）：坏 manifest 与好包共存扫描，好包正常装载", () => {
    ...
    fs.writeFileSync(
      ...
      JSON.stringify({ name: "broken", "taiji": { subagentEngine: { bin: "x", protocol: 1 } } }),
    );
