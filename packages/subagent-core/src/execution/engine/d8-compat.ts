...
//     注册，engineDataDir 经合成 env（TAIJI_AGENT_DATA_DIR）记入 descriptor portFactory；
...
// 通道②（宿主注入 TAIJI_AGENT_ENGINE_ROOTS）由发现器承载，薄壳在注册表已有 descriptor
...
//
// hostKind：registerZcodeEngine 缺省 'pi'（现调用方 = taiji 扩展宿主组合根）；
...
/** registerZcodeEngine 缺省宿主（taiji 扩展宿主组合根是现调用方）。 */
...
export interface D8CompatZcodeEngineDeps {
  /** 引擎数据根（→ 引擎子进程 env TAIJI_AGENT_DATA_DIR，见设计 §3.6 注入矩阵）。 */
 ...
 * pkg["taiji"]["subagentEngine"]。读文件/JSON 解析失败或 manifest 段缺失/非对象
 ...
 * 合成 env TAIJI_AGENT_DATA_DIR 被 portFactory 捕获——「以显式值为准」）。[W11/DoD#5]
 * 定位/解析失败保持未注册（inproc 回退已删，不变量 6「失败不静默」——派发期
 * engine_not_found 显式报错 + 恢复指引；TAIJI_ZCODE_CLI 覆盖通道由 createZcodeEngine
 * 的 deps.cliPath 等价承载）。
 */
export function registerZcodeEngine(engineDataDir: () => string = getEngineDataDir): void {
  ...
  // TAIJI_ZCODE_CLI 覆盖通道由 createZcodeEngine 的 deps.cliPath 等价承载。
  ...
  // 合成 env：显式 engineDataDir 写进 TAIJI_AGENT_DATA_DIR，portFactory 闭包经
  // getEngineDataDir(env) 解析即取显式值（W4 inspectEnginePackage 原样复用，零改动）。
  const syntheticEnv: NodeJS.ProcessEnv = { ...process.env, TAIJI_AGENT_DATA_DIR: engineDataDir() };
