...
//     process-manager.ts spawn env 补注入 TAIJI_AGENT_DATA_DIR 修复（跨包改动）。
//
 ...
 *   1. process.env.TAIJI_AGENT_DATA_DIR（taiji 宿主注入，权威通道）；
...
export function getEngineDataDir(
  ...
  if (!warned) {
    warned = true;
    warn(
      `[engine-data-dir] ${TAIJI_DATA_DIR_ENV} is not set; engine journal/pool fall back to the pi agent dir ` +
