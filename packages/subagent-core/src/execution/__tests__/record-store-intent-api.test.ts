...
//     markFinalized/markCancelled/adoptEngineDeath/markResurrected/markIdleArchived，
//     [collect 退役] 原 markBatchFinalized 已删）+ acquireWriteLease（A6）；
//   - A2 终态写序（D8 v7）：`.state` writeSync 先 → entry/archive → manifest
//     writeSync → `.alive` 删除；markRoundIdle 簿记⑦ `.alive` 保留；markIdleArchived
