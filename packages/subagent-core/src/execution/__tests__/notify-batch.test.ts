...
const STRIPPED_ENV_PREFIXES = ["TAIJI_SUBAGENT_RELAY_", "PI_SUBAGENT_"] as const;
...
describe("buildBatchNotifyId — sync-batch:<sha1(sorted id:epoch:round)>", () => {
  ...
  // [round2-notify-fix 合并注] 原「E1 重建补发成员与 flush 快照同键」回归锁随
  // modeless 波3 删除：syncRebuildToNotifyMember（E1 恢复批补发映射）已随 E1 恢复面
  // 退役，锁的函数不存在了。buildBatchNotifyId 的 round/epoch 防撞不变量仍由上方
  // 用例覆盖。
});
...
describe("notifyBatch — ledger 写账 → 边沿投递（与 notify 同一通道，单 entry）", () => {
  ...
  it("幂等：同成员集二次 notifyBatch 被账本拒绝（零副作用）——E1 重建重发防线", () => {
    ...
    // 乱序同成员集 → 同 hash → 拒绝（logger 模块级共享，计数从本断言起算）
    loggerMock.warn.mockClear();
    expect(notifier.notifyBatch([members[1]!, members[0]!])).toBe(false);
    expect(mock.entries).toHaveLength(entriesBefore);
    // [round2-notify-fix] 拒绝不静默：账本幂等拒绝 warn 留痕（2026-09-14 事故第③层
    // 缺陷——拒绝静默零日志延误排障；review-test-coverage S1 补锁）
    expect(loggerMock.warn).toHaveBeenCalledWith(
      expect.stringContaining("notify ledger rejected duplicate notifyId"),
      { detail: { notifyId: buildBatchNotifyId(mid("sa-1", "sa-2")) } },
    );
