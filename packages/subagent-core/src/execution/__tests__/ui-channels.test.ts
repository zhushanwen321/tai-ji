...
//   - ASK_USER_MARKER = "\0TAIJI_ASK_USER"，payload 在 options[0]
//   - GUI_WIDGET_MARKER = "\0TAIJI_GUI_WIDGET:" + JSON，payload 在同行
// channel 名规范化：去 "TAIJI_" 前缀，小写化（TAIJI_ASK_USER → ask_user）
//
// UiChannelRegistry:
...
// title = "\0TAIJI_ASK_USER"，options[0] = JSON.stringify({questions, allowCancel})
const ASK_USER_MARKER = "\0TAIJI_ASK_USER";
...
// widgetLines[0] = "\0TAIJI_GUI_WIDGET:" + JSON.stringify({component:{...}})
const GUI_WIDGET_MARKER_PREFIX = "\0TAIJI_GUI_WIDGET:";
...
describe("parseChannel — channel 名规范化规则", () => {
  it("去 TAIJI_ 命名空间前缀 + 小写化（TAIJI_ASK_USER → ask_user）", () => {
    const req = selectReq("\0TAIJI_ASK_USER", [JSON.stringify({ x: 1 })]);
    expect(parseChannel(req).channel).toBe("ask_user");
  });

  it("TAIJI_GUI_WIDGET: 带冒号后缀 → gui_widget（去前缀 + 去冒号 + 小写）", () => {
    const req = setWidgetReq("k", ["\0TAIJI_GUI_WIDGET:" + JSON.stringify({ y: 2 })]);
