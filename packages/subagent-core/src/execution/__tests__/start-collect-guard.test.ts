...
//   1. [modeless 波3·E4 删除] collect:"sync" 组合放行（原 sync+conversation 前置拒
//      随 conversation 派发参数删除退役）——collect 是派发时的通知路由选项，不再是
//      record 模式，无需前置拒；
...
// ============================================================
// [modeless 波3] E4 删除：collect 组合放行（conversation 派发参数已随波 5 删除）
// ============================================================
...
describe("startHandler collect combinations ([modeless 波3] E4 deleted)", () => {
  ...
  it("allows collect:async alone (async 语义与 sync 正交)", async () => {
    const service = makeService();
    const result = await sh(service, { ...BASE_INPUT, collect: "async" }, undefined);
    expect(result.subagentId).toBe("sa-new");
    expect(service.execute).toHaveBeenCalledTimes(1);
  });
