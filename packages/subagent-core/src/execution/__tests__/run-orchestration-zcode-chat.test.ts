...
describe("U6b：zcode chatMode 的 Continuation 接线（B-firstround + B-routing + onHandleReady）", () => {
  ...
  beforeEach(() => {
    ...
    process.env.TAIJI_AGENT_DATA_DIR = path.join(agentDir, "engine-data");
    clearEngines();
  ...
  // ============================================================
  // B-firstround：非 pi 分支走 Continuation 首轮（[modeless 波5] conversation
  // 派发参数已删——一切 start 等价，形态由 resume 键承载）
  // ============================================================

  it("[B-firstround] zcode 首轮经 startFirstRound：派发 zcode port、resume={recordId} 无锚、settle 后 markRoundIdle 收口（running-resumable + round+1），非 one-shot 终态化（idle+closedReason='gc'）", async () => {
  ...
  it("[B-routing/onHandleReady] 续轮新 sessionRef 覆写旧锚（cold 每轮换锚）——engineHandle.sessionRef 更新为新一轮 session 且落 entry", async () => {
    ...
    // 第三轮 resume 锚已推进到新 session（闭环：锚随轮自更新）——seed 新条目对齐
    // 真实形态（库条目在；zcode 锚预检查已退役，seed 与否不再影响派发分流）
  ...
  it("[防御] pi 缺省路由的会话轮：RunContext 不挂 onHandleReady（pi 锚面 = outcome.sessionFile 回填，不引入新回填通道）", async () => {
    const handle = await service.execute({ task: "pi chat", slug: "u6b-pi" });
