...
// 覆盖段（TAIJI_AGENT_DATA_DIR 优先）与 warn-once 语义留在 core data-dir.ts，壳不
...
export interface HostServices {
   ...
   *  二级）子项 = 候选引擎包目录，命中 package.json `taiji.subagentEngine` manifest
   *  即发现。打包态主通道是 env `TAIJI_AGENT_ENGINE_ROOTS`（W9 注入），此端口承载宿主
