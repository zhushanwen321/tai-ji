 ...
 *
 * - 确认态：单槽（useTwoStepConfirm 原语：首击置位、再击执行并复位）；clearAbortConfirm
 *   供行 mouseleave 防误触残留清理（tray 面板消费）。
...
export function useWorkflowAction(getSessionId: () => string | null | undefined) {
  ...
  /** 两段式确认原语（单槽确认态；首击置位、再击执行并复位） */
  const confirm = useTwoStepConfirm((runId) => {
  ...
  return {
    isAbortConfirming: confirm.isConfirming,
    onAbortClick: confirm.toggle,
    clearAbortConfirm: confirm.clear,
    abortWorkflow,
  }
}
