...
import { openDrawerTab, getDrawerControlState } from '@taiji/core/domain/drawer'
import type { ServerMessage, SessionTraceMalformedLine } from '@taiji/shared'
import type {
  TraceKindGroup,
  TraceSessionEndMeta,
  TraceSessionHeader,
} from '@taiji/core/domain/session-trace'
 ...
 *  广播的 taiji:current-system-prompt DATA 行（inspector 原始 JSON）承载全文，分区只存计数/时间）。 */
 ...
 * taiji:current-system-prompt DATA 行承载（增量腿自动追加，inspector 可看）。错误存 code，
