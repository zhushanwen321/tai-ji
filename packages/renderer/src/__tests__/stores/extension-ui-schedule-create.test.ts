 ...
 *
 * hasPendingAskUser 谓词扩义为「有 pending 富交互 overlay 请求」（askUser ∨ scheduleCreate），
...
describe('getter 扩义：hasPendingAskUser 覆盖 scheduleCreate（有 pending 阻塞 overlay）', () => {
  it('只有 scheduleCreate 请求 → hasPendingAskUser true（waiting/豁免消费方的数据源）', () => {
  ...
  it('askUser 与 scheduleCreate 双请求并存 → 恒 true；逐个移除后才 false', () => {
    ...
    store.removeRequest('sess-A', 'r1')
    expect(store.hasPendingAskUser('sess-A')).toBe(true)  // scheduleCreate 仍在等
...
describe('hasPendingDialog 对称判据：scheduleCreate 不归 dialog 类', () => {
  it('只有 scheduleCreate 请求 → hasPendingDialog false（与 hasPendingAskUser 不双真）', () => {
    const store = useExtensionUIStore()
    store.addRequest('sess-A', makeScheduleCreate({ requestId: 'r1' }))
