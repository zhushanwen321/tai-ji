...
vi.mock('@taiji/core/transport/api/domains/extension', async (importActual) => {
  const actual = await importActual<typeof import('@taiji/core/transport/api/domains/extension')>()
  return { ...actual, scan: mocks.extensionScan }
})
vi.mock('@taiji/core/transport/api/domains/workspace', async (importActual) => {
...
describe('useSidebar 接缝（TC-1..TC-4）', () => {
  ...
  it('TC-2 deleteSession 代理 core：api.remove 调 + S3 清理 + wasActive 回退 selectSession', async () => {
    const removeMock = (await import('@taiji/core/transport/api/domains/session')).remove as ReturnType<typeof vi.fn>
  ...
  it('TC-5 重连 onConnected 对聚焦 session 重拉 subagent/workflow（首连不拉；分区数据刷新用户可见）', async () => {
    const sessionDomain = await import('@taiji/core/transport/api/domains/session')
