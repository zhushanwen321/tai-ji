 ...
 *
 * 运行：pnpm --filter @taiji/frontend run test -- src/__tests__/panel/turn-working.test.ts
 */
...
describe('Turn · subagent 轮终 running-resumable → isWorkingTurn false（review #8）', () => {
  ...
  function makeSubagentRecord(overrides: Partial<import('@taiji/shared').SubagentRecord>) {
    return {
      subagentId: 'sub-1',
      sessionFile: null,
      ...
      status: 'running',
      ...overrides,
    } as import('@taiji/shared').SubagentRecord
  }
