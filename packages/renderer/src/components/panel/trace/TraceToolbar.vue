<template>
  <!--
    ...
    - 搜索：文本过滤（demo 行为：不匹配隐藏）。Input（本地 ui 原语）+ v-model 转发。
    - context toggle：本地 ui 原语 Switch（禁原生 HTML 表单元素）。
  -->
  ...
  <div class="flex flex-shrink-0 flex-col gap-1.5 pl-4 pr-2 pb-2 pt-1">
    ...
    >
      ...
      <!-- SYSTEM 无留痕降级（§3.1）：现取按钮 + 「当前值，非历史」标注。现取通道在常驻
           ...
           taiji:current-system-prompt DATA 行承载，点行 inspector 可看）。 -->
...
import { TRACE_KIND_GROUPS } from '@taiji/core/domain/session-trace'
import type { TraceKindGroup, TraceRow } from '@taiji/core/domain/session-trace'
