<!--
  ...
  累计退化；指标档把 容量 + 速度 + 缓存 三个数值触发器收为单图标按钮）。

  形态硬约束（S1–S7）：
  - 聚合按钮**单一图标**（Gauge），禁多 icon 重叠；
  - 指标聚合 = **hover** 弹指标聚合页（HoverCard）——与模型聚合的 click 语义区分；
  - 内容 = 三张纯内容卡纵向拼装（ContextCapacityCard + GenStatsSpeedCard + GenStatsCacheCard），
    卡间发丝分隔（token 工具类 h-px bg-border-strong 局部定义，不 import 接线层常量）；
    三卡共享 useGenStats / useContextUsage 分区，与原触发器同源收敛。
...
const props = defineProps<{
  /** 焦点 session id（三卡的分区键；接线方以 v-if 保证非空） */
  sessionId: string
