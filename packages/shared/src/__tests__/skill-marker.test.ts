...
describe('buildSkillMarker', () => {
  it('name + location 产出标记形态', () => {
    expect(buildSkillMarker('cw-cli', '/abs/path/SKILL.md')).toBe(
      '<taiji-skill name="cw-cli" location="/abs/path/SKILL.md"/>',
    )
  })

  it('location 缺省时不输出该属性', () => {
    expect(buildSkillMarker('cw-cli')).toBe('<taiji-skill name="cw-cli"/>')
  })
...
describe('parseSkillMarkers 往返无损', () => {
  ...
  it('残缺标记不解析（顺序错乱 / 非自闭合 / 未闭合引号 / 尾部截断）——破坏即透传（D8）', () => {
    expect(parseSkillMarkers('<taiji-skill location="/b.md" name="a"/>')).toHaveLength(0)
    expect(parseSkillMarkers('<taiji-skill name="a">')).toHaveLength(0)
    expect(parseSkillMarkers('<taiji-skill name="a" location="/b">')).toHaveLength(0)
    expect(parseSkillMarkers('<taiji-skill name="a location="/b"/>')).toHaveLength(0)
    expect(parseSkillMarkers('<taiji-skill name="a" loc')).toHaveLength(0)
...
describe('buildSkillsFallbackBlock（D7 降级块）', () => {
  it('单 skill：块形态 + 指引行精确断言', () => {
    ...
    expect(block).toBe(
      [
        '<taiji-skills>',
        '<taiji-skill name="cw-cli" location="/c/SKILL.md"/>',
        '</taiji-skills>',
  ...
  it('多 skill：每标记独立一行、location 缺省不输出属性', () => {
    ...
    expect(block).toBe(
      [
        '<taiji-skills>',
        '<taiji-skill name="a" location="/a.md"/>',
        '<taiji-skill name="b"/>',
        '</taiji-skills>',
        SKILL_FALLBACK_GUIDANCE,
      ].join('\n'),
  ...
  it('历史中文指引行（C5 前形态）：块本身仍可解析，中文指引行残留为正文（登记性锁定）', () => {
    ...
    // 残留含块尾与指引行之间的分隔换行（块区间止于 </taiji-skills>，可选指引行组不匹配中文）
...
describe('常量与 CJK 字符类定义', () => {
  ...
  it('标签名常量与构建/解析产物一致', () => {
    expect(SKILL_MARKER_TAG).toBe('taiji-skill')
    expect(SKILLS_BLOCK_TAG).toBe('taiji-skills')
  })
...
// ── R4 D11：末尾集中追加包裹块（`<taiji-skill-data>`）────────────────────────

describe('buildSkillDataBlockExpansions（R4 D11 正常形态包裹块）', () => {
  it('常量标签名为 taiji-skill-data（与 SKILL_MARKER_TAG/SKILLS_BLOCK_TAG 命名同族）', () => {
    expect(SKILL_DATA_BLOCK_TAG).toBe('taiji-skill-data')
  })
  ...
  it('空列表：仅包裹壳（join 语义；mapping_unavailable 时是否调用由 runtime 裁决）', () => {
    expect(buildSkillDataBlockExpansions([])).toBe('<taiji-skill-data>\n</taiji-skill-data>')
  })
...
describe('findSkillDataBlockRange（R4 D11 剥块定位切片）', () => {
  it('命中：区间为 slice 约定（[start, end)），切片即块全文，块前后正文无损', () => {
    ...
    const text = `帮我 review 这段代码 <taiji-skill name="a" location="/a.md"/>\n\n${block}`
    ...
    expect(text.slice(0, range.start)).toBe(
      '帮我 review 这段代码 <taiji-skill name="a" location="/a.md"/>\n\n',
    )
  ...
  it('未命中：纯文本 / 只有开标签未闭合 / 只有闭合标签 → null', () => {
    ...
    expect(findSkillDataBlockRange('<taiji-skill-data>\n<taiji-skill name="a"/>')).toBeNull()
    expect(findSkillDataBlockRange('正文 </taiji-skill-data> 尾')).toBeNull()
  })

  it('嵌套截断：非贪婪止于首个闭合标签（嵌套非生产形态，与 parseSkillsFallbackBlocks 同语义）', () => {
    const close = '</taiji-skill-data>'
