...
/**
 * W16 [D4]：subagent 自描述持久化 entry 的 customType（runtime 侧消费值）。
 *
 * 权威源在 extensions/universal/subagent-workflow（跨包依赖方向不允许 runtime
 * import extensions/ 源码，只能在此复制字面量并保持等值）：
 ...
 *
 * 消费方：event-adapter / session-records（entry_appended 失效过滤）、subagent-extractor
 * （entry 扫描的自描述分支）。extension 升级 customType 值时必须同步此处。
 * （workflow-record 的 customType 已收敛至 @zhushanwen/subagent-core 的
 * WORKFLOW_RECORD_CUSTOM_TYPE 单源——runtime 经 core barrel 消费，不经本文件。）
 */
...
/**
 * extension 加载路径预设候选（镜像 skill/agent 的两套：P1 pi 原生 + P2 taiji）。
 *
 * P1 pi 原生扫描目录：
 ...
 *
 * P2 taiji 强制目录结构：
 *   - .taiji/extensions（project 级；user 级 ~/.taiji/extensions 是强制目录不在此列）
 *
 ...
 *   a) 真实会话日志（~/.taiji/logs/pi-*.jsonl，2026-08-17 最繁忙 session）：
...
export const ENGINE_ENV_PREFIXES: readonly string[] = [
  ...
  // 越过 L0/L1 基础设施面自配 TAIJI_ 族变量，一律拒绝 + warn（引擎基础设施键只经 L0 注入）
  'TAIJI_', 'TAIJI_AGENT_', 'TAIJI_SUBAGENT_',
]
 ...
 * - NODE（引擎 node 执行器，与 relay 的 TAIJI_SUBAGENT_RELAY_NODE 不复用）：写入 =
...
export function readLogKeepDays(env: Record<string, string | undefined> = process.env): number {
  return Number(env.TAIJI_LOG_KEEP_DAYS) || DEFAULT_LOG_KEEP_DAYS
}
...
/**
 * 空闲回收判定周期（ms）。`TAIJI_RUNTIME_PI_RECLAIM_TICK_MS` env 覆盖，默认 5 分钟一拍（D4）。
 */
export const TAIJI_RUNTIME_PI_RECLAIM_TICK_MS = 'TAIJI_RUNTIME_PI_RECLAIM_TICK_MS'
/**
 * 空闲阈值（ms）。`TAIJI_RUNTIME_PI_RECLAIM_IDLE_MS` env 覆盖，默认 2 小时（D4：被否
 * 30min——回收/恢复抖动变常态；被否 24h——对午饭级离开太迟）。
 */
export const TAIJI_RUNTIME_PI_RECLAIM_IDLE_MS = 'TAIJI_RUNTIME_PI_RECLAIM_IDLE_MS'
/**
 * 查看豁免窗口（ms）。`TAIJI_RUNTIME_PI_RECLAIM_VIEWED_WINDOW_MS` env 覆盖，默认 30 分钟
