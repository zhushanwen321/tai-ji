...
export const SPAWN_ENV_OUTBOUND_DENY_LIST: readonly string[] = [
   ...
   * `TAIJI_AGENT_PACKAGED=1 bash scripts/validate-runtime-bundle.sh` → exit 1，
   ...
   * 打包版会话 bash 内 `printenv | grep TAIJI_RUNTIME_TOKEN` 直见 hex 令牌——任意
 ...
 * TAIJI_AGENT_PACKAGED / TAIJI_RUNTIME_TOKEN 正是 runtime 进程自身的合法输入（isPackaged()
 ...
 *
 * U0-② 结论（维持原判，LOG_LEVEL 族不入本清单）：TAIJI_LOG_LEVEL / TAIJI_LOG_MAX_BYTES /
 * TAIJI_LOG_KEEP_DAYS 在 extensions/ 全目录生产代码零命中（extension-logger 仅读
 * TAIJI_AGENT_DEBUG），消费者仅 runtime 进程自身（infra/logger.ts:53/:55/:119）——
 * 属 runtime 自身行为开关，无跨界消费证据。
 */
export const SPAWN_ENV_FORWARD_REFERENCE: readonly SpawnEnvForwardEntry[] = [
  {
    name: 'PI_CODING_AGENT_DIR',
    // 非 TAIJI_ 前缀，故不走白名单侥幸，靠 B3 显式追加兜底数据隔离根目录同源
  ...
  {
    name: 'TAIJI_AGENT_DEBUG',
    injectionPath: 'B3 白名单放行（裸 TAIJI_ 前缀）',
    piConsumerAnchors: [
      'extensions/shared/extension-logger/src/index.ts:342（fileLog gate：process.env.TAIJI_AGENT_DEBUG !== "1" 即 no-op）',
      'extensions/universal/subagent-workflow/src/execution/subagent-service.ts:392（debug 分支启用）',
    ],
  },
  {
    name: 'TAIJI_GLOBAL_AGENTS_DIR',
    injectionPath: 'B3 白名单放行（裸 TAIJI_ 前缀）',
    piConsumerAnchors: [
  ...
  {
    name: 'TAIJI_SUBAGENT_RELAY_SOCKET/_NODE/_SCRIPT',
    injectionPath:
  ...
  },
  {
    ...
    name: 'TAIJI_SUBAGENT_IDLE_TIMEOUT_MS',
    injectionPath: 'B3 白名单放行（裸 TAIJI_ 前缀）；extension 无 frame.env 自注入通路（U0① 核实）',
    piConsumerAnchors: [
