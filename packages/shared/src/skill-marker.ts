 ...
 *
 * 设计依据：docs/architecture/composer-multi-skill-injection.md §3.3：
 * - D3：skill segment 序列化产私有标记 `<taiji-skill name="..." location="..."/>`——
 ...
 *   R4 起降级块并入 D11 包裹块，旧 `<taiji-skills>` tag 退役但构建/解析保留以兼容存量落盘消息）
 * - D11（R4）：正文保留 `<taiji-skill/>` 占位标记 + 消息末尾集中追加 `<taiji-skill-data>`
...
/** `<taiji-skill/>` 单标记标签裸名（构建/解析/降级块三处共用，避免字符串漂移）。 */
export const SKILL_MARKER_TAG = 'taiji-skill'

/** `<taiji-skills>` 降级包裹块标签裸名（R4 起退役，仅存量落盘消息的构建/解析兼容面）。 */
export const SKILLS_BLOCK_TAG = 'taiji-skills'

/**
 * `<taiji-skill-data>` 末尾集中追加包裹块标签裸名（R4 D11）：正常形态（块内 pi 对齐
 * 展开的 `<skill>` 全文）与降级形态（块内 `<taiji-skill/>` 清单 + 指引行，原 `<taiji-skills>`
 * 统一并入）共用包裹。tag 名即新旧消息自识别锚点（存量 `<taiji-skills>` 与 R4 新块靠
 * tag 即可区分，D11 被否变体④的裁决依据），对模型传达「数据附挂区」语义。
 */
export const SKILL_DATA_BLOCK_TAG = 'taiji-skill-data'
...
/**
 * 全局解析文本中全部 `<taiji-skill/>` 标记（混排在正文中也命中），返回标记区间信息。
...
/**
 * 构建降级块（D7）：`<taiji-skills>` 包裹全部自闭合标记 + 紧跟块后一行的指引文案。
...
/**
 * 全局解析文本中全部 `<taiji-skills>` 降级块，块内标记复用 parseSkillMarkers 提取
...
/**
 * 构建正常形态包裹块（R4 D11）：`<taiji-skill-data>` 包裹全部已展开的 `<skill>` block，
...
/**
 * 构建降级形态包裹块（R4 D11，原 D7 降级块统一并入）：`<taiji-skill-data>` 包裹
 * `<taiji-skill/>` 标记清单 + 置于清单后的指引行，形态与设计 §3.1 场景 2 逐字一致。
...
/**
 * `<taiji-skill-data>` 块定位正则：非贪婪匹配首个完整块（开闭标签间任意内容含换行）。
...
/**
 * 定位文本中首个完整 `<taiji-skill-data>...</taiji-skill-data>` 块（供 core 反解析剥块，
