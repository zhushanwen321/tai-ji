 ...
 *   （序列化为 `<taiji-skill/>` 私有标记，见 segmentsToText；D3）
 ...
 *
 * skill → `<taiji-skill name=".." location=".."/>` 私有标记（D3，runtime 注入器只认该标记
...
const SEGMENT_SERIALIZERS: { [K in Segment['type']]: (seg: Extract<Segment, { type: K }>) => string } = {
  ...
  slash: (seg) => `/${seg.name}`,
  // D3：`<taiji-skill/>` 私有标记（location 可得时带上）。反解析对偶实现在 core parseSkillBlock。
  ...
  image: (seg) => {
    ...
    // 与 pi TUI（insertTextAtCursor 裸路径粘在光标处）的细微差异：taiji 让每个图片
 ...
 * 两形态含 taiji 私有标记与 pi 原生 block），不经此函数。
 */
