...
// 是两份（@taiji/shared spawn-env-contract 供 runtime；subagent-engine-sdk
...
export interface PiOutboundEnvOptions {
  ...
  /** taiji 托管 pi agent 目录（<dataDir>/agent/，pi-paths SSOT 推导后传入）。 */
  piAgentDir: string
}
 ...
 * TAIJI_AGENT_EXT_LOG 恒注入 '1'（托管环境 extension-logger 落盘 INFO 级日志，
...
export function buildPiOutboundEnv(opts: PiOutboundEnvOptions): NodeJS.ProcessEnv {
  ...
  // D4/G4 观测补齐：taiji 托管环境恒注入 extension 日志开关——extension-logger 见此
