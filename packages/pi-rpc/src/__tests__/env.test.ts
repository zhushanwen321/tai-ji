...
//
// pi 出站 env 组装测试：extras undefined 键跳过 / TAIJI_AGENT_EXT_LOG 恒注入（不可
...
describe('buildPiOutboundEnv', () => {
  ...
  it('TAIJI_AGENT_EXT_LOG 恒注入 "1"（托管日志开关；extras 同名键也被覆盖——不开放覆盖）', () => {
    const { fn } = makeFakeBuilder()
    const env = buildPiOutboundEnv({ parentEnv: {}, buildChildEnv: fn, piAgentDir: '/d' })
    expect(env.TAIJI_AGENT_EXT_LOG).toBe('1')

    const override = buildPiOutboundEnv({
      parentEnv: {},
      extras: { TAIJI_AGENT_EXT_LOG: '0' },
      buildChildEnv: fn,
      piAgentDir: '/d',
    })
    expect(override.TAIJI_AGENT_EXT_LOG).toBe('1')
  })
  ...
  it('底层构建器收到 parentEnv 与过滤后的 extras（白名单基座 + deny 兜底语义归构建器）', () => {
    ...
    expect(calls[0].extras).toEqual({ K: 'v', TAIJI_AGENT_EXT_LOG: '1' })
  })
})
