...
- `@taiji/runtime`（主 agent rpc-client 薄壳）
...
| `env` | `buildPiOutboundEnv`：extras 过滤 + `TAIJI_AGENT_EXT_LOG` 恒注入 + `PI_CODING_AGENT_DIR` 隔离；底层白名单构建器经 DI 注入 | runtime rpc-client buildPiOutboundEnv |
...
7. **`buildOutboundChildEnv` 底层双 SSOT**：`@taiji/shared`（runtime 消费）与 SDK（引擎 CLI 独立 npm 发布需自包含）各一份，本包不复刻第三份——经 `buildChildEnv` 参数注入。
