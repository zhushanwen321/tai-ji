...
//   - idle / ceiling 两形态文案有别（ceiling 附 TAIJI_ZCODE_TURN_MAX_TIMEOUT_MS env
//     自救通道——§5.2 F-2）；
...
describe("超时处置链（P0-1 U2：catch 分流 → stop-outcome 三态裁决 → engine_timeout）", () => {
  ...
  it("态③ stop 无应答（hang=控制面假死）：3s 超时判连接级失败 → killChain 升级 → 进程死亡 + 重建（boot 2）", async () => {
    vi.stubEnv("TAIJI_ZCODE_TURN_IDLE_TIMEOUT_MS", "300");
    // stop 超时注入缝（首轮 + U4 重试轮各经历一次 STOP_TIMEOUT）：真实默认 3s×2
    // 不在单测等待范围——200ms 级覆盖，fake 真子进程 IO 全保真。
    vi.stubEnv("TAIJI_ZCODE_TEST_STOP_TIMEOUT_MS", "200");
    ...
    // 40s 兜底保留：STOP_TIMEOUT 已经 env 注入缩短（实际 <2s），兜底只防 CI 抖动挂死
  }, 40_000);

  it("ceiling 形态（idle 关闭 + 总上界判死）：文案标 chatty-wedge + 附 TAIJI_ZCODE_TURN_MAX_TIMEOUT_MS env 自救通道（F-2）", async () => {
    vi.stubEnv("TAIJI_ZCODE_TURN_IDLE_TIMEOUT_MS", "0"); // 显式关闭 idle（规则 19 opt-out）
    vi.stubEnv("TAIJI_ZCODE_TURN_MAX_TIMEOUT_MS", "400");
    const f = makeEngine();
    const r = await f.engine.run(makeTask({ cwd: f.workspace }), makeCtx());
    expect(r.outcome.error).toMatch(/^engine_timeout:/);
    ...
    expect(r.outcome.error).toContain("TAIJI_ZCODE_TURN_MAX_TIMEOUT_MS");
