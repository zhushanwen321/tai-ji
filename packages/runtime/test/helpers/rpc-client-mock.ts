...
/** '@taiji/shared' mock 工厂（importActual spread，仅覆盖 ENV_WHITELIST_PREFIXES）。 */
export async function sharedModule() {
  const actual = await vi.importActual<typeof import('@taiji/shared')>('@taiji/shared')
  return { ...actual, ENV_WHITELIST_PREFIXES: ['PATH', 'HOME', 'USER', 'LANG', 'TERM'] }
}

/** '@taiji/shared/paths' mock 工厂（getDataDir 固定 /mock/home）。 */
export function sharedPathsModule() {
  return { getDataDir: () => '/mock/home/.taiji' }
}
...
export async function piPathsModule() {
  ...
  return {
    ...
    getConfigDir: () => process.env.TAIJI_AGENT_DATA_DIR ?? tmpdir(),
  }
}
