 ...
 * - 出站 env 契约（undefined extras 键跳过 / TAIJI_AGENT_EXT_LOG 恒注入 / PI_CODING_AGENT_DIR）
 *
...
describe('RpcClient start 启动参数锚定（W3 复杂度债务偿还）', () => {
  ...
  it('E1: env extras——undefined 键跳过不写、非 undefined 键整体覆盖出站、TAIJI_AGENT_EXT_LOG 恒为 1、PI_CODING_AGENT_DIR 在场', async () => {
    await startWith({
      cwd: '/project',
    ...
    expect(spawnEnv.TAIJI_AGENT_EXT_LOG).toBe('1')
    // pi agent 目录隔离
    expect(spawnEnv.PI_CODING_AGENT_DIR).toBe('/mock/home/.taiji/agent')
