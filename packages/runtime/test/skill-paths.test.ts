...
// Mock @taiji/shared barrel — provide constants needed by rpc-client + session-lifecycle
...
vi.mock('@taiji/shared', async (importOriginal) => ({
  ...(await importOriginal<typeof import('@taiji/shared')>()),
  ENV_WHITELIST_PREFIXES: ['PATH', 'HOME', 'USER', 'LANG', 'TERM', 'NODE_', 'NVM_', 'TAIJI_', 'XDG_', 'APPDATA', 'LOCALAPPDATA', 'PROGRAMFILES', 'SYSTEMROOT', 'TEMP', 'TMP'],
...
// Mock @taiji/shared/paths — getDataDir 被 pi-paths 子路径 import（Node-only，隔离于 barrel）
...
// timeout 30s：用例走真实 spawn 链路（process-manager 真实探测 pi binary + RpcClient
// start 自旋等待），单跑 0.5-1.2s，但 pnpm test 全量多 worker 高负载下默认 5s 余量不足——
// 曾 observed 用例超时判死后悬挂 promise 在后续用例期间产出 spawn，污染模块级
// spawnArgsCapture 导致连锁假失败（2026-09-15 全量终验）。
describe('skillPaths passing chain', { timeout: 30_000 }, () => {
  let savedPackaged: string | undefined

  beforeAll(() => {
    savedPackaged = process.env.TAIJI_AGENT_PACKAGED
    delete process.env.TAIJI_AGENT_PACKAGED
  })

  afterAll(() => {
    if (savedPackaged !== undefined) process.env.TAIJI_AGENT_PACKAGED = savedPackaged
  })
  ...
  it('restoreSession passes skillPaths to spawned process', async () => {
    ...
    mockScannedSessions.push({
      ...
      filePath: '/mock/home/.taiji/sessions/test.jsonl',
      cwd: '/project',
