...
vi.mock('../src/infra/pi/pi-paths.js', async (importOriginal) => {
  ...
  return {
    ...actual,
    getPiAgentDir: () => '/mock/taiji/agent',
  }
})
 ...
 *
 * 调用契约：vi.useFakeTimers() 须在触发动作（seedSession/switchModel/applyContextUpdate/
 * fetchAndBroadcastContext 等）之前安装——防抖/挂钩定时器必须挂 fake 时钟，advance 才能推进。
 * 推进 SCALAR_STATE_DEBOUNCE_MS + 50 覆盖整链：防抖到点 → fetch（client 全 mock，微任务
 * 即时落定）→ 挂钩 setTimeout 0 宏任务发布；随后还原真 timers。
 */
...
describe('SessionService · lifecycle', () => {
  ...
  describe('create', () => {
    ...
    it('queries commands and broadcasts session.commands on create', async () => {
      // fake timers 先于 seedSession 安装：激活发布的挂钩宏任务挂 fake 时钟（create 链全 mock，微任务即时落定）
      vi.useFakeTimers()
      const { id } = await setup.seedSession({ commands: [{ name: 'foo-navigate', source: 'extension' }] })
    ...
    it('broadcasts session.commands 含 sourceInfo 透传（W2）', async () => {
      // fake timers 先于 seedSession 安装（同上用例：激活挂钩挂 fake 时钟）
      vi.useFakeTimers()
      const { id } = await setup.seedSession({
        commands: [
          {
    ...
    it('falls back to homedir when requested cwd does not exist (INV-7)', async () => {
      const nonexistentCwd = '/tmp/taiji-test-cwd-nonexistent-' + Date.now()
  ...
  describe('renameSession', () => {
    ...
    it('non-active session：短命 pi 附着后 set_session_name RPC（W11：直写全删）', async () => {
      ...
      // 非活跃分支经短命 pi 附着目标文件后 RPC（taiji 不再直写 session JSONL）
...
describe('SessionService · Facade', () => {
  ...
  describe('switchModel（session 级状态单一 owner：RPC + 缓存 + 广播）', () => {
    ...
    it('切换后广播 session.state_changed（payload 来自 modelId/thinkingLevel 快照，W12；usage 走 context.update，D1）', async () => {
      ...
      // fake timers 安装在播种（真 timers 的 setTimeout 0 flush）之后、动作之前：
      // switchModel 的失效防抖挂 fake 时钟，waitForSnapshotPublish advance 推进收敛
      vi.useFakeTimers()
  ...
  describe('applyContextUpdate（session 级状态单一 owner：W12 事件只失效，发布归快照挂钩）', () => {
    it('失效收敛后广播 context.update（payload 全字段来自 usage 快照），不直写快照', async () => {
      ...
      //（fake timers 安装在动作前：失效防抖挂 fake 时钟）
      vi.useFakeTimers()
  ...
  describe('fetchAndBroadcastContext（session 恢复后推送用量）', () => {
    it('contextUsage.tokens 有值 → 查询失效收敛后经快照挂钩广播 context.update（W12）', async () => {
      ...
      // fake timers 安装在动作前：查询失效的防抖挂 fake 时钟
      vi.useFakeTimers()
...
// dataDir 由 vitest globalSetup 的 TAIJI_AGENT_DATA_DIR 指向 tmp 目录，不会污染用户数据。
describe('SessionService · 业务持久化写安全守卫（W2 ipc-converge-a3 移植）', () => {
  ...
  describe('migrateImage（原 migrate-session-image IPC handler）', () => {
    ...
    it('B1: fromPath 在白名单外（home 目录）→ throw，不 move 文件（防任意文件移动）', async () => {
      ...
      // （tmpdir ∪ attachments）之外」——差集 = dev 数据目录 ~/.taiji-dev（fs-guard
      // 白名单成员，不在 tmpdir 前缀内）。原 home 路径在 fs-guard 下不可写。
      const guardWritableDir = join(homedir(), '.taiji-dev')
      mkdirSync(guardWritableDir, { recursive: true })
      const evilFile = join(guardWritableDir, '.taiji-test-evil-' + Date.now() + '.txt')
    ...
    it('B1: fileName 含路径分隔符 → sanitize 剥离，newPath 落在 attachments/<sid>/ 下不穿越', async () => {
      ...
      const fromPath = join(tmpdir(), 'taiji-test-migrate-' + Date.now() + '.png')
    ...
    it('B1: sessionId 含 ../ → throw（getAttachmentsDir 校验防路径穿越）', async () => {
      ...
      const fromPath = join(tmpdir(), 'taiji-test-migrate-sid-' + Date.now() + '.png')
