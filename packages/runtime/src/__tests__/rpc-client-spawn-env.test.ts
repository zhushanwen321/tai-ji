 ...
 * 子进程 env 必须满足——deny 清单两键（TAIJI_AGENT_PACKAGED / TAIJI_RUNTIME_TOKEN）无论从
...
describe('RpcClient 出站 env 契约（U3：deny 剥除 + 基座保全）', () => {
  ...
  it('T1: 污染父 env → spawn env 不含 deny 两键，PATH/HOME 基座与 PI_CODING_AGENT_DIR 必备键仍在', async () => {
    ...
    vi.stubEnv('TAIJI_RUNTIME_TOKEN', 'deadbeef-hex')
    vi.stubEnv('PATH', '/usr/bin:/bin')
    vi.stubEnv('HOME', '/mock/home')

    await startWith({ env: { TAIJI_AGENT_DATA_DIR: '/mock/home/.taiji' } })

    expect(capturedEnv).not.toBeNull()
    const env = capturedEnv!
    // deny 两键：无论从哪条通路进入，出站契约兜底剥除（本案核心修复点）
    expect(env).not.toHaveProperty('TAIJI_AGENT_PACKAGED')
    expect(env).not.toHaveProperty('TAIJI_RUNTIME_TOKEN')
    // R2：白名单过滤后的父 env 基座保全（整体替换语义下 PATH/HOME 丢失即远距离爆炸）
    expect(env.PATH).toBe('/usr/bin:/bin')
    expect(env.HOME).toBe('/mock/home')
    // 必备注入键：pi 子树数据隔离根目录
    expect(env.PI_CODING_AGENT_DIR).toBe('/mock/home/.taiji/agent')
    expect(env.TAIJI_AGENT_DATA_DIR).toBe('/mock/home/.taiji')
  })
  ...
  it('T2: 非 deny 的白名单内产品变量仍放行（收口只剥 deny 清单，不做全量掐断）', async () => {
    ...
    expect(capturedEnv!.TAIJI_AGENT_DEBUG).toBe('1')
  })

  it('T3: options.env override 覆盖白名单继承值（extras 在基座之上覆盖的既有优先级）', async () => {
    vi.stubEnv('TAIJI_AGENT_DEBUG', '1')

    await startWith({ env: { TAIJI_AGENT_DEBUG: '0' } })

    expect(capturedEnv!.TAIJI_AGENT_DEBUG).toBe('0')
  })

  it('T4: 经 options.env 强塞的 deny 键同样被剥除（deny 兜底位于 extras 之后，不可被调用方复活）', async () => {
    await startWith({ env: { TAIJI_RUNTIME_TOKEN: 'forged-token' } })

    expect(capturedEnv).not.toBeNull()
    expect(capturedEnv).not.toHaveProperty('TAIJI_RUNTIME_TOKEN')
  })
  ...
  it('T5: options.env 含 undefined 键时跳过不删（锁定旧实现行为，防吞掉白名单基座 — R2）', async () => {
    ...
    await startWith({
      ...
      env: { TAIJI_AGENT_DEBUG: undefined, PATH: undefined } as unknown as Record<string, string>,
    })
