 ...
 *   TAIJI_ROLLING_RESTART_DEFER_LIMIT_MS（默认 30min）到点强制执行 reason=defer-limit；
 *   双维硬升级 = heap ≥ TAIJI_RUNTIME_WATCHDOG_FORCE_PCT（默认 92%）或 memPressure 越限
 ...
 * TAIJI_RUNTIME_WATCHDOG_ARMED 解析传入（与 watchdog 同一 resolveWatchdogConfig 结果）。
 *
...
export const ENV_ROLLING_RESTART_DEFER_LIMIT_MS = 'TAIJI_ROLLING_RESTART_DEFER_LIMIT_MS'

/**
 * env 旋钮：硬升级 heap 阈值 %（设计 §5 `TAIJI_RUNTIME_WATCHDOG_FORCE_PCT`，D5 双维硬
 * 升级第一维）。u6 watchdog.ts 明确「本单元不解析不消费」——常量与解析都归本模块。
 */
export const ENV_WATCHDOG_FORCE_PCT = 'TAIJI_RUNTIME_WATCHDOG_FORCE_PCT'
...
export interface RollingRestartOptions {
  ...
  /** 推迟上限 ms（默认 30min；env TAIJI_ROLLING_RESTART_DEFER_LIMIT_MS）。 */
  deferLimitMs?: number
  /** 推迟期重判间隔 ms（默认 30s）。 */
  deferRetryMs?: number
  /** T-30s 预告时长 ms（默认 30s）。 */
  countdownMs?: number
  /** 硬升级 heap 阈值 %（默认 92；env TAIJI_RUNTIME_WATCHDOG_FORCE_PCT）。 */
  forcePercent?: number
