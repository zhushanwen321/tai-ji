 ...
 * 「路径传递」：runtime 推 resources/ 位置 → env TAIJI_AGENT_ENGINE_ROOTS 注入 pi 子进程
 ...
 *
 * 注入面（pi 子进程 env）：
 *  - TAIJI_AGENT_ENGINE_ROOTS：staged 引擎根绝对路径（目录存在才注入——dev 未跑
 ...
 *  - TAIJI_AGENT_ENGINE_NODE + ELECTRON_RUN_AS_NODE=1：仅打包态注入（矩阵①：pi 扩展
...
import { ENGINE_LAUNCH_ENV_KEYS } from '@taiji/shared'
