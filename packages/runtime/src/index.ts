...
import { TaijiProviderStore } from './services/provider-extras-store.js'
...
/**
 * 解析 WS auth token（S1-W1，spec §3.3 D4）：优先 env TAIJI_RUNTIME_TOKEN（Electron 主进程
...
function resolveRuntimeToken(): string | null {
  const envToken = process.env.TAIJI_RUNTIME_TOKEN
  ...
  console.warn('[runtime] WS auth token unavailable (TAIJI_RUNTIME_TOKEN env / <dataDir>/runtime-token both missing) — fail-closed: ALL WebSocket connections will be rejected')
  return null
}
...
async function main(): Promise<void> {
  ...
  const providerExtrasStore = new TaijiProviderStore(getProviderExtrasPath())
  ...
  const pluginService = new PluginService(pluginRegistry, server, {
    ...
    trashFile: trash,
    broadcastFn: (type, payload) => server.broadcast({ type: type as 'config.sessions', id: `push_${Date.now()}`, payload } as import('@taiji/shared').ServerMessage),
  })
  ...
  // armed 门与 watchdog 共用同一 resolveWatchdogConfig 结果（TAIJI_RUNTIME_WATCHDOG_ARMED
  ...
  const rollingRestartHandleLocal = startRollingRestart({
    ...
    broadcast: (type, payload) => server.broadcast({ type, payload } as import('@taiji/shared').ServerMessage),
  })
  ...
  // （TAIJI_RUNTIME_WATCHDOG_ARMED）——off 时纯观测：采样环照跑补全水位数据，relief 与
