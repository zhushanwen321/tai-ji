 ...
 * §3.2-D4：D4 表两行——TAIJI_AGENT_EXT_LOG 注入 + pi 崩溃 stderr 全量落盘）。
 *
 * 锁定：
 * - B（U3-3）：spawn env 经 buildOutboundChildEnv extras 通道恒注入 TAIJI_AGENT_EXT_LOG=1
 *   （taiji 托管环境 extension 日志 INFO 档的开关；deny 剥除语义不受影响）。
...
describe('RpcClient spawn env：TAIJI_AGENT_EXT_LOG 注入（U3-3）', () => {
  beforeEach(() => {
    capturedEnv = null
  ...
  it('spawn env 恒含 TAIJI_AGENT_EXT_LOG=1（extras 通道出站）', async () => {
    await startClient()
    expect(capturedEnv).not.toBeNull()
    expect(capturedEnv!.TAIJI_AGENT_EXT_LOG).toBe('1')
  })

  it('必备注入键不受影响：PI_CODING_AGENT_DIR 在场，deny 键仍被剥除', async () => {
    vi.stubEnv('TAIJI_RUNTIME_TOKEN', 'leaked-token')
    try {
      await startClient()
      expect(capturedEnv!.PI_CODING_AGENT_DIR).toBe('/mock/home/.taiji/agent')
      expect(capturedEnv!.TAIJI_AGENT_EXT_LOG).toBe('1')
