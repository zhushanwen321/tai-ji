...
export class ProcessManager implements IProcessManager {
  ...
  async createSession(sessionId: string, cwd: string, options?: RpcClientOptions): Promise<RpcClient> {
    ...
    // TAIJI_AGENT_DATA_DIR 显式注入：pi 主会话进程内的 extension（subagent-workflow
    // engine 层 journal/隔离池）需要与 runtime 同源的 dataDir。dev 模式下 Electron
    // 已设置该 env、经 TAIJI_ 白名单透传成立；prod 默认路径（~/.taiji）下 runtime
    ...
    // + 打包态执行器 TAIJI_AGENT_ENGINE_NODE/ELECTRON_RUN_AS_NODE（pi 扩展宿主的
    ...
    const injectionEnv: Record<string, string> = {
      TAIJI_AGENT_DATA_DIR: getConfigDir(),
      ...pathEnv,
      ...relayEnv,
      ...engineRootsEnv,
    }
