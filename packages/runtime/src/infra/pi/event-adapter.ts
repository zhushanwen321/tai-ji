...
function translateSetWidgetRequest(event: PiExtensionUiRequestEvent, sid: string): PiTranslatedEvent[] {
  ...
  // marker 命中但校验/解析失败的行包含 NUL + marker 前缀（\x00TAIJI_GUI_WIDGET:...），
 ...
 * per-session bash 待落列（message-dispatcher.flushPendingBashResults），taiji live
...
/**
 * 诊断日志开关：`TAIJI_DEBUG_PI_EVENTS=1` 时打印每个 pi 原始事件。
 *
 ...
 *
 * 默认关闭，生产无噪声。复现步骤：dev 模式启动前 `export TAIJI_DEBUG_PI_EVENTS=1`，
