 ...
 *
 * entry 类型全集 = pi 官方 10 种 + taiji 自定义（handoff_marker JSONL 行 / session_end sidecar）。
...
/** extension 纯数据 entry（不进 LLM context）。SYSTEM 行是 customType='taiji:system-prompt' 的特判。 */
...
/** taiji runtime 直接 append 的交接标记（session-file-utils.ts persistHandedOff；无 id/parentId，不参与链——类型上可选以共享 entry 基形状，真实数据恒缺）。 */
