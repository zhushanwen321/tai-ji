 ...
 *
 * 本文件承载 message entry 体 → taiji Message 的转换链（content parts 解析 / skill block
 ...
 * '@taiji/shared' 与群内文件，不 import core 内群外模块（防 vue 依赖渗入 runtime bundle）。
...
/**
 * [R4 D7 三形态①] 剥除文本中全部 `<taiji-skill-data>...</taiji-skill-data>` 包裹块
 ...
 * 与 pi block 后扫描，起点落入已占用区间的命中跳过。pi block 与两类 taiji 标记的正则
 * 前缀互不重叠（`<skill` vs `<taiji-skill`），互不误命中；未闭合的 `<taiji-skill-data>`
...
function parseSkillBlock(text: string): Segment[] | null {
  ...
  // ②' taiji 单标记（降级块内嵌标记已被上方消费，区间内跳过；`<taiji-skill-data>` 块内
  // 标记已在 ① 随块丢弃，与块区间互斥）
...
interface CollectedContent {
   ...
   * ——pi-ai types.d.ts UserMessage；taiji 发送路径走 segments 路径模式不经此形态，但
...
/**
 * 转换单条 message entry 体为 taiji Message（user/assistant）。
...
export function convertMessageBody(
  ...
  // content 统一为 Segment[]：命中 taiji 私有标记 / pi 原生 block 时产出交错的
