...
// 方向的归属地。类型定义与注释见 @taiji/shared/pi-entry.ts。
//
...
// session_info 三个 entry 类型，taiji 未建模——reducer 对未建模 type 走 default no-op
...
export interface ChatViewState {
   ...
   * toolResult 双发两个事件、taiji 两条帧各喂本 reducer 一次（见 commitToolResultMessage
 ...
 * taiji 统一隐藏——与 mapSessionEntries 引用同一 SSOT，覆写幂等）。
 */
...
function commitToolResultMessage(c: ChatStateCollector, body: PiMessageBody): void {
  ...
  // {role:'toolResult'} 两个事件，taiji 翻译为 message.tool_call_end / message.message_end
