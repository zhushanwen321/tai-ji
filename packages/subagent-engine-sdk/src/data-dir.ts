...
//     TAIJI_ 前缀白名单变量（调查当时经私有 buildSafeEnv，现已被 buildPiOutboundEnv
//     → buildOutboundChildEnv 共享构建器取代），且显式设置 PI_CODING_AGENT_DIR；
//   - dev 模式 Electron main 设置 TAIJI_AGENT_DATA_DIR（apps/electron/main/main.ts:122），
//     经 process-control.ts:262 透传给 runtime → 透传链成立；
//   - prod 默认路径（~/.taiji 缺省）下 runtime 进程 env 中**没有**该变量
...
/** taiji 数据目录 env 名（与 packages/shared/src/paths.ts 的 SSOT 变量同名）。 */
export const TAIJI_DATA_DIR_ENV = "TAIJI_AGENT_DATA_DIR";
 ...
 *   1. env[TAIJI_AGENT_DATA_DIR]（宿主注入，权威通道）；
...
export function resolveEngineDataDir(
  ...
  const fromEnv = env[TAIJI_DATA_DIR_ENV]?.trim();
  if (fromEnv !== undefined && fromEnv !== "") return fromEnv;

  const fallback = opts.fallbackDataRoot?.trim();
  if (fallback === undefined || fallback === "") {
    throw new Error(
      `[subagent-engine-sdk] engine_not_found: ${TAIJI_DATA_DIR_ENV} is not set and no fallback data root was injected. ` +
        `Recovery: the host must inject ${TAIJI_DATA_DIR_ENV} into the engine process env ` +
        `(taiji runtime / pi extension host do this at spawn), or pass ` +
  ...
  if (!getWarnedSlot().current) {
    ...
    warn(
      `[engine-data-dir] ${TAIJI_DATA_DIR_ENV} is not set; engine journal/pool fall back to the injected data root ` +
        `(${fallback}). The taiji host normally injects this env — if you are running inside taiji, ` +
