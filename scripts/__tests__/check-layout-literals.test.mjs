...
describe('R1 路径形态（P2）', () => {
  it('数据目录上下文的 pi/agent、pi/sessions 命中', () => {
    expect(isLayoutLiteralLine('~/.taiji/pi/agent/settings.json')).toBe(true)
    expect(isLayoutLiteralLine('<dataDir>/pi/agent 已由迁移脚本退役')).toBe(true)
    expect(isLayoutLiteralLine('`/Users/u/.taiji/pi/sessions/x.jsonl`')).toBe(true)
  ...
  it('双引号与模板串同样命中（词法无关引号形态）', () => {
    ...
    expect(isLayoutLiteralLine('expect(x).toBe(`/h/.taiji/pi/agent`)')).toBe(true)
  })
})
...
describe('R3 .pi 前缀排除（已知合法引用形态，settings-data.ts:72,75 / resource-discovery.ts:13,568 / paths.ts:46 / pi-maintenance.ts:141 同族）', () => {
  ...
  it('R3 防线整体：真实仓内已知合法引用文件逐行零命中（守护 settings-data/resource-discovery 同族形态）', () => {
    const samples = [
      ...
      'subagents/…（~/.pi/agent/subagents/、~/.taiji/agent/subagents/）',
    ]
...
describe('真实仓抽查（改写后基线）', () => {
  ...
  it('data-source-registry.md 入域且当前零命中；§6 主键旧字面量回流必被拦（F2 补录裁决）', () => {
    ...
    expect(
      scanFile(rel, '| `~/.taiji/pi/agent/settings.json` | pi 子进程 + taiji runtime | ... |'),
    ).toHaveLength(1)
    expect(scanFile(rel, '数据源主键 ~/.taiji/pi/agent/auth.json 登记行')).toHaveLength(1)
  })
})
