...
describe('checkDependencyClosure（检查项 4：workspace 依赖发布闭包）', () => {
  ...
  const PUBLISHED = new Set(['@fixture/lib'])
  const PROBE = (root) => checkDependencyClosure(join(root, 'packages', 'consumer'), WS(root), new Set())

  it('依赖包登记进已发布清单 → 绿（registry 可解析静态信号）', () => {
    const root = consumerRepo({ dependencies: { '@fixture/lib': 'workspace:*' } })
    try {
      expect(
        checkDependencyClosure(join(root, 'packages', 'consumer'), WS(root), new Set(), PUBLISHED),
      ).toEqual([])
    } finally {
      rmSync(root, { recursive: true, force: true, maxRetries: 5, retryDelay: 20 })
    }
  })
  it('依赖包未登记清单但有 pending changeset → 绿（本次发布闭包）', () => {
    const root = consumerRepo({ dependencies: { '@fixture/lib': 'workspace:*' } })
    try {
  ...
  it('两信号皆无 → 红（E404 定性 + 补 changeset 指引 + 清单登记指引）', () => {
    const root = consumerRepo({ dependencies: { '@fixture/lib': 'workspace:*' } })
    try {
      const problems = PROBE(root)
      expect(problems.length).toBe(1)
      expect(problems[0]).toContain('"@fixture/lib" 未登记进 check-publish-surface.mjs 的 PUBLISHED_REGISTRY_PACKAGES')
      expect(problems[0]).toContain('E404')
      expect(problems[0]).toContain('为 @fixture/lib 新增 changeset')
      expect(problems[0]).toContain('PUBLISHED_REGISTRY_PACKAGES 清单')
    } finally {
      rmSync(root, { recursive: true, force: true, maxRetries: 5, retryDelay: 20 })
    }
  })
  it('依赖 private 包 → 红（永不发布，指引内联 + 移除声明）', () => {
    ...
    try {
      ...
      // 双绿信号俱全（已登记清单 + pending）仍红——private 优先短路
  ...
  it('外部依赖（不在 workspace 索引）不纳入；peerDependencies 与 dependencies 同面检查', () => {
    ...
    try {
      ...
      const problems = PROBE(root)
      expect(problems.length).toBe(1)
      expect(problems[0]).toContain('"@fixture/lib" 未登记进')
    } finally {
      rmSync(root, { recursive: true, force: true, maxRetries: 5, retryDelay: 20 })
    }
  })
...
describe('runGuard（rootDir 注入全链路）', () => {
  ...
  it('检查项 4 全链路：依赖包未发布且无 pending changeset → 红；补 changeset 文件后绿', () => {
    ...
    try {
      ...
      expect(red.failures[0]).toContain('"@fixture/lib" 未登记进')
