 ...
 *    「registry 可解析」静态信号 = X 登记在 PUBLISHED_REGISTRY_PACKAGES（首次
 *    publish 后人工登记）∨ X 有 pending changeset（本次发布会发布）。两信号
 *    皆无 → 红：publish 替换 workspace:* 为 ^<version> 后 registry E404，
 *    依赖方 tarball 装不上（npm install 阶段，晚于检查项 1-3）。
...
/**
 * 已发布到 npm registry 的 workspace 包清单（「registry 可解析」静态信号，检查项 4）。
 * 历史：该信号曾用 X/CHANGELOG.md 存在性（changeset version 与 publish 同链产物）；
 * 2026-09-15 全仓 CHANGELOG 清除后改用显式清单——零网络、fail-closed 不变。
 * 维护纪律：新包首次 publish 完成后把包名登记进本清单（merge skill 发布阶段核对项）；
 * 清单漏登只产生假红（按指引登记即绿），清单错登（未发布却列入）才是假绿——禁预登。
 * 核对命令（发布窗口用）：for d in extensions/{taiji,universal,shared}/* packages/*; do
 ...
 * 满足「registry 可解析」二选一：X 登记在 PUBLISHED_REGISTRY_PACKAGES（首次
 * publish 后人工登记——已发布 → registry 可解析）∨ X 有 pending changeset
 * （本次发布会发布）。两信号皆无 → 红。
 *
 * 信号误报面（注释备案）：假绿仅限「清单错登未发布包」——禁预登纪律兜底；
 * 假红方向安全（X 已发布但清单漏登 → 红 → 按指引核对 npm 后登记即绿）。
 * devDependencies 不进 tarball 依赖树不查；外部依赖（ajv 等）registry 存在性
 * 零网络不可验，不在成员索引即跳过（与本脚本零第三方依赖取向一致）。
 *
 * @param {string} pkgDir 依赖方包目录
 * @param {Map<string, {dir: string, private: boolean}>} workspaceByName
 * @param {Set<string>} pendingPkgs scanPendingChangesetPackages 结果
 * @param {Set<string>} publishedPkgs 已发布包清单（默认 PUBLISHED_REGISTRY_PACKAGES；测试注入 fixture 集合）
 */
function checkDependencyClosure(pkgDir, workspaceByName, pendingPkgs, publishedPkgs = PUBLISHED_REGISTRY_PACKAGES) {
  const problems = []
  ...
  for (const name of Object.keys(deps)) {
    ...
    if (publishedPkgs.has(name) || pendingPkgs.has(name)) continue
    problems.push(
      [
        `dependencies 中的 workspace 包 "${name}" 未登记进 check-publish-surface.mjs 的 PUBLISHED_REGISTRY_PACKAGES（未发布）也无 pending changeset（本次不会发布）`,
        ...
        `  若 ${name} 实际已发布（npm view 可查到版本），则把它登记进 PUBLISHED_REGISTRY_PACKAGES 清单`,
      ].join('\n'),
    )
  }
  return problems
