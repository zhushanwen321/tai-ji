...
//   PI_PROBE_FAUX=1 启用 faux LLM 通道（凭证无关：主/子 pi 全走 faux 演员，派发与
//   成员行为脚本化；faux 下快成员毫秒级轮终，总时长缩到 ~1-2 分钟，sleep 240 仍真实执行）
//   含 240s sleep + 三次进程生命周期，长时场景——真实 LLM 轨总时长约 5-8 分钟。
...
function spawnRestart(ws, sessionFile, label) {
  return C.spawnSession({
    ...
    label,
    fauxScriptPath: ws.fauxScriptPath,
  });
}
...
/** 等 n 个 sync 成员在主 session 文件出现轮终末条（result 非空——SP-5 one-shot 成员
 *  完成形态 doFinalizeRoundToIdle；.finalized sidecar 只在完整终态化路径写，sync 轮终
 *  不写，故不能用 countFinalized 观察完成。resumable 字段已随 [U5/D4] 从 record/entry
 *  契约退役（idle 即 resumable——旧口径 `d.resumable === true` 在现行契约下恒
 *  false，2026-09-15 faux 翻轨实测定位并改为 result+idle 判定）。 */
async function waitForRoundTerminal(F, ids, n, timeoutMs) {
  ...
  const doneOf = () => {
    ...
    return ids.filter((id) => {
      const d = last.get(id);
      return d && typeof d.result === "string" && d.result.length > 0;
    });
  };
  for (;;) {
    const done = doneOf();
    ...
    if (Date.now() - start > timeoutMs) {
      const last = lastRecordEntryById(F);
      const states = ids.map((id) => `${id.slice(0, 10)}:result=${JSON.stringify(String(last.get(id)?.result ?? "")).slice(0, 24)},status=${last.get(id)?.status}`).join(" ");
...
async function runProbe(checks, ws, sessions) {
  const starts = [
    ...FAST_RESULTS.map((r, i) => ({
      ...
      slug: `v2-fast-${i + 1}`,
      collect: "sync",
      ...(C.isFauxMode() ? { model: C.FAUX_CHILD_MODELS[i] } : {}),
    })),
    {
      task:
      ...
      slug: "v2-slow",
      collect: "sync",
      ...(C.isFauxMode() ? { model: C.FAUX_CHILD_MODELS[2] } : {}),
    },
  ];
  // faux 通道：model-keyed 响应脚本——主 pi（派发 + 定局文本）与三类子进程（快1/快2/慢）
  // 各自按 --model 选队；子进程经 argv-mirror（--extension）+ env 继承（TAIJI_FAUX_SCRIPT）
  // 透传装配（spawn-runner buildSpawnArgs/buildChildEnv）
  if (C.isFauxMode()) {
    ws.writeFauxScript({
      [C.FAUX_PARENT_MODEL]: [C.fauxDispatchStep(starts), { text: "Dispatched." }],
      [C.FAUX_CHILD_MODELS[0]]: [{ text: FAST_RESULTS[0] }],
      [C.FAUX_CHILD_MODELS[1]]: [{ text: FAST_RESULTS[1] }],
      [C.FAUX_CHILD_MODELS[2]]: [
        { toolCalls: [{ name: "bash", args: { command: `sleep ${SLOW_SLEEP_SECONDS} && echo marker-v2-slow` } }] },
        { text: "v2-slow-done" },
      ],
    });
  }
  ...
  //  形态——不写 .finalized sidecar），慢成员 sleep 中。真实 LLM 轨（mimo 3 并发）快任务
  //  实测可达 2-4 分钟；faux 轨快成员毫秒级轮终（等待 300s 上限不变），慢任务
  //  sleep 240s 保 kill 时点仍在批等待中。──
