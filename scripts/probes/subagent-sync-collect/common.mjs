...
//
// ── faux LLM 通道（L2.5，2026-09-15）────────────────────────────────────────
// `PI_PROBE_FAUX=1` 启用：真 pi 进程 + subagent-workflow/session-reader 真链路 + 假 LLM
//（pi-ai faux provider，经 --extension 注入 runtime 测试 fixture faux-llm-ext.ts）——
// 开发机零凭证可跑（探针仍按需手动触发，不进 CI）。
//   - 主 pi model = faux/faux-1-reasoning（PI_PROBE_MODEL 在 faux 通道被忽略——红线：
//     faux 通道禁真实模型/零 token）；
//   - subagent 子进程的派发经脚本化 toolCall（model 覆盖到 faux-1 / faux-1-b / faux-1-c
//     槽位演员），模型注入经扩展 spawn 透传（argv-mirror 镜像 --extension +
//     buildSpawnArgs 恒拼 --model）；
//   - 响应脚本：TAIJI_FAUX_SCRIPT 指向 model-keyed JSON（{ "faux/faux-1-reasoning": [主
//     pi 步骤], "faux/faux-1-b": [子进程步骤], ... }），env 经 buildChildEnv（parentEnv =
//     主 pi process.env）透传到全部子进程，各自按 --model 选队（faux-llm-ext loadScript）。
...
// [R1 拆分] faux 响应脚本写入（writeFileSync）经独立模块，本文件含 sessions 路径
// 推导痕迹，pre-commit 启发式会保守命中（先例：results-log.mjs 之于 RESULTS.md）。
import { writeFauxScriptFile } from "./faux-script.mjs";
...
// ── faux LLM 通道（PI_PROBE_FAUX=1；文件头「faux LLM 通道」节）──────────────

/** faux provider 注册 extension（runtime 测试 fixture；与 vitest faux 轨同一文件）。 */
export const FAUX_EXTENSION = join(REPO_ROOT, "packages", "runtime", "src", "__tests__", "fixtures", "faux-llm-ext.ts");
/** faux 主 pi 演员（model-keyed 脚本的主队列键；subagent 子进程用槽位演员区分）。 */
export const FAUX_PARENT_MODEL = "faux/faux-1-reasoning";
/** subagent 子进程槽位演员（派发时 model 覆盖；行为由脚本步骤定义，模型本身无语义）。 */
export const FAUX_CHILD_MODELS = ["faux/faux-1", "faux/faux-1-b", "faux/faux-1-c"];

/** faux 通道开关（env；PI_PROBE_MODEL 在此通道被忽略——禁真实模型红线）。 */
export function isFauxMode() {
  return process.env.PI_PROBE_FAUX === "1";
}
...
export function resolveModel() {
  // faux 通道恒 faux 主演员（PI_PROBE_MODEL 忽略：凭证无关红线，误配真实模型即事故）
  if (isFauxMode()) return FAUX_PARENT_MODEL;
...
export function makeWorkspace(label, opts = {}) {
  ...
  // faux 通道（L2.5）恒隔离且不拷凭证（凭证无关红线）：extensions/ 预置 faux-llm-ext——
  // 协议化引擎（独立进程）的 argv-mirror 读的是引擎自身 argv（空 flag 集），subagent
  // 孙进程拿不到主 pi 的显式 --extension，改经 agentDir extensions/ 自动发现装载 faux
  // provider（pi rpc 无 UI 下 bash toolCall 无需 --approve 即执行，2026-09-15 实测）。
  // 同时注入 TAIJI_AGENT_DATA_DIR（协议化引擎 run 的数据根硬前置，缺失即
  // engine_not_found: cannot resolve the engine data root）。
  let agentDir = null;
  let savedAgentDirEnv;
  let savedDataDirEnv;
  const isolate = opts.isolatedAgentDir || isFauxMode();
  if (isolate) {
    agentDir = mkdtempSync(join(tmpdir(), `pi-sync-collect-${label}-agent-`));
    if (isFauxMode()) {
      mkdirSync(join(agentDir, "extensions"), { recursive: true });
      copyFileSync(FAUX_EXTENSION, join(agentDir, "extensions", "faux-llm-ext.ts"));
  ...
  if (isFauxMode()) {
    savedDataDirEnv = process.env.TAIJI_AGENT_DATA_DIR;
    process.env.TAIJI_AGENT_DATA_DIR = join(root, "engine-data");
    mkdirSync(process.env.TAIJI_AGENT_DATA_DIR, { recursive: true });
  }
  return {
    root,
    cwd,
    ...
    /** faux 通道响应脚本路径（场景先 writeFauxScript 落内容，再传给每个 spawnSession）。 */
    fauxScriptPath: join(root, "faux-script.json"),
    /** 写 model-keyed 响应脚本（对象 { "provider/id": steps[] }；faux-llm-ext loadScript 消费）。
     *  写入经 faux-script.mjs 独立模块（R1 启发式拆分，先例同 results-log.mjs）。 */
    writeFauxScript(map) {
      return writeFauxScriptFile(this.fauxScriptPath, map);
    },
    cleanup() {
      try {
      ...
      if (savedDataDirEnv !== undefined || isFauxMode()) {
        if (savedDataDirEnv === undefined) delete process.env.TAIJI_AGENT_DATA_DIR;
        else process.env.TAIJI_AGENT_DATA_DIR = savedDataDirEnv;
      }
    },
  };
}
 ...
 *           model: string, label: string, keepBuiltinTools?: boolean,
 *           fauxScriptPath?: string }} opts
 ...
 *   使用一致时可传 true。
 *   fauxScriptPath: faux 通道（PI_PROBE_FAUX=1）必传——model-keyed 响应脚本 JSON 路径，
 *   经 TAIJI_FAUX_SCRIPT 注入（主 pi 与其 subagent 子进程共享；子进程经扩展 spawn 的
 *   env 继承透传，各自按 --model 选队）。
 */
export function spawnSession(opts) {
  if (isFauxMode() && !opts.fauxScriptPath) {
    throw new Error(`[${opts.label}] faux 通道（PI_PROBE_FAUX=1）需要 fauxScriptPath（model-keyed 响应脚本）`);
  }
  const args = [
    "--no-extensions",
    "--extension", SW_EXTENSION,
    "--extension", SR_EXTENSION,
    ...(isFauxMode() ? ["--extension", FAUX_EXTENSION] : []),
  ...
  // TAIJI_SUBAGENT_RELAY_*（改道 getPiInvocation relay 分支）。探针 pi 必须以 root 层、
  // 自身 cwd 的干净身份运行，否则 A4 子 session 扫描/A6 records 扫描恒空。
  const childEnv = { ...process.env };
  for (const key of Object.keys(childEnv)) {
    if (key.startsWith("PI_SUBAGENT_") || key.startsWith("TAIJI_SUBAGENT_RELAY_")) delete childEnv[key];
  }
  // [引擎协议化 W4 适配] 裸 pi 探针进程的引擎发现根：runtime dev 链路由装配器注入
  // TAIJI_AGENT_ENGINE_ROOTS；裸 spawn 不经该链，缺此根 → 派发报 engine_not_found:
  // engine 'pi' is not registered（2026-09-14 引擎协议化后新增前置；探针上次真跑
  // 09-05 早于该变更）。根指向仓库 packages/（workspace 源码形态，bin/
  // pi-subagent-cli.mjs）而非 staged bundle（apps/electron/resources/engines/<id>/
  // index.js）——pi-invocation 的自举守卫按 bin 名识别自身，staged index.js 形态漏判
  // → 引擎把「自己」当 pi 入口递归 spawn（协议服务器冒充 pi rpc 子进程，run 永久
  // 卡死，2026-09-15 faux 翻轨实测定位）；bin 形态守卫命中 → 落 PATH pi 正确解析。
  // 不用 SW_EXTENSION/node_modules（pnpm symlink 不被 scanEngineRoot 下钻——
  // Dirent.isDirectory() 对 symlink 恒 false）。taiji runtime 生产链路不受此影响
  //（relay 分支先行）。
  if (!childEnv.TAIJI_AGENT_ENGINE_ROOTS) {
    childEnv.TAIJI_AGENT_ENGINE_ROOTS = join(REPO_ROOT, "packages");
  }
  if (isFauxMode()) childEnv.TAIJI_FAUX_SCRIPT = opts.fauxScriptPath;
...
export function dryRunReport({ scenarioId, design, expect, plan, scriptFile }) {
  ...
  console.log(
    `  模型: ${model}${isFauxMode() ? " (faux 通道——主 pi 演员；subagent 派发经脚本 model 覆盖)" : process.env.PI_PROBE_MODEL ? " (PI_PROBE_MODEL)" : " (默认)"}`,
  );
...
// ── faux 通道原语（PI_PROBE_FAUX=1）────────────────────────────────────────

/** faux 通道：主 pi 派发步（model-keyed 脚本主队列的 step 1）——starts → 同轮 N 连
 *  subagent start toolCalls（脚本化调用 100% 确定，取代「prompt 指令 + 盼 LLM 照做」）。
 *  starts[].model（faux 槽位演员）随 args 下发——subagent 工具的 model 覆盖参数，经
 *  扩展 spawn 透传为子进程 --model（buildSpawnArgs），即「模型注入透传」的被测链路。 */
export function fauxDispatchStep(starts) {
  return {
