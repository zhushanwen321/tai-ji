...
//   PI_PROBE_FAUX=1 启用 faux LLM 通道（凭证无关：主/子 pi 全走 faux 演员，派发脚本化，
//   两子进程统一槽位演员 = 同构 sleep+回复；faux 下总时长缩到 ~1.5-2 分钟）
//   含 2×60s sleep + 三次进程生命周期，长时场景——真实 LLM 轨总时长约 4-6 分钟。
...
async function runProbe(checks, ws, sessions) {
  const starts = [60, 61].map((n) => ({
    ...
    slug: `v3-slow-${n}`,
    collect: "sync",
    ...(C.isFauxMode() ? { model: C.FAUX_CHILD_MODELS[0] } : {}),
  }));
  // faux 通道：两子进程统一槽位演员（同构 sleep+回复——V3 本就要求两成员行为一致）；
  // 子进程 sleep/echo 真实执行（--approve 自动批准）
  if (C.isFauxMode()) {
    ws.writeFauxScript({
      [C.FAUX_PARENT_MODEL]: [C.fauxDispatchStep(starts), { text: "Dispatched." }],
      [C.FAUX_CHILD_MODELS[0]]: [
        { toolCalls: [{ name: "bash", args: { command: "sleep 60 && echo marker-v3" } }] },
        { text: "v3-done" },
      ],
    });
  }
