 ...
 *
 * [HISTORICAL] 2026-09-02 会话丢失事故：测试进程在 env 带 TAIJI_AGENT_DATA_DIR=真实
 * ~/.taiji 时运行（globalSetup 旧版「尊重已有 env」使 tmp 重定向失效），
 ...
 *   2. $TAIJI_AGENT_DATA_DIR     —— globalSetup 注入的 tmp / CI 自定义 / dev 实例（pnpm dev
 *                                  的 Electron main 会注入 ~/.taiji-dev）；注入值经合法性
 *                                  过滤（taiji-full-rename R3），不落在 tmpdir()/
 *                                  ~/.taiji-dev 之下的值剔除出白名单并 console.error
 *                                  一次性说明——指向真实目录（含改名前旧形态）的注入不再
 *                                  因「env 已设」被放行；globalSetup 入口另有白名单形态
 *                                  fail-fast 拒跑（与 guard 共用 isInjectedEnvAllowed 判定）
 *   3. ~/.taiji-dev            —— 缺省 dev 数据目录（homedir 动态推导）
 * 真实数据目录 ~/.taiji 无条件拒绝（先于白名单判定——即使白名单逻辑未来被改坏，
