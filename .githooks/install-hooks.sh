...
if [ "${TAIJI_PRE_COMMIT_REEXEC:-0}" != "1" ]; then
    _taiji_self_copy="$(mktemp -t taiji-pre-commit.XXXXXX)"
    cp "$0" "$_taiji_self_copy"
    TAIJI_PRE_COMMIT_REEXEC=1 exec bash "$_taiji_self_copy" "$@"
fi
trap 'rm -f "$0"' EXIT
...
#    迁自旧 pi 扩展仓（已废弃，见 git 历史与 AGENTS.md）的 pre-commit，针对已迁移的 extensions/ 包：
