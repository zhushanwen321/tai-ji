...
3. 禁止 cw v1 工作流临时产物出现在根目录（应归档到 .taiji-harness/）
...
# 这些是 cw 跑完的运行时/归档产物，应放 .taiji-harness/，不是源码。
...
ROOT_FORBIDDEN_PATTERNS = [
    ...
    re.compile(r"^review-.*\.md$"),               # 根目录散落的审查报告 review-*.md（应归档到 .taiji-harness/）
]
...
def check_root_temp_artifacts(staged_files):
    ...
    应归档到 .taiji-harness/。根目录散落会污染项目根，且易被误判为源码。
