...
// 9 step）的逐 step 镜像移植，与全局 saved review-fix-loop.dwf.ts / pi 内置
...
name: pr-lifecycle
description: >-
  taiji PR 全链生命周期单 workflow：9 step 门禁与条件评审修复循环，终态停在 push 授权前，与 zcode 原生版 pr-lifecycle.dwf.ts 语义一致
...
phases: ['发起前检查', '静态门禁', 'PR 元数据起草与 changeset 复核', '约束加载与覆盖率/度量门禁', '条件评审修复循环', '并行多维审查（4 个一批）', '聚合去重与修复分组', '分组并行修复（3 个一批）', '代码简化', '终局三道门禁']
parameters:
  type: object
  properties:
    base:
    ...
    skipSteps:
      type: string
      description: 跳过的 step id，逗号分隔（人工接管逃生舱；合法值 preflight/static-gate/pr-meta/pr-submit/constraints/gate-suite/cr-fix/simplify/final-gates，终态逐项披露）
usage: |
  ## 使用说明
...
let reviewModeNote = null;

log("[pr-lifecycle] 发起：base=" + base + " maxRounds=" + maxRounds +
  (reviewers.length ? " reviewers=[" + reviewers.join(",") + "]" : " reviewers=缺省（preflight 分支形态判定）") +
...
// 下游 changeset 复核（pr-meta）/ static-gate 的 skill-yaml 条件校验 / coverage 的
// sharedSrcArgs / final-gates / cr-fix 等消费点依赖该值，空串渗透会渲染出畸形 git 命令
// 且空 stdout 被当「无改动」静默消费。解析失败与参数校验同类别（发起方配置错误）：
// throw 走 script error，重新发起是唯一恢复路径。
...
await step("gate-suite", async () => {
  ...
  for (let round = 1; round <= GATE_SUITE_ROUNDS; round++) {
    ...
    if (cov.exitCode === 0 && met !== null && met.exitCode === 0) {
      ...
      const stat = await runCmd("git", ["diff", "--stat", baseHash + "...HEAD"]);
...
await step("simplify", async () => {
  if (crFixRan && crFixTerminated !== "clean" && crFixTerminated !== "converged") {
    skippedStepsList.push({ step: "simplify", reason: "cr-fix 未 clean/converged（" + (crFixTerminated || "未知终态") + "），简化缺位（仅在 review 收敛后执行）" });
    return;
  }
