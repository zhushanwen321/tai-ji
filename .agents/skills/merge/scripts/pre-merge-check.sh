...
if [[ -f "package.json" ]]; then
    ...
    if [[ -f "pnpm-lock.yaml" ]] || grep -q '"packageManager":.*"pnpm@' package.json 2>/dev/null; then
        ...
        # 下面这段 npm ci/install + 子目录扫描逻辑只对非 pnpm 项目执行；taiji 走上面的 pnpm 分支，
...
if [[ -f "package.json" ]]; then
    ...
    if [[ -n "$test_script" ]]; then
        ...
        if echo "$test_script" | grep -q "echo.*no test specified"; then
            ...
            echo "  运行 TAIJI_SKIP_REAL_PI=1 pnpm test ..."
            run_check "单元测试（unit 轨）" env TAIJI_SKIP_REAL_PI=1 pnpm test
            TEST_RAN=true
        fi
    fi
fi
