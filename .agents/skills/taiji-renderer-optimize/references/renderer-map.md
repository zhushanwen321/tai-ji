...
pnpm --filter @taiji/core test                                       # core 包独立 vitest

# typecheck / lint
pnpm --filter @taiji/frontend run typecheck                          # vue-tsc --noEmit
...
- `docs/adr/decisions.md` ADR-0039 — 消息 shallowRef 决策（性能基线）
- `docs/adr/decisions.md` ADR-0049 — Map 分区范式 SSOT（CR checklist 语义并入条目正文）
