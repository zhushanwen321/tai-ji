...
//   PR 规模披露) → cr-fix(条件：默认不派/显式 reviewers 才派) → simplify(条件) → final-gates
//
// cr-fix 维度集判定（与 SKILL.md「审查维度集」节同规则）：
//   默认不派代码审查维度（与分支名无关）——显式 reviewers 参数（白名单裁剪）是唯一
//   开启方式。type-safety / test-coverage 已按设计裁决退役，不进任何集合。
//
