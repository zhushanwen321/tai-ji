...
// TAIJI_AGENT_SUBAGENT=1——subagent-engine-sdk env.ts 的 buildEngineChildEnv 对每个引擎
...
export const SUBAGENT_MARKER_ENV = "TAIJI_AGENT_SUBAGENT";
...
export interface GuardStaleCtxOptions {
	 ...
	 * stale 时以捕获的错误为参调用。缺省 = 仅 TAIJI_AGENT_DEBUG=1 时写一行 stderr
...
/** 缺省降级通知：TAIJI_AGENT_DEBUG=1 时 stderr 一行（对齐 extensions 包 TAIJI_AGENT_DEBUG 惯例）。 */
function defaultOnStale(label: string, error?: unknown): void {
	if (process.env.TAIJI_AGENT_DEBUG !== "1") return;
	const detail =
		error === undefined
