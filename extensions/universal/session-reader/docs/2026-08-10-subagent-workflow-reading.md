...
pi-subagent-workflow 产生的记录物理上都在 `agentDir`(动态推导,默认 `~/.pi/agent`,taiji 下为 `~/.taiji/pi/agent`)下:
...
| **5. 路径动态推导** | 4 | dev 数据目录 `~/.taiji-dev/pi/agent` | 无硬编码 `~/.pi` |
...
- **agentDir 注入**:确认 taiji runtime 调 session_read 传入 agentDir 正确指向 `~/.taiji/pi/agent`。
