...
/** 真实 pi agent 目录（pi 官方家目录布局，随宿主 homedir 推导），用于集成测试。 */
 ...
 * 按 sid 运行时定位而非硬编码宿主 workspace 目录名：真实文件挂在哪个 slug 目录随采集机器而异，
 * 硬编码会绑死本机路径形态；定位不到时回落相对布局，由下方 HAS_REAL_SESSION 守卫 skip。
 */
export const REAL_SESSION = (() => {
  const fallback = join(REAL_AGENT_DIR, 'sessions', `2026-05-28T03-17-12-844Z_${E6}.jsonl`)
  if (!existsSync(REAL_AGENT_DIR)) return fallback
  try {
    return (
      execSync(
        `find ${REAL_AGENT_DIR}/sessions -name '*${E6}*' -name '*.jsonl' ! -name '*.finalized' 2>/dev/null | head -1`,
        { encoding: 'utf8' },
      ).trim() || fallback
    )
  } catch {
