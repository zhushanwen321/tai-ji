...
// 不做编排、不发通知、不碰文件系统：session 边界写盘判定的调用时机、ctx.ui.notify 调用与
// session 生命周期清理归 u-ack-turn 单元接线。
//
// 用户可见的失败形态只有两种（`AckNotifyReason`，见 types.ts）：覆写不可用（创建时同步）
// 与合成轮未启动（session 边界判定后异步）。两者共用同一条 i18n 文案与同一级别——形态差异只影响
...
/**
 * session 边界写盘判定：只有「会话文件不存在 **且** 我们的 ack 轮从未启动」才判确实未落盘。
 *
 * 为什么必须带 `ackTurnStarted` 条件：慢速真实轮的应答可能在边界时仍未落盘，此时
 * `ackTurnStarted` 已为 true 而文件尚不存在——「文件不存在」不构成未落盘证据，报了就误伤
 * 正常路径（边界判定不是写盘的最后期限，真实轮完成即落盘）。反之 ack 轮从未启动说明覆写
 * 没机会生效，文件又不存在 ⇒ 任务确实没写入，补发如实文案。
 */
 ...
 * 去重是必需的：同 session 同 task 的判定可能在多个时机触发（重复建任务、多次 session 边界）。
