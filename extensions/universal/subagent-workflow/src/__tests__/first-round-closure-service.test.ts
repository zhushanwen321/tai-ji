// src/__tests__/first-round-closure-service.test.ts
//（P1 抽包留壳：subject 为 subagent-core 件真链路，注入 pi/session-delivery 真机制，见 impl-plan 偏差 #17）
//
// [V2 决策 2/3 → W3 协议形态改写] 首轮闭环（chat 形态轮次收敛）。
//
...
//     notify（record status="running"——chat 轮次完成形态，round 透传供 dedup key 递增）；
...
function makeTmpAgentDir(): string {
  return fs.mkdtempSync(path.join(os.tmpdir(), "first-round-closure-"));
}
...
describe("[V2 决策 2/3] 首轮闭环：run 应答 settle（协议形态）", () => {
  let agentDir: string;
  ...
  it("首轮 run 应答 settle：running(idle 折入) + round 0→1 + notifyComplete（status=running、round 透传）", async () => {
    ...
    // 载荷 status 仍 "running"（chat 轮次完成对主 agent 的对话语义，载荷与实态正交）。
  ...
  it("[M3] chat 轮次完成通知立即送达（不挂 60s 合并窗口）+ 同 id:round dedup 吞第二次", async () => {
    // 生产场景：chat 轮次完成（轮终翻 idle + arm idle timer → run 应答 settle →
