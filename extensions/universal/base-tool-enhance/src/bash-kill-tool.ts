 ...
 * **不可 kill**（跨进程边界：处置权归发起进程；孤儿收殓已下沉 taiji runtime，
	...
	return {
		...
		async execute(
			...
			if (fromStore === undefined) {
				...
				// 不可 kill：处置权归发起进程；属主若已死，孤儿由 taiji runtime 在
				...
				return killedFalse(
					...
					"the task is managed by the pi process that started it (bash_kill from that session); " +
						...
						"inside the taiji runtime it is collected when the owner's session ends or on app restart; " +
