...
配置文件：`<agentDir>/config/rename-session-ext-config.json`（`<agentDir>` 默认 `~/.pi/agent`，`PI_CODING_AGENT_DIR` 可覆盖；taiji 隔离环境为 `~/.taiji/agent`）。
...
1. **`<agentDir>/auto-rename-enabled` flag 文件**（存在 = 开）：taiji runtime 的开关契约——桌面端 SystemPage 开关、首启默认开启都写这个文件。**taiji 用户请通过桌面端开关或 `/auto-rename` 命令管理，不要手改 JSON 的 `enabled`**（flag 存在时视为开，手改会被覆盖）。
