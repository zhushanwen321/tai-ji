...
1. **`<agentDir>/auto-rename-enabled` flag 文件**（存在 = 开）：这是 taiji runtime 的开关契约（SystemPage 开关 / 首启默认开启都写这个文件，live 检查每次事件生效）。**taiji 用户不要手改 JSON 里的 enabled**——桌面端的开关状态存在 flag 文件里，手改 JSON 会被 flag 覆盖（flag 存在即视为开）。
