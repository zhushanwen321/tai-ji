...
- **预算控制**：仅 token 维度预算（耗时仅记账显示，不作预算维度）。70% 预警、90% 发收尾 steering、100% 且已发收尾提示后进入 budget_limited 终态。默认不设预算，仅在用户显式要求时设置
...
ln -s /path/to/taiji/extensions/universal/goal \
