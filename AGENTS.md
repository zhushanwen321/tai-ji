...
| 待执行架构任务 | [docs/todo/remote-use-merge-architecture.md](docs/todo/remote-use-merge-architecture.md)（合并 remote-use 后删除）· [docs/todo/tdflow-two-phase-workflow.md](docs/todo/tdflow-two-phase-workflow.md)（tdflow 两阶段工作流设想：tech-design × dev-flow 参数化编排；plan 包存废与 goal 桥设计（已搁置）随其落地一并裁决） |
