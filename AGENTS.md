...
- **验收构造 mock pi = `node_modules/.bin/pi` 文件级置换**：runtime 的 pi 路径经 `find-pi-executable` 解析（dev 序列：resources/pi 缺失时 PATH → `node_modules/.bin/pi`）且被 runtime 进程缓存——mock 注入只能置换该路径的文件（`mv` 原 symlink 为 `.bak` → 放 mock 可执行脚本 → 新 session 生效于新 spawn）。窗口结束必须恢复：`mv` 回原 symlink + `--version` 输出真实版本 + `ls -l` 形态核对，三证齐才算恢复；窗口期间禁起依赖真实 pi 的新场景
...
- **数据目录缺省反转（C-proc-26，fail-safe default）**：`getDataDir()` 缺省 = `~/.taiji-dev`——prod 目录只能被显式声明触达：打包 main 经 `resolvePackagedDataDir` 钉死（外部值仅 `~/.taiji` 树内采信）并自置位 `TAIJI_AGENT_PACKAGED=1`、dev main 经 `resolveDevDataDir`（`~/.taiji-dev` 树内采信），main spawn runtime 成对注入 `TAIJI_AGENT_PACKAGED` + `TAIJI_AGENT_DATA_DIR`，runtime 组合根交叉断言缺一 fail-fast；裸跑（手动 tsx / 验证脚本 / 探针）一律落缺省 dev 树。**prod 值准入守卫（第二支柱）**：非 PACKAGED 进程持有 `~/.taiji` 树值时 `getDataDir()` 直接 throw——堵「绕过 vitest config 的执行器（bun test 等）+ 宿主泄漏 prod env」组合（2026-09-22 事故残余形态），守卫挂在唯一入口、不依赖 config 被加载。脚本起 runtime 必须显式注入数据目录（mkdtemp / dev 树），`validate-runtime-bundle.sh` 已内置该注入
