...
// CDP 端口：per-worktree 派生（dev-instance.mjs 注入 TAIJI_CDP_PORT，C-dev-01），
// 未注入保持 9222（裸启动兼容）。argv 显式传了 --remote-debugging-port 则尊重不覆盖。
if (!args.some((a) => a.startsWith('--remote-debugging-port'))) {
  args.push(`--remote-debugging-port=${process.env.TAIJI_CDP_PORT ?? '9222'}`)
}

if (process.env.TAIJI_DEV_ELECTRON_VERBOSE === '1') {
...
if (binary !== SOURCE_BINARY) env.TAIJI_DEV_BUNDLE_ICON = '1'
