...
export const VITE_DEV_URL = process.env.TAIJI_VITE_DEV_URL ?? 'http://localhost:1420'
...
async function loadWindowContent(
  ...
  try {
    if (!isE2E && deps.isDev) {
      ...
      // DevTools 默认关闭：需要时显式 TAIJI_DEVTOOLS=1 npm run dev 打开
      if (process.env.TAIJI_DEVTOOLS === '1') {
...
export async function createWindow(
  ...
  win.once('ready-to-show', () => {
    ...
    //   - TAIJI_E2E=1：E2E 构建产物形态
    //   - TAIJI_DEV_BACKGROUND=1：AI agent 真机验收的 dev 实例（C-dev-01，多 agent 并行
    //     dev 时不得打断用户前台工作）
    if (process.env.TAIJI_E2E === '1' || process.env.TAIJI_DEV_BACKGROUND === '1') {
      win.showInactive()
    } else {
      win.show()
    }
