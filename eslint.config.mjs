...
export default [
      ...
      'no-restricted-imports': [
        'error',
        {
          patterns: [
            ...
            // domain 内文件禁止 import 任何「@taiji/core/domain/<域>/<内部模块>」包名路径（含同域内部路径——
            // 域内应走相对路径）。合法形态：单层 '@taiji/core/domain/<域>'（index.ts 公开 API，
            // minimatch * 不跨 / 故单层不匹配下方 pattern）或 '@taiji/core'（包入口 index.ts）。
            // 相对路径跨域（深度可变，patterns 无法表达）由 scripts/check-domain-boundaries.sh 兜底。
            // 2026-08-04 审计：domain 下零包名内部路径 import，规则落地零命中。
            { group: ['@taiji/core/domain/*/*'], message: 'AC10 跨域铁律：domain 内禁 import 域内部模块（包名形式）——经 @taiji/core/domain/<域> 公开 index API 或 @taiji/core 包入口消费' },
            { group: ['@taiji/core/domain/*/**/*'], message: 'AC10 跨域铁律：domain 内禁 import 域内部深层模块（包名形式）——经公开 index API 消费' },
          ],
        },
      ],
  ...
  // debug → TAIJI_AGENT_DEBUG=1 文件日志）。范围限 src 源码：测试文件的 spyOn/monkey-patch
  ...
  // 规则目标是业务/feature 组件直接用原生表单元素（应换 @taiji/ui 组件）；primitives/**
  // 正是 @taiji/ui 组件的实现层——Input.vue/Textarea.vue 的本职就是包装原生
